...
- `issue-triage-transition`: maintainer/collaborator confirmation to
  request-only Issue Fix Request carriers for `ready-for-agent`
  recommendations.
...
| `issue-triage-transition` | `zj-loop-issue-triage.yml` | Enable when maintainers should confirm recommended triage transitions and create request-only Issue Fix Request carriers. |
...
| `issue-triage-transition` | `request-only` with replayed runner | Validate maintainer/collaborator confirmation and create/dedupe independent Issue Fix Request carriers for `ready-for-agent`; covered by `issue-backlog-triage -> issue-triage-transition` E2E replay; no live tracker mutation yet. |
