...
**一句话需求，走到已合并的生产变更。** 在一次 ZCodeGraph roadmap slice 中，用户只给 Codex 一个产品目标：把 C baseline extraction 迁移到 Rust-owned 执行，找一个合适规模的真实 GitHub C 项目验证通过，并在成功后删除已迁移的 TypeScript 实现。

借助 [Roadmap-Sliced Development](patterns/roadmap-sliced-development.md)，这个 loop 把工作推进完整交付路径：

- 创建有边界的分支：`zjal/rust-owned-c-baseline`。
- 把需求拆成 leaf nodes：Rust-owned C 注册、extraction parity、fixture 覆盖、真实 corpus 验证、TypeScript 路径删除。
- 记录范围决策：`.c` 与 C-classified `.h` 归 Rust-owned，C++ / Objective-C header ambiguity 保留为明确边界，并记录 corpus 选择。
- 实现 Rust-owned C extraction：functions、structs、enums、enum members、typedefs、includes、calls、variables 和 ownership metadata。
- 本地验证：Cargo、npm build、Vitest smoke/fallback/engine tests、定向 extraction tests 和 `git diff --check`。
- 在真实项目 [`DaveGamble/cJSON`](https://github.com/DaveGamble/cJSON) commit `fb16e5c` 上验证：121 个 indexed files、3,581 个 nodes、7,000 条 edges，`engineByLanguage.c = rust`，没有 Rust-owned C gap diagnostics。
- 在 ZCodeGraph docs 中沉淀 durable evidence，并打开 [PR #677](https://github.com/jununfly/ZCodeGraph/pull/677)。
- 用户合并 PR 后，继续完成 local main fast-forward、本地分支删除、远端分支删除。

这背后的承诺是：**给 agent 一个产品目标，而不是任务清单；loop 会保留足够状态，把完整交付路径走完。** 关键不是代码生成本身，而是连续性：决策被写下来，验证从 fixture 走到真实 corpus，第一个失败 corpus 变成被记录的边界，PR handoff 没丢，merge 后清理也没丢。
