...
| 添加 GitLab CI smoke/consumer jobs | `npx @jununfly/zj-loop-init . --add gitlab-ci` |
...
# 可选：安装 GitLab CI provider adapter
...
GitHub Actions 是 GitHub provider adapter，不是所有平台的通用自动化层。
在 GitLab 或 self-managed GitLab 项目中，优先安装 GitLab CI adapter：
...
GitLab 生成文件位于 `zj-loop/gitlab-ci/`。如果项目没有根 `.gitlab-ci.yml`，
init 会创建 include 文件；如果已有根 CI，默认跳过并输出 next steps。GitLab
jobs 会产出 `route-decision.json`、`consumer-plan.json` 等 artifact，并使用
GitLab issue note、MR、pipeline、job artifact 语义。Report-only job 不需要 mutation
token；issue note、label、branch、MR 或 cleanup 需要 `GITLAB_TOKEN` 和 route guard。

如果要做未发布版本 dogfood 或内部验证，可以把 core package source 直接渲染进
...
`zj-loop`；如果你的项目只开放了已有 stage，比如 `Fallback`，安装或升级时传：
...
仅 vendor tarball 并不等于 GitLab adapter 已经完全离线：`npm exec` 仍可能需要
registry 或预热 npm cache 来解析 transitive dependencies。完全离线执行应作为单独
packaging 决策处理。

GitLab generated jobs 会让 blocked/refused consumer plan 也保持可观察：
`consumer-plan.json` 会继续作为 artifact 上传。CI Sweeper 还会在 dry-run 验证中
生成 `issue-fix-request.md` 和 `issue-fix-request-result.json`，用于检查 GitLab
fix scope，而不会直接创建 tracker item。
...
`@jununfly/zj-loop-core` package pin、Route Table readiness contract，以及
GitHub/GitLab provider parity：
