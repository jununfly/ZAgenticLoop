...
| Roadmap activation | `roadmap-activation-dispatcher.mjs`, `zj-loop-activation-contract.mjs`, route table `roadmap-sliced-development` row, `zj-loop/roadmap-activation-state.md` | Install-ready request-only activation-comment route with live dogfood bootstrap | Authorized slash commands create append-only activation requests only. Route Table truth is `consumer_kind: activation-consumer`, `execution.mode: request-only`, and `maturity.runner: install-ready`; Roadmap-Sliced Development consumes explicit issue/request ids and owns branch, roadmap, implementation, verification, and PR handoff. |
| PR Steward routes | PR Steward report/fix-request/claim replay scripts, generated workflow-dispatch `live-repair` path, `zj-loop/pr-steward-state.md` | Report-only plus install-ready claim-only route with stronger dogfood evidence | Report routes do not comment, label, rebase, merge, dispatch workflows, repair, or open Fix PRs. Fix request Route Table truth is `consumer_kind: fix-runner`, `execution.mode: claim-only`, and `maturity.runner: install-ready`; workflow-dispatch dogfood run `29237613749` used `core_package=./tools/zj-loop-core`, executed guarded `live-repair`, uploaded `live-repair-result.json`, and produced verifier-backed `escalation-issue` evidence after local action-path execution without mutating the source PR or pushing a repair branch. Run [29381809888](https://github.com/jununfly/ZAgenticLoop/actions/runs/29381809888) additionally consumed a bounded request for controlled draft PR [#121](https://github.com/jununfly/ZAgenticLoop/pull/121), verified its current head SHA, and emitted dry-run escalation evidence without a repair command, confirmation, live-repair artifact, or source PR mutation. Route enablement and live side effects remain separate guarded decisions. |
...
| Dependency Sweeper routes | Dependency route/claim replay scripts, replayed live runner, generated workflow-dispatch `live-repair` path, `zj-loop/dependency-sweeper-state.md` | Claim-only route with live recovery evidence | Supports bounded request and claim lifecycle evidence plus replayed `repair-pr` / `escalation-issue` live-runner evidence. Workflow-dispatch run [29385283810](https://github.com/jununfly/ZAgenticLoop/actions/runs/29385283810) used released core `0.1.8`, recovered a residual candidate branch with `--force-with-lease` only after confirming no open repair PR, passed both verifier gates, and created [repair PR #128](https://github.com/jununfly/ZAgenticLoop/pull/128), which a human merged. Earlier probes exposed and then fixed CLI, bot-identity, token, write-permission, and lease-refresh gaps. Route Table promotion remains a separate explicit decision; the runner did not auto-merge. |
| Changelog Drafter | `.github/workflows/zj-loop-changelog-drafter.yml`, `zj-loop/orchestrations/*/draft-plan.json`, report/draft-request replay scripts, guarded live runner, `zj-loop/changelog-drafter-state.md` | Install-ready report/draft-request path with stronger dogfood evidence | Records release-window evidence and draft request candidates. Route Table truth is `consumer_kind: draft-consumer` and `maturity.runner: install-ready`; generated GitHub/GitLab workflows turn supplied draft request JSON into `zj-loop.signal.v1`, run `zj-loop-dispatch --mode auto`, and upload orchestration `draft-plan.json` as the review artifact. Guarded `live-draft` requires `CREATE_CHANGELOG_DRAFT_PR_OR_EVIDENCE`; workflow-dispatch dogfood run `29247657726` produced `draft-evidence` while proving `tag_created=false`, `release_created=false`, `package_published=false`, and `final_changelog_acceptance=false`. Route Decision still does not generate release notes, edit changelogs, create PRs, tag, release, publish, dispatch workflows, or finalize changelog acceptance. |
