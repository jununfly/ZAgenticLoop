...
| PR Steward routes | PR Steward report/fix-request/claim replay scripts, generated workflow-dispatch `live-repair` path, `zj-loop/pr-steward-state.md` | Report-only plus claim-only route with execution-ready runner evidence | Report routes do not comment, label, rebase, merge, dispatch workflows, repair, or open Fix PRs. Fix request Route Table truth is `consumer_kind: fix-runner`, `execution.mode: claim-only`, and `maturity.runner: execution-ready`; workflow-dispatch dogfood run `29237613749` used `core_package=./tools/zj-loop-core`, executed guarded `live-repair`, uploaded `live-repair-result.json`, and produced verifier-backed `escalation-issue` evidence after local action-path execution without mutating the source PR or pushing a repair branch. Route enablement and live side effects remain separate guarded decisions. |
