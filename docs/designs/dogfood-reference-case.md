...
| Changelog Drafter | `.github/workflows/zj-loop-changelog-drafter.yml`, report/draft-request replay scripts, guarded live runner, `zj-loop/changelog-drafter-state.md` | Report-only route with execution-ready draft evidence/PR runner | Records release-window evidence and draft request candidates. Route Table truth is `consumer_kind: draft-consumer`, `execution.mode: report-only`, and `maturity.runner: execution-ready`; workflow-dispatch dogfood run `29247657726` used `core_package=./tools/zj-loop-core`, executed guarded `live-draft`, uploaded `live-draft-result.json`, and produced `draft-evidence` while proving `tag_created=false`, `release_created=false`, `package_published=false`, and `final_changelog_acceptance=false`. Route Decision still does not generate release notes, edit changelogs, create PRs, tag, release, publish, dispatch workflows, or start consumer work. |
