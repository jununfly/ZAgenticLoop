...
> 数据文件: `release-dependency-roadmap.json` | 最后更新: 2026-07-01 11:49:09
...
├── [~][X+] 1-5. 定义可发布标准与发布包宇宙
│   ├── [ ][Y+] 1-5-1. 盘点 release manifest 与 package publish 信号差异
│   ├── [ ][X+] 1-5-2. 决定 zj-loop-sync 与 zj-loop-mcp-server 发布身份
│   └── [ ][Y+] 1-5-3. 固定发布宇宙验收标准
