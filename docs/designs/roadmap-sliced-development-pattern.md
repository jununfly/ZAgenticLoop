...
This document summarizes the current human-agent development pattern observed in
this repository. It is written in the same shape as `patterns/`, but it is not
yet registered as a public reusable Pattern.

Working name: **roadmap-sliced-development-pattern**  
...
The process roadmap for this candidate pattern recorded 35 decisions. All
...
If this graduates into `patterns/`, a possible registry entry would be:

```yaml
- id: roadmap-sliced-development-pattern
  name: Roadmap-Sliced Development Pattern
  goal: Drive ambiguous architecture/product/release initiatives through bounded roadmap-backed slices
  cadence: human-pulled
  risk: medium
  tools: [codex, claude-code]
  skills: [zj-grill-me, zj-roadmap-driven, zj-grill-with-docs]
  state: docs/plans/<initiative>-roadmap.md
  phases: [explore, decide, slice, verify, commit, closeout]
...
Graduation condition: keep this as a candidate design doc until the next roadmap
initiative completes one real run using the seven-item Minimal Viable Checklist.
After that run, decide whether it should be registered in `patterns/registry.yaml`
as a public loop Pattern.
