...
| `issue-triage-transition` | `triage-transition-confirmation` | Issue Triage Transition | Request-only. Consumes maintainer/collaborator confirmed recommended transitions, validates the fixed confirmation command/phrase, and for `ready-for-agent` creates or dedupes a source issue Issue Fix Request comment. A Route Table allowlisted trusted automation path may skip the human phrase only for that same `ready-for-agent` request-carrier boundary. Live source issue tracker label/state mutation requires later explicit promotion. |
