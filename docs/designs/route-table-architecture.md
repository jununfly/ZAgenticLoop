...
| `issue-triage-transition` | `triage-transition-confirmation` | Issue Triage Transition | Dry-run only. Consumes maintainer/collaborator confirmed recommended transitions, validates the fixed confirmation command/phrase, plans `zj-triage` role/comment side effects, and for `ready-for-agent` plans an Issue Fix Request. Live tracker mutation requires later explicit promotion. |
