...
| 2-init-existing-loop-safety | completed | `init: guard existing loop contract overwrite` | `cd tools/zj-loop-init && npm test`; `npm run check:zj-loop-init`; `git diff --check` | Existing `zj-loop/ZJ-LOOP.md` is skipped by default with explicit next step; `--force` overwrites with warning. `cd tools/zj-loop-init && npm test` and `npm run check:zj-loop-init` passed. |
| 3-daily-triage-runtime-state | completed | `init: scaffold daily triage runtime state safely` | `cd tools/zj-loop-init && npm test`; `bash scripts/ci-audit-gates.sh`; `git diff --check` | Daily-triage init now creates local runtime files plus `.example` templates, and adds runtime files to target `.gitignore` without breaking audit presence checks. `cd tools/zj-loop-init && npm test` and `bash scripts/ci-audit-gates.sh` passed. |
| 4-readiness-execution-language | completed | `docs: separate readiness level from execution authority` | `npm run validate:registry`; `bash scripts/ci-audit-gates.sh`; `git diff --check` | README, Quickstart, and package READMEs now distinguish structural readiness/cost level from Route Table execution authority; cost examples use local project registry by default. All declared gates passed. |
