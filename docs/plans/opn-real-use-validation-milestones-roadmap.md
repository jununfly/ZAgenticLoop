<!-- ROADMAP_SECTION_START -->
## ZJ Roadmap

> 数据文件: `opn-real-use-validation-milestones-roadmap.json` | 最后更新: 2026-08-08 00:40:23

[~][X+] 1. OPN 真实可用与用户价值验证里程碑
├── [x][Y+] 1-1. M1：长期开发测试版 Runtime Trust
│   ├── [x][Y+] 1-1-1. 完成 development artifact bootstrap 与 Human approval
│   └── [x][Y+] 1-1-2. 真实 signed Runtime dogfood 启动门禁通过
├── [x][Y+] 1-2. M2：同机 Multi-Agent Graph Atom
│   ├── [x][Y+] 1-2-1. 同机 Agent node enrollment 与中心责任单元
│   ├── [x][Y+] 1-2-2. 同机 Directed Graph 执行、Evidence 与 Human review
│   └── [x][Y+] 1-2-3. 同机真实双 Agent 只读任务通过
├── [x][Y+] 1-3. M3：跨设备 Multi-node Graph Atom
│   ├── [x][Y+] 1-3-1. 跨设备 pairing、device identity 与 secure transport
│   ├── [x][Y+] 1-3-2. 跨设备 reconnect、replay 与 StateStore 协作
│   └── [x][Y+] 1-3-3. 跨设备真实三节点只读任务通过
├── [ ][Y+] 1-4. M4：写入型 OPN Development Job
│   ├── [ ][Y+] 1-4-1. 任务编排资源隔离检查与 worktree 策略
│   ├── [ ][Y+] 1-4-2. 多 Agent commit、中心合并与合并后验证
│   └── [ ][Y+] 1-4-3. 真实写入型开发任务 Human accept 通过
└── [ ][Y+] 1-5. M5：真实用户价值 Dogfood
    ├── [ ][Y+] 1-5-1. 选择真实 dogfood 任务并建立 baseline
    ├── [ ][Y+] 1-5-2. 记录用户价值、体验和失败恢复反馈
    └── [ ][Y+] 1-5-3. 根据真实反馈规划下一里程碑

### 当前施工：1. OPN 真实可用与用户价值验证里程碑

**决策：**
- Q: 当前路线图的目标是正式发布还是先让产品真实可用？ → 先让产品真实可用并验证用户价值；正式发布、Developer ID、notarization、安装包和自动更新后置。 (路线图以 Human 可实际使用的 OPN 协作为出口，不以发布工程完整性作为前置条件。)
- Q: 从当前 Runtime Trust 到真实 OPN co-work 需要几个里程碑？ → 采用五个：M1 Runtime Trust、M2 同机 Graph Atom、M3 跨设备 Graph Atom、M4 写入型 Development Job、M5 真实用户价值 Dogfood。 (每个里程碑必须有可运行出口，不能只完成协议或 fixture。)
- Q: 是否允许在正式发布能力未完成时长期使用 development channel？ → 允许；development channel 是长期支持的开发测试产品形态，具备完整安全门槛但不承诺官方发布者身份。 (不让 Developer ID 阻塞用户价值验证。)

**当前子树：**
├── [x][Y+] 1-1. M1：长期开发测试版 Runtime Trust
│   ... 2 more child nodes; run tree 1-1 --depth 2 for full view
├── [x][Y+] 1-2. M2：同机 Multi-Agent Graph Atom
│   ... 3 more child nodes; run tree 1-2 --depth 2 for full view
├── [x][Y+] 1-3. M3：跨设备 Multi-node Graph Atom
│   ... 3 more child nodes; run tree 1-3 --depth 2 for full view
├── [ ][Y+] 1-4. M4：写入型 OPN Development Job
│   ... 3 more child nodes; run tree 1-4 --depth 2 for full view
└── [ ][Y+] 1-5. M5：真实用户价值 Dogfood
    ... 3 more child nodes; run tree 1-5 --depth 2 for full view
<!-- ROADMAP_SECTION_END -->
