...
├── [x][Y+] 1-7. 用户项目安装、升级与启用体验
│   ├── [x][Y+] 1-7-1. Install Wizard 与 First-Run Guided Setup
│   ├── [x][Y+] 1-7-2. Upgrade 保留用户意图与配置漂移治理
│   ├── [x][Y+] 1-7-3. Route Bundle 选择、启用与回滚命令
│   └── [x][Y+] 1-7-4. README、Quickstart 与场景化教程重构
├── [x][Y+] 1-8. Dogfood 验证、发布门槛与能力分级
│   ├── [x][Y+] 1-8-1. 本仓 Dogfood Matrix 与 Route 能力台账
│   ├── [x][Y+] 1-8-2. 每条 Route 的 E2E Replay 与 Live Evidence
│   ├── [x][Y+] 1-8-3. 发布前 Gate、版本策略与回归套件
│   └── [x][Y+] 1-8-4. 能力分级晋升到 execution-ready 的证据审计
