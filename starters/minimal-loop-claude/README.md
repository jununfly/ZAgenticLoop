...
1. Copy files into your repo:
   ...
   ```bash
   cp -r starters/minimal-loop-claude/.claude/skills/zj-loop-triage .claude/skills/
   cp starters/minimal-loop-claude/.claude/agents/zj-loop-verifier.md .claude/agents/
   mkdir -p zj-loop
...
3. Start the loop (Claude Code):
   ...
   ```bash
   /loop 1d Run $zj-loop-triage. Read zj-loop/STATE.md first. Append high-priority and watch items. Update Last run timestamp. Do not auto-fix anything in week one.
   ```

4. Read `zj-loop/STATE.md` each morning for 1–2 weeks. Tune the triage skill.

5. When triage quality is good, add `zj-minimal-fix` from `templates/SKILL.md.zj-minimal-fix` and enable small auto-wins with the verifier agent (`isolation: worktree` on implementer tasks).
...
| `.claude/skills/zj-loop-triage/SKILL.md` | Triage skill |
| `.claude/agents/zj-loop-verifier.md` | Checker sub-agent for L2+ |
