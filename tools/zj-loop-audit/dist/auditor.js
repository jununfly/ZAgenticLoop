...
async function auditGeneratedWorkflowBundle(fs) {
    const findings = [];
    let workflowEntries;
    try {
    ...
    const generatedWorkflowNames = workflowEntries
        ...
        .sort();
    if (!generatedWorkflowNames.length)
        return findings;
    const routeTableFinding = await auditWorkflowBundleRouteTable(fs);
    if (routeTableFinding)
        findings.push(routeTableFinding);
...
async function auditGeneratedGitLabSubstrateTracking(root, fs) {
    if (!isGitWorktree(root))
        return [];
    const substratePaths = await collectGitLabSubstratePaths(fs);
    if (!substratePaths.length)
        return [];
...
async function collectGitLabSubstratePaths(fs) {
    const paths = [];
    if (await fs.exists('.gitlab-ci.yml'))
        paths.push('.gitlab-ci.yml');
    if (await fs.exists('zj-loop/zj-loop-route-table.yaml'))
        paths.push('zj-loop/zj-loop-route-table.yaml');
...
function gitPathIsTracked(root, projectPath) {
    try {
...
function gitPathIsIgnored(root, projectPath) {
    try {
...
function shellJoinProjectPaths(projectPaths) {
    return projectPaths.map(shellQuote).join(' ');
}
function shellQuote(value) {
    return `'${value.replace(/'/g, `'\\''`)}'`;
}
async function auditWorkflowBundleRouteTable(fs) {
    ...
    try {
        const parsed = parseYaml(routeTable);
...
async function auditRouteExecutionContractWarnings(fs) {
    const routeTable = await fs.readTextIfExists('zj-loop/zj-loop-route-table.yaml');
    if (!routeTable)
        return [];
    const findings = [];
...
function validateRawRouteExecutionContract(route) {
    ...
    if (consumerKind && !limits)
        errors.push(`unknown consumer_kind: ${consumerKind}`);
    if (mode && !EXECUTION_MODES.has(mode))
        errors.push(`unknown execution.mode: ${mode}`);
    if (sideEffectLevel && !SIDE_EFFECT_LEVELS.includes(sideEffectLevel))
        errors.push(`unknown side_effect_level: ${sideEffectLevel}`);
    if (maturityProtocol && !MATURITY_LEVELS.has(maturityProtocol))
        errors.push(`unknown maturity.protocol: ${maturityProtocol}`);
    if (maturityRunner && !MATURITY_LEVELS.has(maturityRunner))
        errors.push(`unknown maturity.runner: ${maturityRunner}`);
    if (limits && mode && !limits.modes.includes(mode))
        errors.push(`${consumerKind} cannot use execution.mode=${mode}`);
