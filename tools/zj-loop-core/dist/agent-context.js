...
export async function persistActivationSnapshotRef(input) {
    ...
    if (checked.error || !checked.value)
        return blockedActivationRef(checked.error ?? "activation-ref-invalid");
    ...
    try {
        ...
        if (existing !== null) {
            if (!isActivationRef(existing))
                return blockedActivationRef("activation-ref-invalid");
            if (sameActivationRef(existing, activation))
                return { schema: "zj-loop.agent_local_activation_ref.v1", status: "duplicate", activation: existing, commit_id: null, side_effects_executed: false };
...
function validateActivationSnapshotRef(value) {
    if (!/^[a-zA-Z0-9_-]+$/.test(value.activation_id))
        return { error: "activation-id-invalid" };
    if (!/^[a-zA-Z0-9_.-]+(?:\/[a-zA-Z0-9_.-]+)+$/.test(value.project_path))
        return { error: "project-path-invalid" };
    if (!/^[0-9a-f]{40}$/i.test(value.commit))
        return { error: "activation-commit-invalid" };
    if (!/^[a-zA-Z0-9_./-]+$/.test(value.path) || value.path.startsWith("/") || value.path.includes(".."))
        return { error: "activation-path-invalid" };
    if (!/^[0-9a-f]{64}$/i.test(value.sha256))
        return { error: "activation-sha256-invalid" };
    return { value };
}
function sameActivationRef(left, right) {
...
function blockedActivationRef(reason, activation = null) {
...
export async function loadAgentContext(input) {
    ...
    try {
        ...
        const records = { handoff, claim, activation, executions, evidence };
...
async function readRecords(client, paths, ref) {
    if (paths.length > 1000)
        throw new Error("state-record-limit-exceeded");
    return Promise.all(paths.map((path) => client.readJson(path, ref))).then((values) => values.filter((value) => value !== null));
}
async function validateImmutableProjectRefs(project, records) {
    const errors = [];
    if (records.handoff) {
        ...
        if (registration === null)
            errors.push(error("registration-missing", "handoff.registration", "registration file is missing at its pinned commit"));
        else if (digestText(registration) !== records.handoff.registration.sha256)
            errors.push(error("registration-sha256-mismatch", "handoff.registration.sha256", "registration digest does not match the handoff"));
    }
    if (records.activation) {
        const activation = await readProjectText(project, records.activation.path, records.activation.commit);
        if (activation === null)
            errors.push(error("activation-missing", "activation.ref", "activation contract is missing at its pinned commit"));
        else if (digestText(activation) !== records.activation.sha256)
            errors.push(error("activation-sha256-mismatch", "activation.ref.sha256", "activation contract digest does not match the activation ref"));
    }
    return errors;
}
async function readProjectText(project, path, ref) {
    if (project.readText)
        return project.readText(path, ref);
...
function safeId(value) { if (!/^[a-zA-Z0-9_-]+$/.test(value))
    throw new Error("agent-id-invalid"); return value; }
function isHandoff(value) { return Boolean(value && typeof value === "object" && value.schema === "zj-loop.agent_handoff.v1"); }
function isClaim(value) { return Boolean(value && typeof value === "object" && value.schema === "zj-loop.agent_claim.v1"); }
function isActivationRef(value) { return Boolean(value && typeof value === "object" && value.schema === ACTIVATION_SNAPSHOT_REF_SCHEMA); }
function isExecution(value) { return Boolean(value && typeof value === "object" && value.schema === "zj-loop.agent_execution.v1"); }
function isEvidence(value) { return Boolean(value && typeof value === "object" && value.schema === "zj-loop.agent_evidence.v1"); }
export function reconstructAgentContext(input) {
    ...
    const currentEvidence = latest
        ? input.records.evidence.filter((item) => item.execution_id === latest.execution_id)
        : [];
...
function blockedSnapshot(stateHead, records, errors) {
    return {
        schema: AGENT_CONTEXT_SNAPSHOT_SCHEMA,
        status: "blocked",
...
function validateRecords(records) {
    const errors = [];
    if (!records.handoff)
        errors.push(error("handoff-missing", "handoff", "handoff record is missing"));
    if (!records.claim)
        errors.push(error("claim-missing", "claim", "claim record is missing"));
    if (!records.activation)
        errors.push(error("activation-ref-missing", "activation.ref", "activation snapshot ref is missing"));
    ...
    for (const [index, execution] of records.executions.entries()) {
        if (execution.handoff_id !== records.handoff?.handoff_id)
            errors.push(error("execution-handoff-mismatch", `history.executions[${index}].handoff_id`, "execution is bound to another handoff"));
        if (execution.claim_id !== records.claim?.claim_id)
            errors.push(error("execution-claim-mismatch", `history.executions[${index}].claim_id`, "execution is bound to another claim"));
        if (executionIds.has(execution.execution_id))
            errors.push(error("execution-duplicate", `history.executions[${index}].execution_id`, "execution id is duplicated"));
        executionIds.add(execution.execution_id);
    }
    for (const [index, evidence] of records.evidence.entries()) {
        if (!executionIds.has(evidence.execution_id))
            errors.push(error("evidence-execution-mismatch", `history.evidence[${index}].execution_id`, "evidence references an unknown execution"));
        if (evidence.handoff_id !== records.handoff?.handoff_id || evidence.claim_id !== records.claim?.claim_id)
            errors.push(error("evidence-binding-mismatch", `history.evidence[${index}]`, "evidence is not claim-bound to this handoff"));
        if (evidence.sha256 !== null && !/^[0-9a-f]{64}$/i.test(evidence.sha256))
            errors.push(error("evidence-sha256-invalid", `history.evidence[${index}].sha256`, "evidence sha256 must be 64 hexadecimal characters"));
    }
    if (records.executions.length > 0) {
        const latestTime = Math.max(...records.executions.map((item) => Date.parse(item.recorded_at)));
        if (records.executions.filter((item) => Date.parse(item.recorded_at) === latestTime).length > 1)
            errors.push(error("current-execution-ambiguous", "history.executions", "multiple executions share the latest recorded_at"));
    }
    return errors;
}
function compareRecordedAt(a, b) {
