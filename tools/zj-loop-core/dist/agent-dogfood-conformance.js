...
export function evaluateAgentDogfoodConformance(input) {
    ...
    if (input.environment.network_denied.status !== 'proved' || !digest(input.environment.network_denied.evidence_digest))
    ...
    if (!text(trusted.runner_id) || !text(trusted.runner_version) || !Number.isInteger(trusted.attempt) || trusted.attempt < 1 || !digest(trusted.preflight_digest) || !digest(trusted.worktree_digest) || !digest(trusted.network_evidence_digest) || !digest(trusted.credential_evidence_digest) || !digest(trusted.proof_digest))
        reasons.push('trusted-environment-proof-invalid');
    if (trusted.execution_id !== input.execution.execution_id || trusted.attempt !== input.execution.attempt || trusted.preflight_digest !== input.execution.preflight_digest || trusted.registry_snapshot_digest !== input.registry_snapshot.digest || trusted.network_evidence_digest !== input.environment.network_denied.evidence_digest || trusted.credential_evidence_digest !== input.environment.credentials.evidence_digest)
    ...
    if (!observation.after_worktree_clean || !observation.after_network_denied || !observation.after_credentials_clean || observation.side_effects_detected)
...
export function createAgentDogfoodFixture() {
