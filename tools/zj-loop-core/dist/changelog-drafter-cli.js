...
if (argv[0] === 'gitlab-draft-evidence') {
    process.exitCode = await runCli({
        ...
        async handler({ io, options }) {
            if (typeof options.request !== 'string' || typeof options.project !== 'string')
                throw new Error('--request and --project are required');
    ...
    process.exitCode = await runCli({
        ...
        async handler({ io, options }) {
            if (typeof options.request !== 'string' || typeof options.project !== 'string' || typeof options.confirm !== 'string')
                throw new Error('--request, --project and --confirm are required');
    ...
    process.exitCode = await runCli({
        ...
        async handler({ io, options }) {
            for (const name of ['project', 'issue-iid', 'request-id', 'claim-id'])
    ...
    process.exitCode = await runCli({
        ...
        async handler({ io, options }) {
            for (const name of ['request', 'project', 'issue-iid', 'claim-id', 'draft-file', 'branch', 'confirm'])
                if (typeof options[name] !== 'string')
                    throw new Error(`--${name} is required`);
            const request = await readChangelogDraftRequest(String(options.request));
...
function buildArtifactDraft(request) {
    const window = request?.release_window ?? {};
