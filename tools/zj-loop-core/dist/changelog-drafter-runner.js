...
export async function defaultChangelogDrafterRunner(command, args = []) {
    try {
...
export async function readChangelogDraftRequest(path) {
...
export async function executeChangelogDrafterLiveRunner(plan, { runner = defaultChangelogDrafterRunner } = {}) {
...
function normalizeDraftFile(file) {
    ...
    if (!value || value.startsWith('/') || value.includes('..') || !value.endsWith('.md'))
        return '';
    if (value.startsWith('zj-loop/'))
        return '';
    return value;
}
function slug(value) {
