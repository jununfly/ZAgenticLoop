...
export function validateCompletionEvidence(value, options = {}) {
    const errors = [];
    ...
    if (!isCarrier(candidate.carrier))
        errors.push('carrier is required');
    if (!COMPLETION_STATUSES.includes(candidate.status)) {
    ...
    if (!isProviderProvenance(candidate.provenance))
    ...
    for (const [field, expected] of Object.entries(options.expected ?? {})) {
        ...
        if (observed !== expected)
            errors.push(`${field} mismatch: expected ${expected}, observed ${String(observed)}`);
    }
    ...
    if (errors.length > 0)
        return invalid(errors, candidate.side_effects_executed === true);
    return {
        ok: true,
        status: 'valid',
        errors: [],
        side_effects_executed: candidate.side_effects_executed,
        evidence: candidate,
    };
}
function invalid(errors, sideEffects = false) {
    return {
        ok: false,
        status: 'hard_stop',
        errors,
...
function isReviewArtifact(value) {
    return isRecord(value) && isNonEmptyString(value.kind)
...
function isProviderProvenance(value) {
    if (!isRecord(value) || !isNonEmptyString(value.provider))
        return false;
    if (value.provider === 'none')
        return isNonEmptyString(value.orchestration_id)
