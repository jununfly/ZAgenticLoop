...
export function validateDependencySweeperFixtureContract(input) {
    ...
    const errors = [];
    if (subject.package_name !== contract.package_name)
        errors.push('package-name-mismatch');
    if (subject.current_version !== contract.current_version)
        errors.push('current-version-mismatch');
    if (subject.target_version !== contract.target_version)
        errors.push('target-version-mismatch');
    if (subject.update_type !== contract.update_type)
        errors.push('update-type-mismatch');
    if (subject.dependency_section !== contract.dependency_section)
        errors.push('dependency-section-mismatch');
    if (JSON.stringify(subject.manifest_files ?? []) !== JSON.stringify(contract.manifest_files))
        errors.push('manifest-files-mismatch');
    const changedFiles = [...new Set((input.changedFiles ?? []).map((file) => String(file).replaceAll('\\', '/')))].sort();
    if (JSON.stringify(changedFiles) !== JSON.stringify([...contract.manifest_files].sort()))
        errors.push('changed-file-scope-mismatch');
...
function dependencyProviderMetadata(input) {
