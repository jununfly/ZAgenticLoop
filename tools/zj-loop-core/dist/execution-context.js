...
export async function buildAgentExecutionContext(input) {
    ...
    if (!isWithin(repoRoot, contractAbsolute))
        return base("blocked-incomplete-contract", { reason: "activation-contract-path-escapes-repo", next_steps: ["Use a repository-relative activation contract path."] });
    try {
        contractText = await readFile(contractAbsolute, "utf8");
        contract = JSON.parse(contractText);
    }
    catch {
        return base("blocked-incomplete-contract", { reason: "activation-contract-required", next_steps: [`Create or restore ${contractPath} before execution.`] });
    }
    ...
    if (!isWithin(repoRoot, roadmapAbsolute))
        return base("blocked-incomplete-contract", { reason: "roadmap-path-escapes-repo", next_steps: ["Use a repository-relative roadmap path."] });
    let roadmapExists = false;
    try {
        await readFile(roadmapAbsolute, "utf8");
        roadmapExists = true;
    }
    ...
    if (!roadmapExists)
        return { ...context, status: "blocked-missing-roadmap", reason: "roadmap-file-required", next_steps: [`Create or restore the roadmap at ${selectedRoadmapPath}.`, "Do not infer roadmap intent from chat history."] };
    const missing = [handoff.executor.kind === "agent-local" ? null : "agent-local executor", handoff.executor.profile ? null : "executor profile", handoff.executor.capabilities.length > 0 ? null : "allowed executor capabilities", handoff.workspace.base_commit ? null : "workspace base commit", branch ? null : "activation branch", targetBranch ? null : "merge-request target branch"].filter(Boolean);
    if (missing.length > 0)
        return { ...context, reason: `missing-context-fields:${missing.join(",")}`, next_steps: ["Complete the activation execution contract before editing."] };
...
function safeActivationId(value) { if (!/^[a-zA-Z0-9_-]+$/.test(value))
    throw new Error("activation-id-invalid"); return value; }
