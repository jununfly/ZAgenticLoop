...
import { buildFirstRunPlan } from './first-run-runner.js';
const GOALS = ['auto', 'smoke', 'roadmap', 'issue-backlog', 'ci', 'closeout'];
const exitCode = await runCli({
    ...
    async handler({ io, options }) {
        ...
        if (options.json === true) {
            ...
            for (const step of plan.automatic_next_steps)
            ...
            for (const signal of plan.stop_signals) {
                ...
                for (const step of signal.next_steps)
                    io.stdout(`next step: ${step}`);
            }
        }
