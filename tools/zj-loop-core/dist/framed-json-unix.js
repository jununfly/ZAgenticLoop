...
import { createFramedJsonCodec, FramedJsonDecoder } from './framed-json-transport.js';
...
function connectionFor(socket, codec) {
    return {
        async send(frame) { if (socket.destroyed)
            throw new Error('framed-json-socket-closed'); await new Promise((resolve, reject) => { socket.write(codec.encode(frame), (error) => error ? reject(error) : resolve()); }); },
        close() { socket.destroy(); },
    };
}
export function createUnixFramedJsonServer(input) {
    let server;
    ...
    return {
        async start() {
            if (server)
                throw new Error('framed-json-server-already-started');
...
export async function connectUnixFramedJson(input) {
    ...
    if (!Number.isInteger(timeout) || timeout < 1 || timeout > 60_000) {
        socket.destroy();
        throw new Error('framed-json-connect-timeout-invalid');
    }
    socket.on('data', async (chunk) => {
        const result = decoder.push(new Uint8Array(chunk));
        if (result.status === 'blocked')
