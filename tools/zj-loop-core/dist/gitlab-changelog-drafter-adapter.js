...
export function validateGitLabChangelogDraftActions(actions, expectedFile) {
    const errors = [];
    if (!isSafeRepoPath(expectedFile))
        errors.push('draft-file-invalid');
    ...
    if (typeof action?.content !== 'string')
        errors.push('draft-commit-content-required');
    if (action?.encoding !== undefined && action.encoding !== 'text')
        errors.push('draft-commit-encoding-invalid');
    return { ok: errors.length === 0, errors };
}
export async function createGitLabChangelogDraftCarrier(input) {
    ...
    const blocked = (reason, extra = {}) => ({ schema: 'zj-loop.gitlab_changelog_draft_carrier.v1', status: 'blocked', reason, audit, issue: null, side_effects_executed: false, ...extra });
    if (input.confirmationPhrase !== GITLAB_CHANGELOG_CARRIER_CONFIRMATION)
        return blocked('confirmation-required', { required_phrase: GITLAB_CHANGELOG_CARRIER_CONFIRMATION });
    if (!input.token)
        return blocked('gitlab-token-required');
    const validation = validateDraftRequest(input.request, input.projectPath);
    if (!validation.ok)
        return blocked('request-source-mismatch', { errors: validation.errors });
    ...
    if (!search.ok)
        return blocked('carrier-dedupe-read-failed', { http_status: search.status });
    const existing = (await search.json()).find((issue) => parseGitLabLifecycleMarker({ body: issue.description }, 'changelog-draft-request')?.request_id === input.request.request_id);
    if (existing)
        return { schema: 'zj-loop.gitlab_changelog_draft_carrier.v1', status: 'completed', outcome: 'duplicate', audit, issue: { iid: Number(existing.iid), url: String(existing.web_url ?? '') }, side_effects_executed: false };
    ...
    if (!response.ok)
        return blocked('carrier-create-failed', { http_status: response.status, side_effects_executed: true });
...
export async function claimGitLabChangelogDraftCarrier(input) {
    ...
    const blocked = (reason, extra = {}) => ({ schema: 'zj-loop.gitlab_changelog_draft_claim.v1', status: 'blocked', reason, audit, claim: null, side_effects_executed: false, ...extra });
    if (!input.token)
        return blocked('gitlab-token-required');
    const fetchImpl = input.fetchImpl ?? globalThis.fetch;
    if (!fetchImpl)
    ...
    if (!request || request.schema !== CHANGELOG_CARRIER_SCHEMA || request.request_id !== input.requestId || request.release_window?.repo !== input.projectPath)
    ...
    if (!before.response.ok)
        return blocked('claim-read-failed', { http_status: before.response.status });
    if (before.claims.length > 0)
        return { schema: 'zj-loop.gitlab_changelog_draft_claim.v1', status: 'completed', outcome: 'duplicate', audit, claim: before.claims[0], side_effects_executed: false };
    ...
    if (!write.ok)
        return blocked('claim-write-failed', { http_status: write.status, side_effects_executed: true });
    const after = await readClaims();
    if (!after.response.ok)
        return blocked('claim-reread-failed', { http_status: after.response.status, side_effects_executed: true });
    if (after.claims.length !== 1 || after.claims[0].claim_id !== input.claimId)
        return { schema: 'zj-loop.gitlab_changelog_draft_claim.v1', status: 'completed', outcome: 'duplicate', audit, claim: after.claims[0] ?? null, side_effects_executed: true };
...
export async function createGitLabChangelogDraftMr(input) {
    ...
    const blocked = (reason, extra = {}) => ({ schema: 'zj-loop.gitlab_changelog_draft_mr.v1', status: 'blocked', reason, audit, merge_request: null, side_effects_executed: false, ...extra });
    if (!input.token)
        return blocked('gitlab-token-required');
    if (!validateDraftRequest(input.request, input.projectPath).ok)
        return blocked('request-source-mismatch');
    if (!CHANGELOG_DRAFT_BRANCH_PATTERN.test(input.branch) || input.targetBranch !== input.request.release_window.base_branch)
        return blocked('draft-branch-or-target-invalid');
    const actionValidation = validateGitLabChangelogDraftActions(input.actions, input.draftFile);
    if (!actionValidation.ok)
        return blocked('draft-commit-actions-invalid', { action_validation: actionValidation });
    const fetchImpl = input.fetchImpl ?? globalThis.fetch;
    if (!fetchImpl)
        return blocked('gitlab-fetch-unavailable');
    const headers = { ...buildGitLabAuthHeaders({ token: input.token }), 'User-Agent': 'zj-loop-changelog-drafter' };
    ...
    if (!issue.ok)
        return blocked('issue-read-failed', { http_status: issue.status });
    const notes = await fetchImpl(`${issueUrl}/notes?per_page=100`, { headers });
    const claims = notes.ok ? (await notes.json()).map((note) => parseGitLabLifecycleMarker(note, 'changelog-draft-claim')).filter((claim) => claim?.claim_id === input.claimId && claim?.request_id === input.request.request_id) : [];
    if (!notes.ok || claims.length !== 1)
    ...
    if (!existing.ok)
        return blocked('draft-mr-dedupe-read-failed', { http_status: existing.status });
    const existingMr = (await existing.json())[0];
    if (existingMr)
        return completedMr(audit, 'duplicate', existingMr);
    ...
    if (!branchCreate.ok && branchCreate.status !== 400)
        return blocked('draft-branch-create-failed', { http_status: branchCreate.status });
    const branchRead = await fetchImpl(branchUrl, { headers });
    if (!branchRead.ok)
        return blocked('draft-branch-not-ready', { http_status: branchRead.status });
    const fileUrl = buildGitLabApiUrl({ apiBaseUrl: input.apiBaseUrl, projectPath: input.projectPath, path: ['repository', 'files', input.draftFile] });
    const fileRead = await fetchImpl(`${fileUrl}?ref=${encodeURIComponent(input.branch)}`, { headers });
    if (![200, 404].includes(fileRead.status))
        return blocked('draft-file-read-failed', { http_status: fileRead.status });
    const commitActions = input.actions.map((action) => ({ ...action, action: fileRead.status === 200 ? 'update' : 'create' }));
    const commitsUrl = buildGitLabApiUrl({ apiBaseUrl: input.apiBaseUrl, projectPath: input.projectPath, path: ['repository', 'commits'] });
    const commit = await postJson(fetchImpl, commitsUrl, headers, { branch: input.branch, commit_message: input.commitMessage, actions: commitActions });
    if (!commit.ok)
        return blocked('draft-commit-create-failed', { http_status: commit.status });
    ...
    if (!mr.ok)
        return blocked('draft-mr-create-failed', { http_status: mr.status });
    return completedMr(audit, 'created', await mr.json());
}
function validateDraftRequest(request, projectPath) {
    const errors = [];
    if (!request || request.schema !== CHANGELOG_CARRIER_SCHEMA)
        errors.push('schema-invalid');
    if (!request?.request_id || request.status !== 'draft-request-candidate')
        errors.push('request-invalid');
    if (request?.release_window?.repo !== projectPath)
        errors.push('repo-mismatch');
    if (!request?.release_window?.base_branch)
        errors.push('base-branch-required');
    if (!request?.release_window?.since_ref || !request?.release_window?.until_ref)
        errors.push('release-window-required');
    return { ok: errors.length === 0, errors };
}
function isSafeRepoPath(value) { return Boolean(value) && !value.startsWith('/') && !value.includes('\\') && !value.split('/').includes('..') && !value.split('/').includes('') && !value.endsWith('/'); }
async function postJson(fetchImpl, url, headers, payload) { try {
    return await fetchImpl(url, { method: 'POST', headers: { ...headers, 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
}
catch {
    return new Response('', { status: 0 });
} }
function completedMr(audit, outcome, mr) { return { schema: 'zj-loop.gitlab_changelog_draft_mr.v1', status: 'completed', outcome, audit, merge_request: { iid: Number(mr.iid), url: String(mr.web_url ?? ''), source_branch: String(mr.source_branch ?? audit.branch), target_branch: String(mr.target_branch ?? audit.target_branch), draft: true }, side_effects_executed: outcome === 'created' }; }
