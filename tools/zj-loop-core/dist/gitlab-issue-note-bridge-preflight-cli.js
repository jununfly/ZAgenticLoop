...
import { GITLAB_ISSUE_NOTE_BRIDGE_PREFLIGHT_EXIT_CODES, GITLAB_ISSUE_NOTE_BRIDGE_PREFLIGHT_SCHEMA, runGitLabIssueNoteBridgePreflight, validateGitLabIssueNoteBridgePreflightManifest, } from './gitlab-issue-note-bridge-preflight.js';
...
if (!configPath) {
    ...
    run(configPath).catch((error) => print({ schema: GITLAB_ISSUE_NOTE_BRIDGE_PREFLIGHT_SCHEMA, status: 'blocked', reason: error instanceof Error ? error.message : String(error), side_effects_executed: false }));
}
