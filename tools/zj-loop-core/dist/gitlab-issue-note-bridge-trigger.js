...
export async function triggerGitLabIssueNoteBridgePipeline(input) {
    ...
    if (!input.token)
        return blocked('trigger-token-required');
    if (!input.config.projectPath.trim() || input.config.projectPath !== input.envelope.project_path)
        return blocked('route-mismatch');
    if (!input.config.routeId.trim() || input.config.targetRoute !== input.envelope.target_route)
        return blocked('route-mismatch');
    if (input.config.allowedEventType !== input.envelope.event_type)
        return blocked('event-type-mismatch');
    if (!input.config.pipelineRef.trim() || !input.config.targetRoute.trim() || input.config.pipelineRef !== input.envelope.target_ref)
        return blocked('route-mismatch');
    if (!input.config.enabled || !['install-ready', 'execution-ready', 'dogfood-verified', 'replayed'].includes(input.config.maturity))
        return blocked('route-not-triggerable');
    if (!input.envelopeRef.trim())
        return blocked('envelope-ref-required');
    const variables = buildVariables(input.envelope, input.config.targetRoute, input.envelopeRef);
    if (!variables)
        return blocked('variable-contract-mismatch');
...
function buildVariables(envelope, targetRoute, envelopeRef) {
    const values = [
        envelope.event_id,
        envelope.dedupe_key,
    ...
    if (values.some((value) => !value || value.length > 512) || !/^\d+$/.test(values[3]) || !/^\d+$/.test(values[4]) || Number(values[3]) <= 0 || Number(values[4]) <= 0)
        return null;
    return VARIABLE_KEYS.map((key, index) => ({ key, value: values[index] }));
}
function pipelineUrlBelongsToProject(url, projectPath) {
    try {
