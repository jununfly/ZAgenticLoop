...
import { parseIssueFixRequestComments, resolveIssueFixRequestDedupe, } from './issue-fix-request-contract.js';
import { buildIssueTriageTransitionIssueFixRequestBody, runIssueTriageTransitionRunner, } from './issue-triage-transition-runner.js';
import { validateTriageRoleMapping, resolveTriageRoles } from './triage-role-mapping.js';
...
export function buildGitLabTrustedTransitionRequest(input) {
    ...
    if (!mappingValidation.ok)
        return { status: 'blocked', reason: 'triage-role-mapping-invalid', errors: mappingValidation.errors, request: null, skipped_count: 0 };
    ...
    if (!selected)
        return { status: 'report-only', reason: 'no-eligible-ready-for-agent-candidate', request: null, skipped_count: 0 };
...
export async function executeGitLabIssueTriageTransition(input) {
    ...
    if (!input.token)
        return hardStop('gitlab-token-required');
    if (!Number.isInteger(Number(input.issueIid)) || Number(input.issueIid) <= 0)
        return hardStop('request-source-mismatch');
    const binding = validateSourceBinding(input);
    if (!binding.ok)
        return hardStop('request-source-mismatch', { binding_errors: binding.errors });
    if (input.confirmationMode === 'trusted-automation') {
        ...
        if (input.roleMapping) {
            ...
            if (!mappingValidation.ok || roles.errors.length > 0)
                return hardStop('triage-role-mapping-invalid', { mapping_errors: [...mappingValidation.errors, ...roles.errors] });
        }
    }
    if (!fetchImpl)
        return hardStop('gitlab-fetch-unavailable');
    ...
    if (input.confirmationMode !== 'trusted-automation' && !actorPermission) {
        ...
        if (!actorPermission)
            return hardStop('actor-permission-unavailable');
    }
    ...
    if (!notesResponse.ok)
        return hardStop('source-issue-notes-read-failed', { http_status: notesResponse.status });
...
async function resolveGitLabActorPermission(input) {
    ...
    if (Number(member.access_level) >= 40)
        return 'maintainer';
    if (Number(member.access_level) >= 30)
        return 'collaborator';
    return null;
}
function validateSourceBinding(input) {
    ...
    const errors = [];
    if (source?.tracker !== 'gitlab' && source?.provider !== 'gitlab')
        errors.push('tracker must be gitlab');
    if (String(source?.repo ?? source?.project_path ?? '') !== String(input.projectPath))
        errors.push('source project does not match CI project');
    if (Number(source?.issue ?? source?.issue_iid) !== Number(input.issueIid))
        errors.push('source issue does not match CI issue');
    if (source?.issue_url && !sourceIssueUrlMatches(source.issue_url, input))
        errors.push('source issue URL does not match CI issue');
    return { ok: errors.length === 0, errors };
}
function sourceIssueUrl(input) {
    const source = input.request?.source;
    if (source?.issue_url)
        return String(source.issue_url);
...
function sourceIssueUrlMatches(value, input) {
    try {
...
async function recoverUncertainWrite(input) {
    const response = await input.fetchImpl(`${input.url}?per_page=100`, { headers: input.headers });
    if (response.ok) {
        const notes = await response.json();
        const parsed = parseIssueFixRequestComments(notes.map((note) => ({ id: note.id, body: note.body, author: note.author?.username, createdAt: note.created_at })));
        const existing = parsed.find((item) => item.request?.request_id === input.request.request_id);
        if (existing)
            return completedResult({ audit: input.audit, plan: input.plan, outcome: 'recovered-duplicate', note: noteAudit(existing.commentId, input.input), handoff: handoff(input.request) });
    }
...
function noteAudit(id, input, url) {
...
function buildGitLabIssueUrl(input, issue) {
    const host = String(input.apiBaseUrl ?? 'https://gitlab.com/api/v4').replace(/^https?:\/\//, '').split('/')[0];
    return `https://${host}/${input.projectPath}/-/issues/${issue}`;
}
function buildGitLabNoteUrl(input, noteId) {
    if (!noteId)
        return null;
