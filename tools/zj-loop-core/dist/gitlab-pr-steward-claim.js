...
export async function claimGitLabPrStewardIssueFixRequest(input) {
    const audit = { project_path: input.projectPath, issue_iid: Number(input.issueIid), merge_request_iid: Number(input.mergeRequestIid), request_id: input.requestId, claim_id: input.claimId, auth_source: input.token ? 'GITLAB_TOKEN' : null };
    const blocked = (reason, extra = {}) => ({ schema: 'zj-loop.gitlab_pr_steward_claim.v1', status: 'blocked', reason, audit, claim: null, side_effects_executed: false, ...extra });
    if (!input.token)
    ...
    if (!request || request.status !== 'requested' || request.route_decision?.target_consumer !== 'pr-steward' || request.requested_consumer?.capability !== 'pr-review-and-readiness-fix')
    ...
    if (before.claims.length > 0) {
        if (before.claims[0].claim_id !== input.claimId)
            return blocked('claim-mismatch', { existing_claim_id: before.claims[0].claim_id });
    ...
    if (!response.ok)
        return blocked('claim-write-failed', { http_status: response.status, side_effects_executed: true });
    ...
    if (after.claims.length !== 1 || after.claims[0].claim_id !== input.claimId)
        return { schema: 'zj-loop.gitlab_pr_steward_claim.v1', status: 'completed', outcome: 'duplicate', audit, claim: after.claims[0] ?? null, side_effects_executed: true };
