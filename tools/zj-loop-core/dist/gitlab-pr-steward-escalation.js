...
export async function appendGitLabPrStewardEscalation(input) {
    ...
    if (!before.response.ok)
        return blocked('notes-read-failed', { http_status: before.response.status });
    ...
    if (existing)
        return { schema: 'zj-loop.gitlab_pr_steward_escalation.v1', status: 'completed', outcome: 'duplicate', audit, escalation: existing, side_effects_executed: false };
    const escalation = {
    ...
    if (!noteResponse.ok)
        return blocked('escalation-write-failed', { http_status: noteResponse.status, side_effects_executed: true });
    const after = await readNotes();
    if (!after.response.ok)
        return blocked('escalation-reread-failed', { http_status: after.response.status, side_effects_executed: true });
    ...
    if (matches.length !== 1)
        return blocked('escalation-reread-ambiguous', { side_effects_executed: true, escalation: matches[0] ?? null });
