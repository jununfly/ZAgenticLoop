...
export async function fetchGitLabPrStewardReport(input) {
    ...
    let response;
    try {
        response = await fetchImpl(url, { headers: { ...buildGitLabAuthHeaders({ token: input.token }), 'User-Agent': 'zj-loop-pr-steward' } });
    }
    catch {
        return blocked('merge-request-read-failed');
    }
    if (!response.ok)
        return blocked('merge-request-read-failed', { http_status: response.status });
    const mr = await response.json();
    if (Number(mr.iid) !== Number(input.mergeRequestIid))
        return blocked('merge-request-source-mismatch');
    ...
    if (pipeline.id !== undefined && pipeline.id !== null) {
        ...
        let jobsResponse;
        try {
            jobsResponse = await fetchImpl(jobsUrl, { headers: { ...buildGitLabAuthHeaders({ token: input.token }), 'User-Agent': 'zj-loop-pr-steward' } });
        }
        catch {
            return blocked('pipeline-jobs-read-failed');
        }
        if (!jobsResponse.ok)
            return blocked('pipeline-jobs-read-failed', { http_status: jobsResponse.status });
