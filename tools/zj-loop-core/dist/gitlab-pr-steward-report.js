...
export async function fetchGitLabPrStewardReport(input) {
    ...
    const checks = FAILURE_STATUSES.has(pipelineStatus) ? 'failure' : pipelineStatus;
    return {
        ...
        report: {
            ...
            observations: { checks, pipeline_status: pipelineStatus, pipeline_id: pipeline.id ?? null, pipeline_url: String(pipeline.web_url ?? '') },
