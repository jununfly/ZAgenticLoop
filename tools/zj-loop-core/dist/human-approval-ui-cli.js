...
process.exitCode = await runCli({
    ...
    async handler({ io, options }) {
        ...
        if (!networkId || !pairingEndpoint || !humanId || !deviceKeyId || !keyTag || !helperPath)
            throw new Error('network-id-pairing-endpoint-human-id-device-key-id-key-tag-helper-path-required');
        if (typeof options['client-cert'] !== 'string' || typeof options['client-key'] !== 'string')
