...
import { loadOpnWebUiConfig, opnWebUiIdentityDir } from './opn-web-ui-config.js';
...
process.exitCode = await runCli({
    ...
    async handler({ io, options }) {
        ...
        const configured = identityDir ? await loadOpnWebUiConfig(identityDir) : undefined;
        ...
        if (command === 'install-service' || command === 'uninstall-service') {
            if (!identityDir)
                throw new Error('opn-web-ui-identity-dir-required-for-service');
            if (!configuredNetworkId)
                throw new Error('opn-web-ui-network-id-required-for-service');
        ...
        if (!networkId || !pairingEndpoint || !humanId || !deviceKeyId || (!signerKey && (!keyTag || !helperPath)))
            throw new Error('network-id-pairing-endpoint-human-id-device-key-id-signer-required');
        ...
        if (!clientCertPath || !clientKeyPath)
            throw new Error('human-device-client-cert-and-key-required');
        ...
        try {
            deviceFingerprint = createHash('sha256').update(new X509Certificate(clientCert).raw).digest('hex');
        }
        catch {
        ...
        let graphFeatureStatus = { status: 'not-configured' };
        ...
        if (stateStore && graphPlanPath) {
            if (!graphEvidenceRoot) {
                ...
                try {
                    ...
                    if (configured?.graph_plan_digest && plan.plan_digest !== configured.graph_plan_digest)
                        throw new Error('graph-plan-digest-mismatch');
