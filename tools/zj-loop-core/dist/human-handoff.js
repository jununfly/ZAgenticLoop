...
function whyRequiredForStop(stopCode, reason) {
...
function retryPolicyForStop(stopCode) {
    if (stopCode === 'duplicate-completed-loop')
        return 'do-not-retry';
    if (stopCode === 'previous-loop-failed')
        return 'new-request-or-bounded-resume';
