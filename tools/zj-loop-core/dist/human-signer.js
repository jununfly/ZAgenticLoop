...
export function createInMemoryHumanSigner(input) {
    ...
    const identity = { schema: HUMAN_SIGNER_SCHEMA, human_id: humanId, algorithm: 'Ed25519', public_key_pem: publicKeyPem, public_key_fingerprint: publicKeyFingerprint };
    return {
...
export function verifyHumanSignature(input) {
    if (input.identity.schema !== HUMAN_SIGNER_SCHEMA || input.identity.algorithm !== 'Ed25519' || input.signature.schema !== HUMAN_SIGNATURE_SCHEMA || input.signature.algorithm !== 'Ed25519')
        return false;
    ...
    try {
        ...
        if (publicKey.asymmetricKeyType !== 'ed25519' || fingerprint(publicKey) !== input.identity.public_key_fingerprint)
            return false;
