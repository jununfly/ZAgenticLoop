...
const ISSUE_FIX_REQUEST_COMMENT_PATTERN = /<!--\s*zj-loop:issue-fix-request\s+(?<json>[\s\S]*?)-->/g;
...
export function resolveIssueFixRequestDedupe(input) {
...
export function applyFixConsumerTransition(input) {
    ...
    if (input.transition === 'open_pr') {
        if (!input.linkedPr)
            throw new Error('linkedPr is required for open_pr');
...
function statusForConsumerTransition(transition) {
    ...
    if (!status)
        throw new Error(`unsupported consumer transition ${transition}`);
    return status;
}
