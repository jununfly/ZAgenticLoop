...
export async function readIssueTriageActionRequest(path) {
...
function decideIssueTriageAction(input) {
    ...
    if (!input.route)
        return decision('rejected', 'route-not-found');
    if (!routeValid)
        return decision('rejected', 'route-contract-invalid');
    if (!requestValid)
        return decision('rejected', 'request-schema-invalid');
    if (input.live)
        return decision('rejected', 'live-side-effects-not-enabled');
    if (humanGuard)
        return decision('escalated', 'human-guard-matched');
    if (verifierMissing)
        return decision('rejected', `missing-verifier:${verifierMissing}`);
    if (!actionAllowed.allowed)
        return decision('rejected', actionAllowed.reason);
...
function completionFor(input) {
    if (input.decision.status === 'escalated')
        return { form: 'escalation-issue', status: 'escalated' };
    if (input.decision.status === 'rejected')
        return { form: 'triage-action-skipped', status: 'skipped' };
...
function actionsFor(request, decisionResult, live) {
    if (decisionResult.status !== 'dry-run-completed')
        return [];
...
function requiresHumanReview(request) {
    if (request?.risk === 'high' || request?.risk === 'unknown')
        return true;
