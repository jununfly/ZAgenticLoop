...
import { buildIssueTriageTransitionIssueFixRequestBody, buildIssueTriageTransitionIssueFixRequestTitle, buildRecommendedTriageTransitionFixture, readRecommendedTriageTransition, runIssueTriageTransitionRunner, } from './issue-triage-transition-runner.js';
...
if (argv[0] === 'confirm-plan') {
    ...
    process.exitCode = await runCli({
        ...
        async handler({ io, options }) {
            if (typeof options['transition-plan'] !== 'string')
                throw new Error('--transition-plan is required');
            ...
            if (typeof options['title-out'] === 'string')
                await writeFile(options['title-out'], `${title}\n`);
    ...
    process.exitCode = await runCli({
        ...
        async handler({ io, options }) {
            ...
            const route = findRoute(table, 'issue-triage-transition');
            const roleMapping = getTriageRoleMapping(table, 'gitlab');
            const mode = String(options.mode ?? 'human-fixed-phrase');
            let request;
            let labels = [];
