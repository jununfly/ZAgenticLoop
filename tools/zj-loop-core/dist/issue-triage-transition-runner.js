...
export const ISSUE_RECOMMENDATIONS_ARTIFACT_SCHEMA = 'zj-loop.issue_recommendations.v1';
export const TRANSITION_REQUESTS_ARTIFACT_SCHEMA = 'zj-loop.transition_requests.v1';
...
export async function readRecommendedTriageTransition(path) {
    return JSON.parse(await readFile(path, 'utf8'));
}
export function buildRecommendedTriageTransitionFixture(overrides = {}) {
    const request = {
...
export function buildIssueTriageTransitionIssueFixRequestTitle(issueFixRequest) {
...
export function buildIssueRecommendationsArtifact(input) {
    const provider = String(input.provider ?? 'github');
    const project = String(input.projectPath ?? input.repo ?? '');
    const recommendations = (Array.isArray(input.recommendations) ? input.recommendations : []).map((item) => ({
        provider: String(item.provider ?? provider),
        repo: String(item.repo ?? project),
        project_path: String(item.project_path ?? item.projectPath ?? project),
        issue: item.issue ?? item.issue_iid ?? item.iid ?? null,
        issue_iid: item.issue_iid ?? item.iid ?? item.issue ?? null,
        issue_url: String(item.issue_url ?? item.url ?? ''),
        labels: Array.isArray(item.labels) ? item.labels.map(String) : [],
        assignees: Array.isArray(item.assignees) ? item.assignees.map(String) : [],
        recommendation: String(item.recommendation ?? item.recommended_state ?? ''),
        reason: String(item.reason ?? ''),
        request: item.request ?? null,
    }));
    return {
        schema: ISSUE_RECOMMENDATIONS_ARTIFACT_SCHEMA,
        provider,
        project_path: project,
        pipeline_url: String(input.pipelineUrl ?? ''),
        route: String(input.route ?? 'issue-backlog-triage'),
        source: String(input.source ?? (provider === 'gitlab' ? 'gitlab-issues-api' : 'issues-api')),
        issue_count: recommendations.length,
        recommendations,
    };
}
export function buildTransitionRequestsArtifact(input) {
    const provider = String(input.provider ?? 'github');
    const transitions = (Array.isArray(input.transitions) ? input.transitions : []).map((item) => {
        const plan = item.plan ?? item.transition_plan ?? item;
        const confirmed = plan.confirmed_transition ?? item.confirmed_transition ?? {};
        const issueFixRequest = confirmed.issue_fix_request ?? item.issue_fix_request ?? null;
        const source = confirmed.source ?? plan.request?.source ?? item.source ?? {};
        const routeDecision = plan.decision ?? item.route_decision ?? {};
        const consumerPlan = item.consumer_plan ?? item.consumerPlan ?? null;
        return {
            provider: String(item.provider ?? source.tracker ?? source.provider ?? provider),
            issue: source.issue ?? item.issue ?? item.issue_iid ?? null,
            issue_iid: item.issue_iid ?? source.issue ?? item.issue ?? null,
            issue_url: String(source.issue_url ?? item.issue_url ?? issueFixRequest?.source_signal?.url ?? ''),
            requested_transition: String(confirmed.confirmed_state ?? item.requested_transition ?? ''),
            side_effect_policy: String(item.side_effect_policy ?? 'request-only'),
            live_issue_mutation: Boolean(item.live_issue_mutation ?? false),
            route_decision: routeDecision,
            consumer_plan: consumerPlan,
            command_exit_codes: item.command_exit_codes ?? {},
            issue_fix_request: issueFixRequest,
        };
    });
    return {
        schema: TRANSITION_REQUESTS_ARTIFACT_SCHEMA,
        provider,
        route: String(input.route ?? 'issue-triage-transition'),
        source: String(input.source ?? (provider === 'gitlab' ? 'gitlab-ci' : 'github-actions')),
        candidate_count: transitions.length,
        transitions,
    };
}
function decideTransition(input) {
    ...
    if (!requestValid.ok)
        return decision('rejected', `request-invalid:${requestValid.errors[0]}`);
    ...
    if (confirmationMode === TRUSTED_AUTOMATION_MODE) {
        ...
        if (!ALLOWED_PERMISSIONS.has(input.actorPermission))
            return decision('rejected', 'actor-not-maintainer-or-collaborator');
        if (input.command !== input.request.confirm_command)
            return decision('rejected', 'confirm-command-mismatch');
        if (input.confirmationPhrase !== 'CONFIRM_TRIAGE_TRANSITION')
            return decision('rejected', 'fixed-confirmation-phrase-required');
    }
    if (input.request.recommended_state === 'wontfix')
        return decision('escalated', 'wontfix-requires-human-review');
...
function validateRecommendedTransition(request) {
    const errors = [];
    if (request?.schema !== RECOMMENDED_TRIAGE_TRANSITION_SCHEMA)
        errors.push('schema');
    if (!request?.request_id)
        errors.push('request_id');
    if (!request?.source?.repo || !request?.source?.issue)
        errors.push('source');
    if (request?.route_decision?.route_id !== 'issue-backlog-triage')
        errors.push('route_decision.route_id');
    if (!ALLOWED_CATEGORIES.has(request?.category_role))
        errors.push('category_role');
    if (!ALLOWED_STATES.has(request?.recommended_state))
        errors.push('recommended_state');
    if (!request?.brief_draft?.kind || !request?.brief_draft?.body)
        errors.push('brief_draft');
    if (!request?.dedupe_key)
        errors.push('dedupe_key');
    if (request?.confirm_command !== `${CONFIRM_TRIAGE_TRANSITION_COMMAND} ${request?.request_id}`) {
        errors.push('confirm_command');
    }
    return { ok: errors.length === 0, errors };
}
function buildConfirmedTransition(input) {
...
function operationsFor(request, decisionResult) {
    if (decisionResult.status !== 'confirmed')
        return [];
    return [
...
function commentFor(request, decisionResult) {
    if (decisionResult.status !== 'confirmed')
        return null;
    return [
...
function confirmationFor(decisionResult) {
...
function confirmationVerifierFor(request, decisionResult) {
...
function sourceIssueUrlFor(request) {
...
function completionFormFor(request, decisionResult) {
    if (decisionResult.status === 'escalated')
        return 'escalation-issue';
    if (decisionResult.status !== 'confirmed')
        return 'triage-action-skipped';
