...
function shapeError(value) {
    ...
    if (!Number.isInteger(item.plan_revision) || item.plan_revision < 1 || !Number.isInteger(item.attempt) || item.attempt < 1)
    ...
    for (const key of ['executable_digest', 'argv_digest', 'cwd_digest', 'env_policy_digest', 'sandbox_policy_digest', 'network_denied_evidence_digest', 'orchestration_preflight_digest', 'preflight_digest'])
