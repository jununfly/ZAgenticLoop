...
import { NATIVE_CHECKPOINT_FIXTURE_SCHEMA, NATIVE_CHECKPOINT_RESUME_ORACLE_SCHEMA, NATIVE_CHECKPOINT_AUTHORITY_INVARIANTS, } from './native-checkpoint-fixture.js';
import { NATIVE_CHECKPOINT_ADAPTER_INPUT_SCHEMA, NATIVE_CHECKPOINT_ADAPTER_OUTPUT_SCHEMA, } from './native-checkpoint-adapter.js';
export const NATIVE_CHECKPOINT_ADAPTER_BOUNDARY_SCHEMA = 'zj-loop.native_checkpoint_adapter_boundary.v1';
export const NATIVE_CHECKPOINT_ADAPTER_CONTRACT_REVISION = 1;
/**
...
function canonical(value) {
    ...
    if (typeof json !== 'string')
        throw new Error('native-checkpoint-adapter-boundary-canonicalization-invalid');
    return json;
}
function calculateDigest(value) {
    return `sha256:${createHash('sha256').update(canonical(value), 'utf8').digest('hex')}`;
}
function withoutContractDigest(value) {
...
function validateNativeContractPin(value) {
    if (!isRecord(value) || !exactKeys(value, NATIVE_CONTRACT_KEYS))
...
function validateProviderDependency(value) {
    if (!isRecord(value) || !exactKeys(value, PROVIDER_DEPENDENCY_KEYS))
...
function validateExitBoundary(value) {
    if (!isRecord(value) || !exactKeys(value, EXIT_BOUNDARY_KEYS))
...
function validateBoundaryShape(value) {
    const errors = [];
    if (!isRecord(value) || !exactKeys(value, CONTRACT_KEYS))
        return ['adapter-boundary-field-invalid'];
    ...
    if (digest(value.contract_digest)) {
        push(errors, value.contract_digest === calculateDigest(withoutContractDigest(value)), 'adapter-boundary-digest-mismatch');
    }
...
export function nativeCheckpointAdapterBoundaryDigest(value) {
    return calculateDigest(withoutContractDigest(value));
}
export function createNativeCheckpointAdapterBoundaryContract(input) {
    const unsigned = { schema: NATIVE_CHECKPOINT_ADAPTER_BOUNDARY_SCHEMA, ...structuredClone(input) };
...
export function validateNativeCheckpointAdapterBoundaryContract(value) {
    const errors = validateBoundaryShape(value);
    return errors.length > 0 ? { status: 'blocked', errors } : { status: 'valid', contract: value };
}
export function validateNativeCheckpointAdapterCompatibility(input) {
    ...
    if (contractValidation.status === 'blocked')
...
function blockedExit(reason, nativeFallbackAvailable) {
    return {
        status: 'blocked',
...
export function evaluateNativeCheckpointAdapterExit(input) {
    ...
    if (!input.dependency_available)
        return blockedExit('dependency-missing', input.native_fallback_available);
    if (input.compatibility === 'skewed')
        return blockedExit('version-skew', input.native_fallback_available);
    if (input.contract_digest === 'drifted')
        return blockedExit('contract-digest-drift', input.native_fallback_available);
    if (input.state_translation === 'lossy')
        return blockedExit('state-translation-loss', input.native_fallback_available);
    if (input.conformance === 'failed') {
        if (!input.native_fallback_available)
            return blockedExit('native-fallback-unavailable', false);
        return {
            status: 'retire',
