...
export const NATIVE_CHECKPOINT_FIXTURE_SCHEMA = 'zj-loop.native_checkpoint_fixture.v1';
export const NATIVE_CHECKPOINT_RESUME_ORACLE_SCHEMA = 'zj-loop.native_checkpoint_resume_oracle.v1';
const DIGEST = /^sha256:[0-9a-f]{64}$/;
const ID = /^[^\s]{1,256}$/;
const NATIVE_CORE = 'native-core';
/**
...
export const NATIVE_CHECKPOINT_EVIDENCE_INVARIANTS = Object.freeze({
    evidence_execution_binding: Object.freeze(['execution_id', 'task_id', 'attempt']),
...
function canonical(value) {
    ...
    if (typeof json !== 'string')
        throw new Error('native-checkpoint-canonicalization-invalid');
    return json;
}
function calculateDigest(value) {
    return `sha256:${createHash('sha256').update(canonical(value), 'utf8').digest('hex')}`;
}
function withoutFixtureDigest(value) {
...
function validateFixtureShape(value) {
    const errors = [];
    if (!isRecord(value) || !exactKeys(value, FIXTURE_KEYS))
        return ['fixture-field-invalid'];
    ...
    if (!isRecord(work) || !exactKeys(work, WORK_ITEM_KEYS))
        errors.push('work-item-field-invalid');
    else {
    ...
    if (!isRecord(execution) || !exactKeys(execution, EXECUTION_KEYS))
        errors.push('execution-field-invalid');
    else {
    ...
    if (!isRecord(checkpoint) || !exactKeys(checkpoint, CHECKPOINT_KEYS))
        errors.push('checkpoint-field-invalid');
    else {
    ...
    if (!isRecord(resume) || !exactKeys(resume, RESUME_KEYS))
        errors.push('resume-field-invalid');
    else {
    ...
    if (!isRecord(evidence) || !exactKeys(evidence, EVIDENCE_KEYS))
        errors.push('evidence-field-invalid');
    else {
    ...
    if (!isRecord(verification) || !exactKeys(verification, VERIFICATION_KEYS))
        errors.push('verification-field-invalid');
    else {
    ...
    if (!isRecord(acceptance) || !exactKeys(acceptance, ACCEPTANCE_KEYS))
        errors.push('human-acceptance-field-invalid');
    else {
    ...
    if (!isRecord(authority) || !exactKeys(authority, AUTHORITY_KEYS))
        errors.push('authority-field-invalid');
    else
        push(errors, AUTHORITY_KEYS.every((key) => authority[key] === NATIVE_CHECKPOINT_AUTHORITY_INVARIANTS[key]), 'authority-invalid');
    const deliveries = value.deliveries;
    if (!Array.isArray(deliveries) || deliveries.length < 2)
        errors.push('delivery-history-incomplete');
    else {
        const ids = new Set();
        let accepted = 0;
        let duplicate = 0;
        for (const delivery of deliveries) {
            if (!isRecord(delivery) || !exactKeys(delivery, DELIVERY_KEYS) || !id(delivery.delivery_id) || !id(delivery.execution_id) || !positiveInteger(delivery.attempt) || !['accepted', 'duplicate'].includes(delivery.disposition)) {
                errors.push('delivery-field-invalid');
                continue;
            }
            if (ids.has(delivery.delivery_id))
                errors.push('delivery-id-duplicate');
            ids.add(delivery.delivery_id);
            if (delivery.disposition === 'accepted')
                accepted += 1;
            else
                duplicate += 1;
        }
    ...
    if (isRecord(value) && typeof value.fixture_digest === 'string') {
        ...
        errors.push('fixture-digest-invalid');
    return [...new Set(errors)];
}
export function createNativeCheckpointFixture(input) {
    const candidate = { schema: NATIVE_CHECKPOINT_FIXTURE_SCHEMA, ...structuredClone(input), fixture_digest: `sha256:${'0'.repeat(64)}` };
    const errors = validateFixtureShape(candidate);
    if (errors.length > 0 && !(errors.length === 1 && errors[0] === 'fixture-digest-invalid'))
...
export function nativeCheckpointFixtureDigest(value) {
    return calculateDigest(withoutFixtureDigest(value));
}
export function validateNativeCheckpointFixture(value) {
    const errors = validateFixtureShape(value);
    return errors.length === 0 ? { status: 'valid' } : { status: 'blocked', errors };
}
function compareIdentity(errors, actual, expected, prefix) {
    if (actual.execution_id !== expected.execution_id)
        errors.push(`${prefix}-execution-identity-mismatch`);
    if (actual.task_id !== expected.task_id)
        errors.push(`${prefix}-task-identity-mismatch`);
    if (actual.attempt !== expected.attempt)
        errors.push(`${prefix}-attempt-mismatch`);
}
function compareObservation(value, observation) {
    const errors = [];
    if (!isRecord(observation) || !isRecord(observation.checkpoint) || !isRecord(observation.resume) || !Array.isArray(observation.deliveries) || !isRecord(observation.evidence) || !isRecord(observation.verification) || !isRecord(observation.human_acceptance) || !isRecord(observation.authority))
        return ['observation-invalid'];
    const expectedExecution = value.execution;
    const expectedCheckpoint = value.checkpoint;
    const actualCheckpoint = observation.checkpoint;
    if (actualCheckpoint.checkpoint_id !== expectedCheckpoint.checkpoint_id)
        errors.push('checkpoint-identity-mismatch');
    if (actualCheckpoint.source_execution_id !== expectedCheckpoint.source_execution_id || actualCheckpoint.source_task_id !== expectedCheckpoint.source_task_id || actualCheckpoint.source_attempt !== expectedCheckpoint.source_attempt)
        errors.push('checkpoint-source-binding-mismatch');
    if (actualCheckpoint.source_revision !== expectedCheckpoint.source_revision)
        errors.push('checkpoint-source-revision-mismatch');
    if (actualCheckpoint.artifact_ref !== expectedCheckpoint.artifact_ref)
        errors.push('checkpoint-artifact-binding-mismatch');
    if (actualCheckpoint.state_digest !== expectedCheckpoint.state_digest)
        errors.push('checkpoint-state-digest-mismatch');
    const actualResume = observation.resume;
    const expectedResume = value.resume;
    if (actualResume.execution_id !== expectedResume.execution_id)
        errors.push('resume-execution-identity-mismatch');
    if (actualResume.task_id !== expectedResume.task_id)
        errors.push('resume-task-identity-mismatch');
    if (actualResume.attempt !== expectedResume.attempt)
        errors.push('resume-attempt-mismatch');
    if (actualResume.agent_id !== expectedResume.agent_id)
        errors.push('resume-agent-identity-mismatch');
    if (actualResume.checkpoint_id !== expectedResume.checkpoint_id)
        errors.push('resume-checkpoint-mismatch');
    if (actualResume.state_digest !== expectedResume.state_digest)
        errors.push('resume-state-digest-mismatch');
    ...
    if (hasOtherAuthority || acceptedExecutionIds.size > 1)
        errors.push('duplicate-created-second-execution-authority');
    else if (accepted.length !== 1 || accepted[0]?.execution_id !== value.execution.execution_id || accepted[0]?.attempt !== value.execution.attempt)
        errors.push('duplicate-delivery-not-idempotent');
    if (!observation.deliveries.some((delivery) => delivery.disposition === 'duplicate' && delivery.execution_id === value.execution.execution_id && delivery.attempt === value.execution.attempt))
        errors.push('duplicate-delivery-not-observed');
    const actualEvidence = observation.evidence;
    const expectedEvidence = value.evidence;
    if (actualEvidence.evidence_digest !== expectedEvidence.evidence_digest)
        errors.push('evidence-digest-mismatch');
    if (actualEvidence.execution_id !== expectedExecution.execution_id)
        errors.push('evidence-execution-binding-mismatch');
    if (actualEvidence.task_id !== expectedExecution.task_id)
        errors.push('evidence-task-binding-mismatch');
    if (actualEvidence.attempt !== expectedExecution.attempt)
        errors.push('evidence-attempt-binding-mismatch');
    const actualVerification = observation.verification;
    const expectedVerification = value.verification;
    if (actualVerification.verification_digest !== expectedVerification.verification_digest)
        errors.push('verification-digest-mismatch');
    if (actualVerification.evidence_digest !== actualEvidence.evidence_digest)
        errors.push('verification-evidence-binding-mismatch');
    if (actualVerification.evidence_digest !== expectedEvidence.evidence_digest)
        errors.push('verification-evidence-binding-mismatch');
    compareIdentity(errors, actualVerification, expectedExecution, 'verification');
    if (actualVerification.status !== 'passed')
        errors.push('verification-not-passed');
    const actualAcceptance = observation.human_acceptance;
    const expectedAcceptance = value.human_acceptance;
    if (actualAcceptance.acceptance_digest !== expectedAcceptance.acceptance_digest)
        errors.push('human-acceptance-digest-mismatch');
    if (actualAcceptance.review_handoff_digest !== expectedAcceptance.review_handoff_digest)
        errors.push('human-acceptance-review-handoff-binding-mismatch');
    if (actualAcceptance.verification_digest !== actualVerification.verification_digest || actualAcceptance.verification_digest !== expectedVerification.verification_digest)
        errors.push('human-acceptance-verification-binding-mismatch');
    compareIdentity(errors, actualAcceptance, expectedExecution, 'human-acceptance');
    if (actualAcceptance.decision !== 'accepted')
        errors.push('human-acceptance-not-accepted');
    if (actualAcceptance.side_effects_executed !== false)
        errors.push('human-acceptance-side-effects-invalid');
    const actualAuthority = observation.authority;
    if (actualAuthority.resume_admission !== NATIVE_CORE || actualAuthority.execution_lifecycle !== NATIVE_CORE || actualAuthority.evidence !== NATIVE_CORE || actualAuthority.human_acceptance !== NATIVE_CORE)
        errors.push('authority-bypass');
    return [...new Set(errors)];
}
function oracleDigest(value) {
    return calculateDigest(value);
}
export function evaluateNativeCheckpointResumeOracle(input) {
