    ...
    return {
        async processNext(args) {
            ...
            if (inbound)
                ...
                envelope = await input.transport.receive({ session_id: args.session_id, ...(args.receive_wait_ms === undefined ? {} : { wait_ms: args.receive_wait_ms }) });
            if (!envelope)
            ...
            if (inbound && input.stateStore) {
                ...
                if (claimed.status !== 'recorded')
                    return { status: 'blocked', message_id: envelope.message_id, reason: 'inbound-task-already-processing', side_effects_executed: false };
            }
            try {
