...
const required = (options, name) => {
    ...
    if (!value)
        throw new Error(`opn-agent-worker-${name.replaceAll('_', '-')}-required`);
    return value;
};
const spec = {
    ...
    async handler({ options, io }) {
        ...
        if (command !== 'install')
            throw new Error('opn-agent-worker-service-command-invalid');
        ...
        if (provider !== 'codex' && provider !== 'workbuddy-code')
            throw new Error('opn-agent-worker-provider-invalid');
        ...
        if (receive_wait_ms)
            args.push('--receive-wait-ms', receive_wait_ms);
        if (session_refresh_margin_ms)
            args.push('--session-refresh-margin-ms', session_refresh_margin_ms);
...
if (process.argv[1]?.endsWith('opn-agent-worker-service-cli.js'))
