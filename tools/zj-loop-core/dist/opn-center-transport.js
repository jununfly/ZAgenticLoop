...
function payloadOf(event) {
    ...
    if (payload.schema !== PAYLOAD_SCHEMA || !payload.envelope)
        return null;
...
export function createLocalOpnTransportAdapter(input) {
    ...
    if (!input.stateStore)
        throw new Error('opn-center-transport-state-store-required');
    const now = input.now ?? (() => new Date().toISOString());
    const sessions = new Map();
    async function sessionFor(session_id) {
    ...
    async function messages() {
        ...
        for (const event of events) {
            ...
            if (!envelope)
                continue;
            if (event.event_type === OFFERED_EVENT)
                resultMap.set(envelope.message_id, { envelope, acknowledged: false, cancelled: false });
            if (event.event_type === ACKNOWLEDGED_EVENT && resultMap.has(envelope.message_id))
                resultMap.get(envelope.message_id).acknowledged = true;
            if (event.event_type === CANCELLED_EVENT && resultMap.has(envelope.message_id))
                resultMap.get(envelope.message_id).cancelled = true;
        }
        return resultMap;
    }
    async function appendFact(envelope, event_type) {
        const current = await input.stateStore.getRevision(input.network_id);
        const event = { event_id: eventId(event_type, envelope), aggregate_type: AGGREGATE_TYPE, aggregate_id: envelope.message_id, event_type, occurred_at: event_type === OFFERED_EVENT ? envelope.created_at : now(), payload: { schema: PAYLOAD_SCHEMA, envelope } };
    ...
    return {
        async openSession(session) {
            ...
            if (session.network_id !== input.network_id || session.node_id !== input.node_id)
                throw new Error('transport-local-session-identity-mismatch');
        ...
        async send(sessionInput) {
            ...
            if (envelope.network_id !== input.network_id || envelope.from_node_id !== input.node_id || envelope.target_node_id === input.node_id)
                throw new Error('transport-local-envelope-identity-mismatch');
            ...
            if (existing) {
                ...
                if (prior?.envelope_digest !== envelope.envelope_digest)
                    throw new Error('transport-message-conflict');
        ...
        async acknowledge(sessionInput) {
            ...
            if (!message || message.envelope.target_node_id !== session.node_id || message.envelope.envelope_digest !== sessionInput.envelope_digest)
                throw new Error('transport-ack-message-mismatch');
        ...
        async cancel(sessionInput) {
            ...
            const message = (await messages()).get(sessionInput.message_id);
            if (!message || message.envelope.from_node_id !== session.node_id || message.envelope.envelope_digest !== sessionInput.envelope_digest)
                throw new Error('transport-cancel-message-mismatch');
            if (message.acknowledged)
                throw new Error('transport-cancel-after-ack');
