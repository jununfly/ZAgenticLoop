...
export async function uninstallOpnEndpointService(label, platform = process.platform) {
    if (platform === 'darwin') {
        ...
        if (result.status !== 0 && !/Could not find service/i.test(result.stderr || ''))
            throw new Error(`opn-endpoint-launchd-uninstall-failed:${(result.stderr || '').trim()}`);
        return { platform: 'darwin', path: pathname };
    }
    if (platform === 'win32') {
        const command = ['schtasks.exe', '/Delete', '/TN', label, '/F'];
        const result = spawnSync(command[0], command.slice(1), { encoding: 'utf8', windowsHide: true });
        if (result.status !== 0 && !/does not exist/i.test(result.stderr || ''))
            throw new Error(`opn-endpoint-task-uninstall-failed:${(result.stderr || '').trim()}`);
        return { platform: 'win32', command };
    }
    throw new Error('opn-endpoint-service-platform-unsupported');
}
