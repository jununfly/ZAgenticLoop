    ...
    const server = createPairingHttpServer({
        ...
        humanActionCommand: input.artifact_store ? {
            async decide({ network_id, request, decision }) {
                ...
                if (!targetNodeId?.trim() || targetNodeId === localNodeId)
                    throw new Error('human-action-target-node-invalid');
                ...
                try {
                    const sent = await localTransport.send({ session_id: session.session_id, envelope });
                    return { status: sent.status, artifact_id: artifact.metadata.artifact_id, message_id: envelope.message_id, target_node_id: targetNodeId };
                }
                finally {
