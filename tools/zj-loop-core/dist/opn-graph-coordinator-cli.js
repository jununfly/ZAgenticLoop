...
const spec = {
    ...
    async handler({ options, io }) {
        if (String(options.command ?? '') !== 'receive')
            throw new Error('opn-graph-coordinator-command-invalid');
        ...
        const plan = JSON.parse(await readFile(value('plan_file', 'opn-graph-coordinator-plan-required'), 'utf8'));
...
if (process.argv[1]?.endsWith('opn-graph-coordinator-cli.js'))
