...
export const OPN_GRAPH_DOGFOOD_CLI_SCHEMA = 'zj-loop.opn_graph_dogfood_cli.v1';
export async function runOpnGraphDogfoodScopeObservation(input) {
    ...
    const source = [...snapshot.events].reverse().map((event) => event.payload).find((record) => record.phase === 'source_execution');
    if (!source)
        return { schema: OPN_GRAPH_DOGFOOD_CLI_SCHEMA, status: 'blocked', reason: 'opn-graph-dogfood-source-phase-required', side_effects_executed: false };
    ...
    if (!result.record)
        return { schema: OPN_GRAPH_DOGFOOD_CLI_SCHEMA, status: result.status, ...(result.reason ? { reason: result.reason } : {}), side_effects_executed: false };
    ...
    if (appended.status === 'conflict' || appended.revision === undefined)
        return { schema: OPN_GRAPH_DOGFOOD_CLI_SCHEMA, status: 'outcome-uncertain', reason: 'opn-graph-dogfood-scope-phase-append-conflict', phase_status: result.status, side_effects_executed: false };
...
export async function runOpnGraphDogfoodVerification(input) {
    const snapshot = await input.state_store.readEvents({ network_id: input.network_id, aggregate_type: 'real-agent-dogfood-graph', aggregate_id: input.plan.dogfood_id });
    const current = projectRealAgentDogfoodGraphPhaseRecord({ plan: input.plan, events: snapshot.events });
    if (!current || current.phase !== 'scope_observation' || current.status !== 'passed' || !current.completed_phases.includes('scope_observation'))
        return { schema: OPN_GRAPH_DOGFOOD_CLI_SCHEMA, status: 'blocked', reason: 'opn-graph-dogfood-scope-phase-required', side_effects_executed: false };
    const result = await createRealAgentDogfoodGraphOpnIndependentVerificationAdapter({ plan: input.plan, network_id: input.network_id, coordinator_id: input.coordinator_id, verifier_id: input.verifier_id, transport: input.transport, artifact_store: input.artifact_store, evidence_store: input.evidence_store, source_phase: snapshot.events.map((event) => event.payload).find((record) => record.phase === 'source_execution' && record.status === 'passed'), scope_phase: current, source_evidence: async () => input.source_bytes, publish_artifact: input.publish_artifact, download_artifact: input.download_artifact })();
    if (!result.record)
        return { schema: OPN_GRAPH_DOGFOOD_CLI_SCHEMA, status: result.status, ...(result.reason ? { reason: result.reason } : {}), side_effects_executed: false };
    const appended = await appendRealAgentDogfoodGraphPhaseRecord({ stateStore: input.state_store, plan: input.plan, network_id: input.network_id, record: result.record, expected_revision: await input.state_store.getRevision(input.network_id) });
    if (appended.status === 'conflict' || appended.revision === undefined)
        return { schema: OPN_GRAPH_DOGFOOD_CLI_SCHEMA, status: 'outcome-uncertain', reason: 'opn-graph-dogfood-phase-append-conflict', phase_status: result.status, side_effects_executed: false };
    return { schema: OPN_GRAPH_DOGFOOD_CLI_SCHEMA, status: result.status, phase: result.record.phase, phase_status: result.record.status, evidence_digest: result.evidence_digest, state_revision: appended.revision, side_effects_executed: false };
}
const spec = {
    ...
    async handler({ options, io }) {
        ...
        if (command !== 'run' && command !== 'observe-scope')
            throw new Error('opn-graph-dogfood-command-invalid');
        const value = (name, error) => { const result = String(options[name] ?? '').trim(); if (!result)
            throw new Error(error); return result; };
        const plan = createRealAgentDogfoodGraphPlan(JSON.parse(await readFile(value('graph_plan', 'opn-graph-dogfood-graph-plan-required'), 'utf8')));
...
export async function runOpnGraphDogfoodCli(argv = process.argv.slice(2)) { return runCli(spec, argv); }
if (process.argv[1]?.endsWith('opn-graph-dogfood-cli.js'))
    process.exitCode = await runOpnGraphDogfoodCli();
