...
export function createOpnMessageReadModel(input) {
    ...
    const next_action = input.delivery_state === 'acknowledged' ? 'none' : input.delivery_state === 'cancelled' ? 'cancelled' : input.delivery_state === 'blocked' || input.delivery_state === 'rejected' ? 'blocked' : input.delivery_state === 'retry_scheduled' ? 'retry-delivery' : input.envelope.artifact_refs.length ? 'inspect-artifact' : 'await-ack';
