...
export const OPN_OUTBOUND_TASK_APPROVAL_SCHEMA = 'zj-loop.opn_outbound_task_approval.v1';
export const OPN_OUTBOUND_TASK_APPROVAL_AGGREGATE = 'opn-outbound-task-approval';
function eventId(approvalId, eventType) { return `${eventType}:${approvalId}`; }
function payloadOf(event) {
    const value = event.payload;
    return value.schema === OPN_OUTBOUND_TASK_APPROVAL_SCHEMA && value.approval ? value : null;
}
function currentStatus(events) {
    ...
    for (const event of events) {
        ...
        if (payload)
            current = { ...payload.approval };
    }
    return current;
}
export function outboundTaskApprovalDigest(envelope) {
...
export function createOutboundTaskApproval(input) {
    if (validateTransportEnvelope(input.envelope).status !== 'valid')
        throw new Error('outbound-task-envelope-invalid');
    if (input.envelope.network_id !== input.network_id || input.envelope.notification_kind !== 'agent.task')
        throw new Error('outbound-task-envelope-scope-invalid');
...
export async function listOutboundTaskApprovals(input) {
    ...
    return [...grouped.values()].map(currentStatus).filter((value) => value !== null).map((value) => value.status === 'pending' && Date.parse(value.expires_at) <= Date.now() ? { ...value, status: 'expired' } : value);
}
export async function recordOutboundTaskApproval(input) {
    ...
    if (current) {
        if (current.envelope_digest !== input.approval.envelope_digest)
            throw new Error('outbound-task-approval-id-conflict');
    ...
    if (result.status !== 'recorded')
        return { status: result.status, approval: input.approval };
    return { status: 'recorded', approval: input.approval };
}
export async function appendOutboundTaskApprovalDecision(input) {
    ...
    if (!existing || existing.envelope_digest !== input.approval.envelope_digest)
        throw new Error('outbound-task-approval-not-found');
    if (existing.status === input.decision || existing.status === 'published')
        return { status: 'duplicate', approval: existing };
    if (existing.status !== 'pending')
        throw new Error('outbound-task-approval-state-conflict');
    const approval = { ...existing, status: input.decision, human_id: input.human_id, human_note: input.human_note, decided_at: now };
...
export async function appendOutboundTaskPublished(input) {
    ...
    if (existing.status !== 'approved')
        throw new Error('outbound-task-approval-not-approved');
    const approval = { ...existing, status: 'published', published_at: now, published_message_id: input.message_id };
    const revision = await input.stateStore.getRevision(input.approval.network_id);
