...
export const opnTransportCliSpec = {
    ...
    async handler({ options, io }) {
        ...
        if (command === 'gateway-task-send') {
            ...
            try {
                ...
                if (response.statusCode !== 200 && response.statusCode !== 201)
                    throw new Error(String(result.reason ?? 'opn-gateway-task-approval-request-failed'));
        ...
        if (command === 'gateway-cancel') {
            ...
            if (!message_id || !envelope_digest || !reason)
                throw new Error('opn-gateway-cancel-fields-required');
            ...
            const result = parsedArtifactResponse(response);
            if (response.statusCode !== 200 && response.statusCode !== 202)
                throw new Error(String(result.reason ?? 'opn-gateway-message-cancel-failed'));
        ...
        try {
            ...
            if (command === 'result-send') {
                ...
                if (!target || !messageId)
                    throw new Error('opn-result-target-and-message-id-required');
                const resultFile = String(options.result_file ?? '').trim();
                const resultStatus = String(options.result_status ?? 'succeeded').trim();
                if (resultStatus !== 'succeeded' && resultStatus !== 'blocked')
                    throw new Error('opn-result-status-invalid');
                const artifactRoot = String(options.artifact_store ?? '').trim();
                if (!artifactRoot)
                    throw new Error('opn-result-artifact-store-required');
