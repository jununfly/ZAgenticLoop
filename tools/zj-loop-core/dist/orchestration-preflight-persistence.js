...
export const ORCHESTRATION_PREFLIGHT_FACT_SCHEMA = 'zj-loop.orchestration_preflight_fact.v1';
export async function persistOrchestrationPreflight(input) {
    if (!/^sha256:[0-9a-f]{64}$/.test(input.result.plan_digest) || input.result.side_effects_executed !== false)
        throw new Error('orchestration-preflight-result-invalid');
    const json = canonicalize(input.result);
    if (typeof json !== 'string')
        throw new Error('orchestration-preflight-artifact-invalid');
