...
import { projectPairingEnrollment, projectPairingRequests } from './pairing-projection.js';
...
function sessionResponse(session, projection, token, enrollment) {
    return {
        session: {
...
export function createPairingHttpServer(input) {
    ...
    return createServer({ ...input.tls, requestCert: true, rejectUnauthorized: false }, async (request, response) => {
        ...
        if (ownerOutboundTaskList || ownerOutboundTaskRequest || ownerOutboundTaskDecision) {
            ...
            if (!input.ownerOutboundTaskApproval) {
                blocked(response, 'outbound-task-approval-unavailable');
                return;
            }
            let value = {};
            ...
            try {
                if (ownerOutboundTaskList) {
                    json(response, 200, { schema: PAIRING_HTTP_SCHEMA, status: 'ok', network_id: networkId, ...(await input.ownerOutboundTaskApproval.list({ network_id: networkId })), side_effects_executed: false });
                    return;
                }
                const approval = value.approval;
                if (!approval || typeof approval !== 'object' || approval.network_id !== networkId) {
                    blocked(response, 'outbound-task-approval-invalid');
                    return;
                }
                ...
                if (approval.approval_id !== requestId || (value.decision !== 'approved' && value.decision !== 'rejected') || typeof value.human_note !== 'string' || !value.human_note.trim()) {
                    blocked(response, 'outbound-task-decision-invalid');
                    return;
                }
                ...
                blocked(response, error instanceof Error ? error.message : 'outbound-task-approval-failed');
            }
            return;
        }
        ...
        if (ownerMessage) {
            ...
            if (!input.ownerMessageCommand) {
                blocked(response, 'owner-message-command-unavailable');
                return;
            }
            let value;
            try {
            ...
            if (!networkId.trim() || !envelope || validateTransportEnvelope(envelope).status !== 'valid') {
                blocked(response, 'owner-message-envelope-invalid');
                return;
            }
            const transportEnvelope = envelope;
            if (transportEnvelope.network_id !== networkId) {
                blocked(response, 'owner-message-network-mismatch');
                return;
            }
            const auth = await Promise.resolve(input.ownerAuthenticator.authenticate({ action: 'message.send', authorization: typeof request.headers.authorization === 'string' ? request.headers.authorization : null }));
            if (auth.status !== 'allowed') {
                blocked(response, auth.reason ?? 'owner-not-authorized');
                return;
            }
            try {
                const result = await input.ownerMessageCommand.send({ network_id: networkId, envelope: transportEnvelope });
                json(response, 202, { schema: PAIRING_HTTP_SCHEMA, status: 'accepted', network_id: networkId, message_id: transportEnvelope.message_id, envelope_digest: transportEnvelope.envelope_digest, result, side_effects_executed: false });
            }
            catch (error) {
                blocked(response, error instanceof Error ? error.message : 'owner-message-send-failed');
            }
            return;
        }
        ...
        if (ownerList || ownerApprove || ownerReject) {
            ...
            try {
                ...
                if (ownerApprove) {
                    ...
                    if (existingApproval && existingApproval.type === 'human-approved')
                        decision = existingApproval;
                    else {
