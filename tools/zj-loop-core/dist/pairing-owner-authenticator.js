    ...
    return {
        authenticate(request) {
            if (!bearerMatches(request.authorization, input.owner_token))
                return blocked();
            if (request.action === 'pairing.list' || request.action === 'pairing.inbox' || request.action === 'human.action.list' || request.action === 'human.action.decide' || request.action === 'message.send')
