...
export function createPemFileHumanSigner(input) {
    ...
    const load = async () => {
        ...
        if (cached && cached.identity.public_key_fingerprint !== current.identity.public_key_fingerprint)
            throw new Error('pem-human-signer-identity-changed');
        cached = current;
        return current;
    };
    return {
        ...
        async sign(input) {
            ...
            return { schema: HUMAN_SIGNATURE_SCHEMA, algorithm: 'ECDSA-P256', public_key_fingerprint: current.identity.public_key_fingerprint, signature_base64: Buffer.from(signature).toString('base64') };
        },
    };
}
