...
import { buildPrdHandoffRequest, readPrdHandoffRequest, runPrdHandoffRunner, } from './prd-handoff-runner.js';
...
process.exitCode = await runCli({
    ...
    async handler({ io, options }) {
        if (options.command !== 'handoff-plan')
        ...
        if (typeof options['comment-out'] === 'string')
            await writeFile(options['comment-out'], result.comment_body);
