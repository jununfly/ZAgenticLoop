...
import { createProviderAuthAuthorityStartAssemblyFromConfig } from './provider-auth-authority-start-assembly.js';
function parseConfigPath(argv) {
    if (argv.length !== 2 || argv[0] !== '--config' || !argv[1] || !argv[1].startsWith('/'))
        throw new Error('provider-auth-authority-child-config-argument-invalid');
    return argv[1];
}
function createConfiguredPeerGate(config) {
    if (process.platform !== 'darwin')
        throw new Error('provider-auth-authority-child-platform-unsupported');
    if (!config.macos_helper_path || !config.macos_helper_digest)
        throw new Error('provider-auth-authority-child-macos-helper-required');
...
export async function startProviderAuthAuthorityChild(input) {
    ...
    return {
        ...
        async shutdown() {
            ...
            try {
                ...
                return { status: 'outcome-uncertain', reason: 'provider-auth-authority-state-store-close-failed' };
            }
        },
    };
}
export async function main(argv = process.argv.slice(2)) {
    ...
    try {
        ...
        await new Promise((resolve) => {
            let handled = false;
            const onSignal = async () => { if (handled)
                return; handled = true; const result = await child?.shutdown(); if (result?.status !== 'stopped')
