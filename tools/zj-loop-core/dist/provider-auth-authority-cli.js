...
import { createProviderAuthAuthorityExternalStartController } from './provider-auth-authority-external-start-controller.js';
    ...
    return runCli({
        ...
        async handler({ options }) {
            ...
            if (command === 'start') {
                ...
                if (!configPath)
                    throw new Error('provider-auth-authority-start-config-required');
                let controller;
                try {
                    ...
                    return await new Promise((resolve) => {
                        ...
                        const onSignal = async (signal) => {
                            if (stopping)
                                return;
                            stopping = true;
                            const result = await controller.stop().catch(() => ({ status: 'outcome-uncertain', reason: 'provider-auth-authority-cli-stop-failed' }));
