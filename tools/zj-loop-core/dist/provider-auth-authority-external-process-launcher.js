...
export function createProviderAuthAuthorityExternalProcessLauncher(input) {
    if (!input.executable || !input.executable.startsWith('/') || input.executable.includes('\0'))
        throw new Error('provider-auth-authority-external-executable-invalid');
    if (!Array.isArray(input.args) || !input.args.every((arg) => typeof arg === 'string' && !arg.includes('\0')))
        throw new Error('provider-auth-authority-external-args-invalid');
    if (!input.cwd || !input.cwd.startsWith('/') || input.cwd.includes('\0'))
        throw new Error('provider-auth-authority-external-cwd-invalid');
    if (!input.socket_path || !input.socket_path.startsWith('/') || input.socket_path.includes('\0'))
        throw new Error('provider-auth-authority-external-socket-path-invalid');
    if (!input.process_adapter || typeof input.process_adapter.launch !== 'function')
        throw new Error('provider-auth-authority-external-process-adapter-required');
    ...
    if (!Number.isInteger(timeoutMs) || timeoutMs < 1 || !Number.isInteger(terminationGraceMs) || terminationGraceMs < 1 || !Number.isInteger(closeTimeoutMs) || closeTimeoutMs < 1)
        throw new Error('provider-auth-authority-external-timeout-invalid');
    ...
    return {
        ...
        async readiness() {
            if (!handle || !(await probe(input.socket_path)))
                return { status: 'blocked', reason: 'provider-auth-authority-external-ipc-unavailable' };
        ...
        async close() {
            ...
            let timer;
            try {
                await Promise.race([current.wait(), new Promise((_, reject) => { timer = setTimeout(() => reject(new Error('provider-auth-authority-external-close-timeout')), closeTimeoutMs); })]);
                handle = undefined;
            }
            finally {
                if (timer)
