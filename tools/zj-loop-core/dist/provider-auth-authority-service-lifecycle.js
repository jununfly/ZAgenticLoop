...
export function createProviderAuthAuthorityServiceLifecycle(input) {
    if (!input.verifier || typeof input.verifier.verify !== 'function')
        throw new Error('provider-auth-authority-process-identity-verifier-required');
    ...
    return {
        async status({ binding }) {
            ...
            if (!(await probe(binding.socket_path)))
                return { status: 'outcome-uncertain', service_id: binding.service_id, pid: binding.pid, socket_path: binding.socket_path, reason: 'provider-auth-authority-ipc-unavailable' };
...
export async function readProviderAuthAuthorityServiceStatus(input) {
