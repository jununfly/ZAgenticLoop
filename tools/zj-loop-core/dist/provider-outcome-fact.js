...
export async function recordProviderOutcome(input) {
    ...
    if (validation.status === 'blocked')
        return { schema: PROVIDER_OUTCOME_RECORDED_SCHEMA, status: 'blocked', event_id, side_effects_executed: false, reason: 'provider-outcome-invalid' };
    const aggregate_id = aggregateId(outcome);
    const result = await input.stateStore.runAtomic((transaction) => {
        const rows = transaction.database.prepare("SELECT event_id, payload_json FROM state_events WHERE network_id = ? AND aggregate_type = 'provider-execution' AND aggregate_id = ? AND event_type = 'provider.outcome.recorded'").all(outcome.network_id, aggregate_id);
        if (rows.length > 0) {
            ...
            return existing.outcome?.outcome_digest === outcome.outcome_digest && rows[0].event_id === event_id
                ...
                : { status: 'conflict', event_id, current_revision: input.expected_revision, reason: 'provider-request-outcome-conflict' };
        }
