...
export const PROVIDER_RUNTIME_ARTIFACT_MANIFEST_SCHEMA = 'zj-loop.provider_runtime_artifact_manifest.v1';
...
function canonical(value) {
    ...
    if (typeof result !== 'string')
        throw new Error('provider-runtime-artifact-manifest-canonicalization-invalid');
    return result;
}
...
export function providerRuntimeArtifactManifestDigest(value) {
    const { manifest_digest: _, ...unsigned } = value;
    return digest(unsigned);
}
export function createProviderRuntimeArtifactManifest(input) {
    const unsigned = { schema: PROVIDER_RUNTIME_ARTIFACT_MANIFEST_SCHEMA, ...structuredClone(input) };
    if (!validUnsigned(unsigned))
        throw new Error('provider-runtime-artifact-manifest-invalid');
    return Object.freeze({ ...unsigned, manifest_digest: digest(unsigned) });
}
export function validateProviderRuntimeArtifactManifest(input, options) {
    if (!input || typeof input !== 'object' || Array.isArray(input))
        return { status: 'blocked', reason: 'provider-runtime-artifact-manifest-invalid' };
    const value = input;
    if (!validUnsigned(value) || !DIGEST.test(String(value.manifest_digest)))
        return { status: 'blocked', reason: 'provider-runtime-artifact-manifest-invalid' };
    const { manifest_digest: manifestDigest, ...unsigned } = value;
    if (manifestDigest !== digest(unsigned))
        return { status: 'blocked', reason: 'provider-runtime-artifact-manifest-digest-invalid' };
    if (options?.profile && value.profile !== options.profile)
        return { status: 'blocked', reason: 'provider-runtime-artifact-profile-mismatch' };
    const signing = value.signing;
    if (value.profile === 'development-local') {
        if (signing.kind !== 'ad-hoc' || signing.team_id !== null || signing.notarized)
            return { status: 'blocked', reason: 'provider-runtime-development-signing-policy-invalid' };
    }
...
export async function readProviderRuntimeArtifactManifest(filePath) {
    if (typeof filePath !== 'string' || !filePath.startsWith('/') || filePath.includes('\0'))
        throw new Error('provider-runtime-artifact-manifest-path-invalid');
