...
import { createProviderRuntimeForegroundService } from './provider-runtime-foreground-service.js';
...
export function createProviderRuntimeStartAssembly(input) {
    ...
    if (!/^sha256:[0-9a-f]{64}$/.test(input.process_identity_digest))
        throw new Error('provider-runtime-start-process-identity-digest-invalid');
    if (typeof input.revoke_ref !== 'function')
        throw new Error('provider-runtime-start-revoke-authority-required');
    if (typeof input.create_launcher !== 'function')
        throw new Error('provider-runtime-start-launcher-factory-required');
    ...
    const launcher = input.create_launcher({ runtime, resolver, config, state_store: stateStore });
