...
async function start(options) {
    ...
    if (mode === 'write-enabled' && allowedFiles.length === 0)
        throw new Error('write-scope-allowed-file-required');
...
async function status(options) {
    ...
    try {
        ...
        const graphSummary = {};
        try {
            const summary = JSON.parse(await readFile(path.join(evidenceStore, `${dogfoodId}.approval-summary.json`), 'utf8'));
            if (summary.graph_plan && typeof summary.graph_plan === 'object') {
                const graphPlan = createRealAgentDogfoodGraphPlan(summary.graph_plan);
                const graphSnapshot = await stateStore.readEvents({ network_id: networkId, aggregate_type: 'real-agent-dogfood-graph', aggregate_id: dogfoodId });
                const phase = projectRealAgentDogfoodGraphPhaseRecord({ plan: graphPlan, events: graphSnapshot.events });
...
async function replay(options) {
    ...
    try {
        ...
        const lifecycleEvents = lifecycleSnapshot.events;
        const summary = JSON.parse(await readFile(path.join(evidencePath, `${dogfoodId}.approval-summary.json`), 'utf8'));
        if (!summary.graph_plan || typeof summary.graph_plan !== 'object')
            throw new Error('replay-graph-plan-required');
        const plan = createRealAgentDogfoodGraphPlan(summary.graph_plan);
        const graphSnapshot = await stateStore.readEvents({ network_id: networkId, aggregate_type: 'real-agent-dogfood-graph', aggregate_id: dogfoodId });
