...
import { appendRealAgentDogfoodEvent, createRealAgentDogfoodDraft, createRealAgentDogfoodTransition, projectRealAgentDogfoodLifecycle, } from './real-agent-dogfood-lifecycle.js';
...
async function buildAdapterContractDigest(input) {
...
async function validateRuntimePaths(input) {
    ...
    if (isInside(stateStore, repo) || isInside(evidenceStore, repo))
        throw new Error('runtime-storage-path-inside-repo');
...
function executionMode(options) {
    ...
    if (value === undefined)
        return 'read-only';
    if (value === 'read-only' || value === 'write-enabled')
        return value;
    throw new Error('execution-mode-invalid');
}
function outputLifecycle(lifecycle, extra = {}) {
    return {
...
async function append(stateStore, networkId, event, expectedRevision) {
...
async function start(options) {
    ...
    if (!['network-denied', 'network-allowed'].includes(networkPolicy))
        throw new Error('network-policy-invalid');
    ...
    try {
        ...
        const policyDigest = digest({ repo: paths.repo, worktree_path: worktree.worktree_path, base_commit: worktree.base_commit, branch: worktree.branch, executable, network_policy: networkPolicy, execution_mode: mode });
        ...
        const summaryBase = { schema: 'zj-loop.real_agent_dogfood_approval_summary.v1', status: awaiting.lifecycle.status, network_id: networkId, dogfood_id: dogfoodId, execution_id: executionId, attempt: 1, goal, repo: paths.repo, worktree_path: worktree.worktree_path, branch: worktree.branch, base_commit: worktree.base_commit, provider_id: providerId, adapter: adapter, adapter_contract_digest: adapterContractDigest, executable, network_policy: networkPolicy, execution_mode: mode, policy_digest: policyDigest, lifecycle_revision: revision, lifecycle_digest: awaiting.lifecycle.lifecycle_digest, created_at: now };
...
async function resume(options) {
    ...
    if (!/^[A-Za-z0-9._-]{1,256}$/.test(approvalId))
        throw new Error('approval-id-invalid');
    const defaults = defaultRealAgentDogfoodRuntimePaths();
    const evidenceStore = await canonicalPath(typeof options['evidence-store'] === 'string' ? options['evidence-store'] : defaults.evidence_store);
    const statePath = typeof options['state-store'] === 'string' ? options['state-store'] : defaults.state_store;
    const stateStore = createSqliteStateStore({ filename: statePath });
    try {
        ...
        if (lifecycle.dogfood_id !== dogfoodId)
            throw new Error('dogfood-id-mismatch');
        ...
        const summaryPath = path.join(evidenceStore, `${dogfoodId}.approval-summary.json`);
        const summary = JSON.parse(await readFile(summaryPath, 'utf8'));
        if (summary.summary_digest !== envelope.approval_summary_digest || summary.policy_digest !== envelope.policy_digest || summary.lifecycle_revision !== envelope.lifecycle_revision || typeof summary.adapter_contract_digest !== 'string' || !DIGEST.test(summary.adapter_contract_digest))
        ...
        if (lifecycle.provider_id === 'codex') {
            ...
            if (summary.execution_mode !== 'read-only' && summary.execution_mode !== 'write-enabled')
        ...
        if (result.status === 'conflict')
            throw new Error('lifecycle-revision-conflict');
        if (!workerContext)
        ...
        try {
            ...
            const blockedResult = await append(stateStore, networkId, blocked.event, result.revision);
            if (blockedResult.status === 'conflict')
                throw new Error('worker-start-failure-record-conflict');
