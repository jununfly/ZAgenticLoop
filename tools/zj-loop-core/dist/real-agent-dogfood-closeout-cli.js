...
function required(options, name) { const value = options[name]; if (typeof value !== 'string' || value.trim() === '')
    throw new Error(`${name}-required`); return value; }
function signerFor(options, deps) {
    if (deps.signer)
        return deps.signer;
    if (process.platform !== 'darwin')
        throw new Error('closeout-signer-adapter-required');
...
export function runRealAgentDogfoodCloseoutCli(argv = process.argv.slice(2), io = defaultCliIo, deps = {}) {
    return runCli({
        ...
        async handler({ options }) {
            if (String(options.command) !== 'closeout')
                throw new Error('unsupported-closeout-command');
...
if (process.argv[1] && path.resolve(process.argv[1]) === path.resolve(fileURLToPath(import.meta.url)))
    process.exitCode = await runRealAgentDogfoodCloseoutCli();
