...
import { createRealAgentDogfoodVerificationPlan, prepareDisposableRealAgentDogfoodVerifierWorktree } from './real-agent-dogfood-independent-verification.js';
...
export function createRealAgentDogfoodGraphIndependentVerificationAdapter(input) {
    return async () => {
        if (input.source_phase.phase !== 'source_execution' || input.source_phase.status !== 'passed' || input.scope_phase.phase !== 'scope_observation' || input.scope_phase.status !== 'passed')
            return { status: 'outcome-uncertain', reason: 'independent-verification-prerequisite-not-passed' };
        if (input.source_phase.network_id !== input.network_id || input.scope_phase.network_id !== input.network_id || input.source_phase.plan_digest !== input.plan.plan_digest || input.scope_phase.plan_digest !== input.plan.plan_digest || !input.source_phase.completed_phases.includes('source_execution') || !input.scope_phase.completed_phases.includes('scope_observation'))
            return { status: 'outcome-uncertain', reason: 'independent-verification-prerequisite-binding-invalid' };
        if (input.source_phase.execution_id !== input.plan.execution_id || input.scope_phase.execution_id !== input.plan.execution_id || input.scope_phase.actor_kind !== 'coordinator' || !input.scope_phase.actor_identity || !input.verifier_id.trim())
            return { status: 'outcome-uncertain', reason: 'independent-verification-identity-binding-invalid' };
        if (!DIGEST.test(input.source_phase.execution_binding_digest ?? '') || !DIGEST.test(input.source_phase.worker_lease_digest ?? ''))
            return { status: 'outcome-uncertain', reason: 'independent-verification-source-binding-invalid' };
        let inputCommit;
        try {
            inputCommit = await git(input.plan.source_worktree, ['rev-parse', 'HEAD']);
        }
        catch {
            return { status: 'outcome-uncertain', reason: 'independent-verification-source-commit-unavailable' };
        }
        if (!COMMIT.test(inputCommit))
            return { status: 'outcome-uncertain', reason: 'independent-verification-source-commit-invalid' };
        const verificationPlan = createRealAgentDogfoodVerificationPlan({ execution_id: input.plan.execution_id, attempt: input.plan.attempt, verifier_id: input.verifier_id, input_commit: inputCommit, repo_root: input.plan.source_worktree, verifier_worktree_root: path.dirname(input.plan.verifier_worktree), commands: input.commands });
        if (path.resolve(path.join(verificationPlan.verifier.worktree_root, `${input.plan.execution_id}-attempt-${input.plan.attempt}`)) !== path.resolve(input.plan.verifier_worktree))
            return { status: 'outcome-uncertain', reason: 'independent-verification-worktree-binding-invalid' };
        let prepared;
        try {
            prepared = await (input.prepare_worktree ?? prepareDisposableRealAgentDogfoodVerifierWorktree)({ repo_root: input.plan.source_worktree, worktree_root: verificationPlan.verifier.worktree_root, execution_id: input.plan.execution_id, attempt: input.plan.attempt, input_commit: inputCommit, verifier_id: input.verifier_id });
        }
        catch {
            return { status: 'outcome-uncertain', reason: 'independent-verification-worktree-prepare-failed' };
        }
        let expectedWorktreePath;
        let preparedWorktreePath;
        try {
            ...
            return { status: 'outcome-uncertain', reason: 'independent-verification-worktree-fact-invalid' };
        }
        if (preparedWorktreePath !== expectedWorktreePath || prepared.input_commit !== inputCommit)
            return { status: 'outcome-uncertain', reason: 'independent-verification-worktree-fact-invalid' };
        const observations = [];
        ...
        try {
            ...
            return { status: 'outcome-uncertain', reason: 'independent-verification-worktree-observation-failed' };
        }
        ...
        let evidence;
        try {
            evidence = await input.evidence_store.put({ content: JSON.stringify(evidencePayload), kind: 'real-agent-dogfood-graph-independent-verification' });
        }
        catch {
            return { status: 'outcome-uncertain', reason: 'independent-verification-evidence-write-failed' };
        }
        if (!DIGEST.test(evidence.digest))
            return { status: 'outcome-uncertain', reason: 'independent-verification-evidence-invalid' };
