...
export function createRealAgentDogfoodGraphOpnIndependentVerificationAdapter(input) {
    return async () => {
        if (input.source_phase.phase !== 'source_execution' || input.source_phase.status !== 'passed' || input.scope_phase.phase !== 'scope_observation' || input.scope_phase.status !== 'passed')
            return { status: 'outcome-uncertain', reason: 'opn-independent-verification-prerequisite-not-passed' };
        if (input.source_phase.plan_digest !== input.plan.plan_digest || input.scope_phase.plan_digest !== input.plan.plan_digest || input.source_phase.execution_id !== input.plan.execution_id || input.scope_phase.execution_id !== input.plan.execution_id || input.source_phase.network_id !== input.network_id || input.scope_phase.network_id !== input.network_id)
            return { status: 'outcome-uncertain', reason: 'opn-independent-verification-prerequisite-binding-invalid' };
        if (input.scope_phase.actor_kind !== 'coordinator' || !input.scope_phase.actor_identity || !input.verifier_id.trim() || !input.coordinator_id.trim())
            return { status: 'outcome-uncertain', reason: 'opn-independent-verification-identity-binding-invalid' };
        ...
        try {
            ...
            if (!validDigest(sourceArtifact.metadata.artifact_id))
                throw new Error('source-artifact-digest-invalid');
            if (input.publish_artifact)
                await input.publish_artifact({ bytes, metadata: sourceArtifact.metadata, transfer_id: `graph-verification-request:${messageId}`, target_node_id: input.verifier_id });
        }
        catch {
            return { status: 'outcome-uncertain', reason: 'opn-independent-verification-source-artifact-unavailable' };
        }
        ...
        let session;
        try {
            session = await input.transport.openSession({ network_id: input.network_id, node_id: input.coordinator_id });
        }
        catch {
            return { status: 'outcome-uncertain', reason: 'opn-independent-verification-session-unavailable' };
        }
        try {
            const sent = await input.transport.send({ session_id: session.session_id, envelope });
            if (sent.status === 'blocked')
                return { status: 'blocked', reason: 'opn-independent-verification-request-blocked' };
            ...
            if (!response)
                return { status: 'outcome-uncertain', reason: 'opn-independent-verification-result-unavailable' };
            if (response.target_node_id !== input.coordinator_id || response.from_node_id !== input.verifier_id || response.notification_kind !== 'graph.verification.result' || response.network_id !== input.network_id || response.plan_id !== planId || response.plan_revision !== input.plan.attempt || response.task_id !== input.plan.execution_id || response.artifact_refs.length < 1)
                return { status: 'blocked', reason: 'opn-independent-verification-result-scope-mismatch' };
            let result;
            let resultArtifact;
            try {
                ...
                if (validation.status === 'blocked')
                    return { status: 'blocked', reason: `opn-independent-verification-result-${validation.reason}` };
                result = parsed;
                if (result.graph_id !== input.plan.dogfood_id || result.network_id !== input.network_id || result.plan_id !== planId || result.plan_revision !== input.plan.attempt || result.task_id !== input.plan.execution_id || result.plan_digest !== input.plan.plan_digest || result.source_evidence_ref !== sourceArtifact.metadata.artifact_id || result.verifier_node_id !== input.verifier_id)
                    return { status: 'blocked', reason: 'opn-independent-verification-result-scope-mismatch' };
            ...
            let evidence;
            try {
                evidence = await input.evidence_store.put({ content: JSON.stringify(evidencePayload), kind: 'real-agent-dogfood-graph-opn-independent-verification' });
            }
            catch {
                return { status: 'outcome-uncertain', reason: 'opn-independent-verification-evidence-write-failed' };
            }
            ...
            return { status: 'outcome-uncertain', reason: error instanceof Error ? `opn-independent-verification-${error.message}` : 'opn-independent-verification-outcome-uncertain' };
        }
        finally {
            await input.transport.closeSession({ session_id: session.session_id }).catch(() => { });
        }
    };
}
