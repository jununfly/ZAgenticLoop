...
const COMMAND_IDS = ['git-diff-check', 'project-build', 'target-test', 'graph-regression'];
...
function completedThroughMerge(status) {
    return status === 'passed'
        ? ['source_execution', 'scope_observation', 'independent_verification', 'human_acceptance', 'merge', 'post_merge_gate']
...
export function createRealAgentDogfoodGraphPostMergeGateAdapter(input) {
    return async () => {
        if (input.merge_phase.phase !== 'merge' || input.merge_phase.status !== 'passed' || !['coordinator', 'human'].includes(input.merge_phase.actor_kind ?? '') || !input.merge_phase.actor_identity || input.merge_phase.network_id !== input.network_id || input.merge_phase.plan_digest !== input.plan.plan_digest || input.merge_phase.execution_id !== input.plan.execution_id || !input.merge_phase.completed_phases.includes('merge') || !input.merge_phase.evidence_digest || !DIGEST.test(input.merge_phase.evidence_digest))
            return { status: 'blocked', reason: 'post-merge-gate-merge-prerequisite-invalid' };
        if (input.human_acceptance.decision !== 'accepted' || input.human_acceptance.merge_authorization_digest !== nativeOpnTracerMergeAuthorizationDigest(input.authorization))
            return { status: 'blocked', reason: 'post-merge-gate-human-acceptance-binding-invalid' };
        if (!input.verifier_id.trim() || !validCommands(input.commands))
            return { status: 'blocked', reason: 'post-merge-gate-command-plan-invalid' };
        ...
        if (ref.status !== 0 || targetHead.status !== 0 || sourceCommit.status !== 0 || targetStatus.status !== 0) {
            ...
            let evidence;
            try {
                evidence = await input.evidence_store.put({ content: JSON.stringify(evidencePayload), kind: 'real-agent-dogfood-graph-post-merge-gate' });
            }
            catch {
                return { status: 'outcome-uncertain', reason: 'post-merge-gate-evidence-write-failed' };
            }
            if (!DIGEST.test(evidence.digest))
                return { status: 'outcome-uncertain', reason: 'post-merge-gate-evidence-invalid' };
        ...
        if (!COMMIT.test(targetHeadSha) || !COMMIT.test(sourceCommitSha))
            return { status: 'outcome-uncertain', reason: 'post-merge-gate-commit-observation-invalid' };
        const observations = [];
