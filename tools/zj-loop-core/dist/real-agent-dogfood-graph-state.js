...
function assertPlanBinding(input) {
    if (input.plan.dogfood_id !== input.dogfood_id || input.plan.execution_id !== input.execution_id || !input.network_id.trim())
        throw new Error('graph-state-plan-binding-invalid');
}
function assertRecord(record, plan) {
    ...
    if (hasEvidence && (!/^sha256:[0-9a-f]{64}$/.test(record.evidence_digest ?? '') || !Array.isArray(record.evidence_refs) || record.evidence_refs.length === 0 || record.evidence_refs.some((ref) => !/^sha256:[0-9a-f]{64}$/.test(ref))))
        throw new Error('graph-state-evidence-binding-invalid');
    if (record.actor_kind === 'agent-node' && (!hasEvidence || !/^sha256:[0-9a-f]{64}$/.test(record.execution_binding_digest ?? '') || !/^sha256:[0-9a-f]{64}$/.test(record.worker_lease_digest ?? '')))
        throw new Error('graph-state-execution-binding-required');
}
export function createRealAgentDogfoodGraphPhaseRecord(input) {
    ...
    if (!phasePrefix(completed) || completed.length > REAL_AGENT_DOGFOOD_GRAPH_PHASES.length || !REAL_AGENT_DOGFOOD_GRAPH_PHASES.includes(input.phase))
        throw new Error('graph-state-phase-invalid');
    const record = {
...
export function projectRealAgentDogfoodGraphPhaseRecord(input) {
    let current = null;
    for (const event of input.events) {
        if (event.aggregate_type !== REAL_AGENT_DOGFOOD_GRAPH_STATE_AGGREGATE || event.aggregate_id !== input.plan.dogfood_id || event.event_type !== REAL_AGENT_DOGFOOD_GRAPH_STATE_EVENT)
            continue;
        ...
        if (record.execution_id !== input.plan.execution_id || record.plan_digest !== input.plan.plan_digest)
            continue;
        assertRecord(record, input.plan);
        if (current && record.status === 'passed' && record.completed_phases.length <= current.completed_phases.length)
            throw new Error('graph-state-phase-order-invalid');
        current = record;
    }
    return current;
}
