...
export const REAL_AGENT_DOGFOOD_POST_RUN_PROOF_SCHEMA = 'zj-loop.real_agent_dogfood_post_run_proof.v1';
const DIGEST = /^sha256:[0-9a-f]{64}$/;
function canonicalDigest(value) {
    const json = canonicalize(value);
    if (typeof json !== 'string')
        throw new Error('real-agent-dogfood-post-run-proof-canonicalization-invalid');
    return `sha256:${createHash('sha256').update(json, 'utf8').digest('hex')}`;
}
function validDigest(value) { return typeof value === 'string' && DIGEST.test(value); }
export function realAgentDogfoodPostRunProofDigest(proof) { return canonicalDigest(proof); }
...
export function verifyRealAgentDogfoodPostRunProof(input) {
    const proof = input.proof;
    const reasons = [];
    if (proof.schema !== REAL_AGENT_DOGFOOD_POST_RUN_PROOF_SCHEMA || proof.status !== 'signed')
        reasons.push('post-run-proof-signature-missing');
    if (proof.execution_id !== input.execution_id || proof.attempt !== input.attempt || proof.worktree_path !== input.worktree_path || proof.executable_digest !== input.executable_digest || proof.stdout_digest !== input.stdout_digest || proof.stderr_digest !== input.stderr_digest)
        reasons.push('post-run-proof-binding-invalid');
    if (!validDigest(proof.executable_digest) || !validDigest(proof.stdout_digest) || !validDigest(proof.stderr_digest))
        reasons.push('post-run-proof-digest-invalid');
    if (!proof.process_boundary?.all_descendants_terminated || proof.process_boundary.orphan_processes_detected || proof.process_boundary.unknown_descendants_detected)
        reasons.push('post-run-proof-process-boundary-invalid');
    if (!proof.after_worktree_clean || !proof.after_network_policy_proved || !proof.after_credentials_clean || proof.side_effects_detected)
        reasons.push('post-run-proof-safety-state-invalid');
    const { signature: _, ...unsigned } = proof;
    if (!validSignature(realAgentDogfoodPostRunProofDigest(unsigned), proof.signature))
        reasons.push('post-run-proof-signature-invalid');
    return reasons.length === 0 ? { status: 'accepted' } : { status: 'blocked', reasons: [...new Set(reasons)].sort() };
}
export function createFakeRealAgentDogfoodPostRunProof(input) {
