...
function signerFor(options, deps) {
    ...
    if (process.platform !== 'darwin')
        throw new Error('review-signer-adapter-required');
...
async function readPackage(options, actor) {
...
async function decide(options, deps) {
    ...
    try {
        ...
        if (lifecycle.status !== 'review-pending')
...
export function runRealAgentDogfoodReviewCli(argv = process.argv.slice(2), io = defaultCliIo, deps = {}) {
    return runCli({
...
if (process.argv[1] && path.resolve(process.argv[1]) === path.resolve(fileURLToPath(import.meta.url)))
    process.exitCode = await runRealAgentDogfoodReviewCli();
