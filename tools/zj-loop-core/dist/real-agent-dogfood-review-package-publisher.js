...
export const REAL_AGENT_DOGFOOD_REVIEW_PACKAGE_AGGREGATE_TYPE = 'real-agent-dogfood-review-package';
export const REAL_AGENT_DOGFOOD_REVIEW_PACKAGE_PUBLISHED_SCHEMA = 'zj-loop.real_agent_dogfood_review_package_published.v1';
...
function validateInput(input) {
    ...
    if (!DIGEST.test(input.evidence_digest))
        throw new Error('real-agent-dogfood-review-package-evidence-digest-invalid');
}
export async function publishRealAgentDogfoodReviewPackage(input) {
    validateInput(input);
    ...
    return input.stateStore.runAtomic((transaction) => {
        const currentRevision = () => transaction.database.prepare('SELECT current_revision FROM network_metadata WHERE network_id = ?').get(reviewPackage.network_id)?.current_revision ?? input.expected_revision;
        const existing = transaction.database.prepare("SELECT payload_json FROM state_events WHERE network_id = ? AND aggregate_type = ? AND aggregate_id = ? AND event_type = 'real-agent-dogfood-review-package.published'").get(reviewPackage.network_id, REAL_AGENT_DOGFOOD_REVIEW_PACKAGE_AGGREGATE_TYPE, aggregate_id);
        if (existing) {
            const payload = JSON.parse(existing.payload_json);
            if (payload.package_digest === reviewPackage.package_digest && payload.evidence_digest === input.evidence_digest)
                return { status: 'duplicate', aggregate_id, current_revision: currentRevision() };
