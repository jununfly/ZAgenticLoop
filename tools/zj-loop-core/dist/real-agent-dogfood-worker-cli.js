...
async function runWorkerContext(contextPath) {
    ...
    if (required.some((key) => typeof context[key] !== 'string' || context[key] === '') || !context.binding || !Number.isInteger(context.expected_revision))
    ...
    try {
        ...
        const result = await executeRealAgentDogfoodWorker({ stateStore, evidenceStore, lifecycle, worker_id: context.worker_id, lease_id: context.lease_id, binding: context.binding, worktree_path: context.worktree_path, executable: context.executable, goal: context.goal, provider, post_run_proof: context.post_run_proof, expected_revision: context.expected_revision });
