...
function latest(events) {
    ...
    if (fact.schema !== REAL_AGENT_DOGFOOD_WORKER_LEASE_SCHEMA || fact.network_id !== event.network_id || fact.execution_id !== event.aggregate_id)
...
export async function renewRealAgentDogfoodWorkerLease(input) {
    ...
    const ttl = input.ttl_ms ?? 30_000;
    const current = await readLease(input);
    if (!current || current.fact.lease_id !== input.lease_id || current.fact.worker_id !== input.worker_id || Date.parse(now) >= Date.parse(current.fact.expires_at))
        return { status: 'blocked', reason: current ? 'worker-lease-expired' : 'worker-lease-mismatch' };
