...
function formatRegistryError(source, error) {
...
export function parsePatternRegistry(raw, source = '<inline>') {
    const parsed = source.endsWith('.json') ? JSON.parse(raw) : yaml.parse(raw);
    const result = registrySchema.safeParse(parsed);
    if (!result.success)
        throw formatRegistryError(source, result.error);
    return result.data;
}
export async function loadPatternRegistry(options) {
