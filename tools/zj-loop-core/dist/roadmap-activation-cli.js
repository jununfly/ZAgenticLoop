...
import { buildRoadmapBoundedSlicePack, buildActivationRequestId, buildRoadmapActivationBranchName, buildRoadmapActivationPrContract, buildRoadmapActivationPrTitle, buildRoadmapActivationReviewContract, buildRoadmapActivationReviewTitle, dispatchRoadmapActivationCommand, executeGitLabRoadmapActivation, readActivationComments, renderRoadmapActivationWorkflowSummary, verifyRoadmapBoundedSliceResult, } from './roadmap-activation-runner.js';
...
if (argv[0] === 'activation-plan') {
    ...
    process.exitCode = await runCli({
        ...
        async handler({ io, options }) {
            if (typeof options['contract-plan'] !== 'string')
                throw new Error('--contract-plan is required');
