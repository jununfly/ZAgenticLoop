...
function gitLabAuthHeaders(input) {
    if (input.token)
...
export function verifyRoadmapBoundedSliceResult(input) {
    const errors = [];
    ...
    if (pack.schema !== 'zj-loop.roadmap_bounded_slice_pack.v1')
        errors.push('pack schema must be zj-loop.roadmap_bounded_slice_pack.v1');
    if (result.schema !== 'zj-loop.roadmap_bounded_slice_result.v1')
        errors.push('result schema must be zj-loop.roadmap_bounded_slice_result.v1');
    if (result.activation_request_id !== pack.activation_request_id)
        errors.push('activation_request_id must match pack');
    if (result.branch_name !== pack.branch_name)
        errors.push('branch_name must match pack');
    if (sliceResults.length > maxSlices)
        errors.push('slice_results exceeds max_slices');
    const selectedIds = new Set(selectedSlices.map((slice) => String(slice.slice_id)));
    for (const slice of sliceResults) {
        const sliceId = String(slice.slice_id ?? '');
        if (!selectedIds.has(sliceId))
            errors.push(`slice ${sliceId || '<missing>'} was not selected by pack`);
        ...
        if (!String(slice.notes ?? '').trim())
            errors.push(`slice ${sliceId || '<missing>'} notes are required`);
        ...
        if (slice.status === 'completed') {
            ...
            if (!slice.verification?.some((entry) => String(entry.status) === 'passed' || String(entry.exit_code) === '0')) {
                errors.push(`slice ${sliceId || '<missing>'} must include a passed verification command`);
            }
            if (!String(slice.commit?.intent ?? '').trim())
                errors.push(`slice ${sliceId || '<missing>'} commit intent is required`);
...
function normalizeMaxSlices(value) {
    const parsed = Number(value ?? DEFAULT_BOUNDED_SLICE_MAX_SLICES);
    if (!Number.isInteger(parsed) || parsed < 1)
        return DEFAULT_BOUNDED_SLICE_MAX_SLICES;
    return parsed;
}
function isEligibleLeafSlice(slice) {
...
export function hasRoadmapActivationLoopMarker(input) {
...
function allowedLifecycleTransitions(state) {
    if (state === 'requested')
        return ['consumed', 'blocked', 'failed'];
    if (state === 'consumed')
        return ['running', 'blocked', 'failed'];
    if (state === 'running')
        return ['blocked', 'failed', 'completed'];
    if (state === 'blocked')
...
function defaultRoadmapActivationNextSteps(input) {
