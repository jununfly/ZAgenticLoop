...
export async function inspectGitLabScheduleHealth(input) {
    ...
    if (capability.status !== 'ready')
        return result({ schema: SCHEDULE_HEALTH_SCHEMA, target: input.target, audit }, 'configuration_missing', 'gitlab-capability-unverified');
    let schedule;
    let pipelines;
    try {
    ...
    const enrichedPipeline = { ...pipeline, job: job.name, artifact: input.target.artifact, artifact_schema: artifact.schema, artifact_payload: artifact.payload };
...
function infraError(error) {
    if (error instanceof GitLabInfraError)
