...
export async function createGitLabOwnedSchedule(input) {
    ...
    if (!verified.ok)
        return { ...created, status: 'escalated', reason: 'owned-schedule-read-failed' };
...
export async function readGitLabOwnedSchedulePipeline(input) {
    const fetchImpl = input.fetchImpl ?? globalThis.fetch;
    const apiUrl = String(input.apiUrl ?? 'https://gitlab.com/api/v4').replace(/\/$/, '');
    const project = encodeURIComponent(input.plan.project);
    const headers = { 'PRIVATE-TOKEN': input.token };
    ...
    if (!scheduleResponse.ok)
        throw new Error('gitlab-owned-schedule-read-failed');
    ...
    if (!response.ok)
        throw new Error('gitlab-owned-schedule-pipeline-read-failed');
    const pipeline = await response.json();
    if (pipeline.source !== 'schedule')
        return { status: 'invalid', reason: 'owned-schedule-pipeline-source-mismatch' };
    if (pipeline.ref !== input.plan.temporary_schedule.ref)
        return { status: 'invalid', reason: 'owned-schedule-pipeline-ref-mismatch' };
...
export async function readGitLabScheduleProbeReceipt(input) {
    const fetchImpl = input.fetchImpl ?? globalThis.fetch;
    const apiUrl = String(input.apiUrl ?? 'https://gitlab.com/api/v4').replace(/\/$/, '');
    const project = encodeURIComponent(input.plan.project);
    const headers = { 'PRIVATE-TOKEN': input.token };
    ...
    if (!jobsResponse.ok)
        throw new Error('gitlab-schedule-probe-receipt-job-read-failed');
    const job = (await jobsResponse.json()).find((candidate) => candidate.name === 'zj_loop_schedule_probe_receipt');
    if (!job || !['success', 'failed', 'canceled'].includes(job.status))
        return { status: 'missing' };
    if (job.status !== 'success')
        return { status: 'invalid', reason: `schedule-probe-receipt-job-${job.status}` };
    ...
    let receipt;
    try {
        receipt = JSON.parse(await artifactResponse.text());
    }
    catch {
        return { status: 'invalid', reason: 'schedule-probe-receipt-json-invalid' };
    }
    if (receipt.schema !== 'zj-loop.gitlab_schedule_probe_receipt.v1')
        return { status: 'invalid', reason: 'schedule-probe-receipt-schema-mismatch' };
    if (receipt.probe_id !== input.plan.probe_id)
        return { status: 'invalid', reason: 'schedule-probe-receipt-probe-id-mismatch' };
    if (String(receipt.pipeline_id) !== String(input.pipeline.id))
        return { status: 'invalid', reason: 'schedule-probe-receipt-pipeline-id-mismatch' };
    if (receipt.project !== input.plan.project)
        return { status: 'invalid', reason: 'schedule-probe-receipt-project-mismatch' };
    if (receipt.ref !== input.plan.temporary_schedule.ref)
        return { status: 'invalid', reason: 'schedule-probe-receipt-ref-mismatch' };
    if (receipt.source !== 'schedule')
        return { status: 'invalid', reason: 'schedule-probe-receipt-source-mismatch' };
    ...
    while (now().getTime() < new Date(state.deadline_at).getTime()) {
        ...
        try {
            ...
            if (observed.status === 'invalid')
                terminalReason = observed.reason;
            if (pipeline) {
                const observedReceipt = await readGitLabScheduleProbeReceipt({ ...input, plan: state, pipeline });
                if (observedReceipt.status === 'found')
                    receipt = observedReceipt.receipt;
                if (observedReceipt.status === 'invalid')
                    terminalReason = observedReceipt.reason;
            }
        }
        catch {
            pollErrors += 1;
