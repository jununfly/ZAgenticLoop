...
import { createProviderOutcome } from './provider-outcome.js';
export const SIMULATED_PROVIDER_RESULT_SCHEMA = 'zj-loop.simulated_provider_result.v1';
function digest(value) {
    const json = canonicalize(value);
    if (typeof json !== 'string')
        throw new Error('simulated-provider-canonicalization-invalid');
    return `sha256:${createHash('sha256').update(json).digest('hex')}`;
}
function text(value) { return typeof value === 'string' && value.length > 0; }
function scope(value) { return Array.isArray(value) && value.every(text); }
function requestScope(input) { return `${input.network_id}:${input.event_id}:${input.plan_id}:${input.plan_revision}:${input.execution_id}:${input.task_id}:${input.provider_request_id}`; }
export function createSimulatedProvider(input) {
    if (!text(input.provider_id) || !text(input.namespace))
        throw new Error('simulated-provider-identity-invalid');
    const requests = new Map();
    const fixtureState = new Map();
    const stateDigest = () => digest([...fixtureState.entries()].sort(([left], [right]) => left.localeCompare(right)));
    const blocked = (reason) => ({ schema: SIMULATED_PROVIDER_RESULT_SCHEMA, status: 'blocked', provider_kind: 'simulated', provider_id: input.provider_id, fixture_state_digest: stateDigest(), virtual_side_effects: [], real_provider_calls: 0, side_effects_executed: false, reason });
    return {
        async execute(request) {
            if (!text(request.network_id) || !text(request.event_id) || !text(request.plan_id) || !Number.isInteger(request.plan_revision) || request.plan_revision < 1 || !text(request.execution_id) || !text(request.task_id) || !text(request.provider_request_id) || !text(request.request_digest) || !scope(request.resource_scope) || !text(request.observed_at))
                return blocked('request-invalid');
            const key = requestScope(request);
            const existing = requests.get(key);
            if (existing) {
                if (existing.request_digest !== request.request_digest)
                    return blocked('provider-request-conflict');
            ...
            if (virtualSideEffects.length > 0)
                fixtureState.set(key, virtualSideEffects);
