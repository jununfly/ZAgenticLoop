    ...
    return {
        async issueIntent(request) {
            ...
            return transaction(db, () => {
                const existing = db.prepare('SELECT request_id, issuance_digest, credential_id, intent_expires_at FROM credential_issue_intents WHERE request_id = ?').get(request.request_id);
                if (existing) {
