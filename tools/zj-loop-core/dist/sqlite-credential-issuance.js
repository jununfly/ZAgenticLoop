...
export function parseHumanApprovalEnvelope(context) {
    ...
    if (!value || typeof value !== 'object' || Array.isArray(value) || !('approval' in value) || !('human_identity' in value))
        throw new Error('approval-context-invalid');
    const envelope = value;
    if (!envelope.approval || typeof envelope.approval !== 'object' || !envelope.human_identity || typeof envelope.human_identity !== 'object')
        throw new Error('approval-context-invalid');
    return { approval: envelope.approval, human_identity: envelope.human_identity };
}
export function createCredentialIssueIntentService(input) {
    ...
    return {
        async issueIntent(request) {
            ...
            if (envelope.approval.human_id !== request.human_id || envelope.human_identity.human_id !== request.human_id)
                throw new Error('approval-context-invalid');
            const value = request.request;
            if (typeof value.request_id !== 'string' || typeof value.node_id !== 'string' || typeof value.event_id !== 'string' || typeof value.task_id !== 'string' || typeof value.issued_at !== 'string' || typeof value.expires_at !== 'string' || !Array.isArray(value.capabilities) || !value.capabilities.every((capability) => typeof capability === 'string'))
                throw new Error('credential-issue-intent-invalid');
            return input.issuance.issueIntent({ request_id: value.request_id, network_id: request.network_id, node_id: value.node_id, event_id: value.event_id, task_id: value.task_id, capabilities: value.capabilities, issued_at: value.issued_at, expires_at: value.expires_at, approval: envelope.approval, human_identity: envelope.human_identity, expected_revision: request.expected_revision });
        },
    };
}
...
function parseTime(value, error) {
...
export function credentialIssuanceDigest(input) {
...
export function createSqliteCredentialIssuance(input) {
    ...
    const atomic = (work) => {
        if (input.stateStore)
            return input.stateStore.runAtomic(({ database, appendEvent }) => { bootstrapTables(database); return work(database, appendEvent); });
        if (!db)
    ...
    return {
        async issueIntent(request) {
            for (const [value, error] of [[request.request_id, 'request-id-required'], [request.network_id, 'network-id-required'], [request.node_id, 'node-id-required'], [request.event_id, 'event-id-required'], [request.task_id, 'task-id-required']])
            ...
            if (verifyHumanApprovalContextDetailed({ identity: request.human_identity, context: request.approval, now: current, require_v2: true }).status !== 'current-v2-accepted')
                throw new Error('approval-context-invalid');
            if (request.approval.action !== 'credential.issue' || request.approval.request_id !== request.request_id)
                throw new Error('approval-context-mismatch');
            if (JSON.stringify([...new Set(request.approval.approved_capabilities)].sort()) !== JSON.stringify(capabilities.sort()))
                throw new Error('approval-capability-mismatch');
            const issuanceDigest = credentialIssuanceDigest(request);
            if (request.approval.request_digest !== issuanceDigest)
                throw new Error('approval-context-mismatch');
            ...
            if (input.stateStore && (!Number.isInteger(request.expected_revision) || request.expected_revision < 1))
                throw new Error('expected-revision-invalid');
            return atomic((database, appendEvent) => {
                ...
                if (appendEvent) {
                    ...
                    const result = appendEvent({ network_id: request.network_id, expected_revision: request.expected_revision, event, now: current });
                    if (result.status === 'conflict')
                        throw new Error(result.reason ?? 'event-conflict');
                }
        ...
        async issuePairingIntent(request) {
            for (const [value, error] of [[request.request_id, 'request-id-required'], [request.network_id, 'network-id-required'], [request.node_id, 'node-id-required'], [request.request_digest, 'request-digest-required'], [request.human_id, 'human-id-required']])
                requireText(value, error);
            const issuedAt = parseTime(request.issued_at, 'credential-issued-time-invalid');
            const expiresAt = parseTime(request.expires_at, 'credential-expiry-invalid');
            if (issuedAt >= expiresAt)
            ...
            return atomic((database, appendEvent) => {
                ...
                if (appendEvent) {
                    const event = createCredentialIssueIntentEvent({ request_id: request.request_id, network_id: request.network_id, node_id: request.node_id, credential_id: credentialId, issuance_digest: issuanceDigest, capabilities, issued_at: request.issued_at, expires_at: request.expires_at, intent_expires_at: intentExpiresAt });
                    const currentRevision = database.prepare('SELECT current_revision FROM network_metadata WHERE network_id = ?').get(request.network_id).current_revision;
        ...
        async claim(request) {
            for (const [value, error] of [[request.request_id, 'request-id-required'], [request.network_id, 'network-id-required'], [request.node_id, 'node-id-required'], [request.credential_id, 'credential-id-required']])
                requireText(value, error);
            const current = request.now ?? now();
            const currentTime = parseTime(current, 'credential-clock-invalid');
            return atomic((database, appendEvent) => {
                const row = database.prepare('SELECT request_id, credential_id, network_id, node_id, intent_expires_at, expires_at, claimed_at, token_hash, revoked_at FROM credential_issue_intents WHERE request_id = ?').get(request.request_id);
        ...
        async claimForPairingSession(request) {
            for (const [value, error] of [[request.request_id, 'request-id-required'], [request.network_id, 'network-id-required'], [request.node_id, 'node-id-required'], [request.session_id, 'pairing-session-id-required']])
                requireText(value, error);
            const row = await atomic((database) => database.prepare('SELECT credential_id FROM credential_issue_intents WHERE request_id = ? AND network_id = ? AND node_id = ?').get(request.request_id, request.network_id, request.node_id));
            if (!row)
                throw new Error('credential-not-available');
        ...
        async revoke(request) {
            ...
            return atomic((database, appendEvent) => {
                const row = database.prepare('SELECT revoked_at FROM credential_issue_intents WHERE credential_id = ?').get(request.credential_id);
                if (!row)
                ...
                if (appendEvent) {
                    const credential = database.prepare('SELECT request_id, network_id FROM credential_issue_intents WHERE credential_id = ?').get(request.credential_id);
                    const currentRevision = database.prepare('SELECT current_revision FROM network_metadata WHERE network_id = ?').get(credential.network_id).current_revision;
