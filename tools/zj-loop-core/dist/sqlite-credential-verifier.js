...
export const SQLITE_CREDENTIAL_STORE_SCHEMA = 'zj-loop.sqlite_credential_store.v1';
...
function bootstrap(db) {
  ...
  `);
    const version = db.prepare('SELECT schema_version FROM credential_store_metadata WHERE id = 1').get();
    if (!version || version.schema_version !== 1)
        throw new Error('credential-schema-version-unsupported');
}
export function createSqliteCredentialVerifier(input) {
    ...
    return {
        async issueCredential(request) {
            ...
            const existing = db.prepare('SELECT credential_id FROM credential_metadata WHERE credential_id = ?').get(credential.credential_id);
            if (existing)
                return { status: 'duplicate', credential_id: credential.credential_id };
        ...
        async revokeCredential(request) {
            ...
            const existing = db.prepare('SELECT revoked_at FROM credential_metadata WHERE credential_id = ?').get(request.credential_id);
            if (!existing)
        ...
        async verify(request) {
            ...
            if (current < parseTime(row.issued_at, 'credential-issued-time-invalid'))
                return { status: 'blocked', reason: 'credential-not-yet-valid' };
            if (current > parseTime(row.expires_at, 'credential-expiry-invalid'))
            ...
            if (request.network_id !== row.network_id)
                return { status: 'blocked', reason: 'credential-network-mismatch' };
            if (request.event_id !== undefined && request.event_id !== row.event_id)
                return { status: 'blocked', reason: 'credential-event-mismatch' };
            if (request.task_id !== undefined && request.task_id !== row.task_id)
                return { status: 'blocked', reason: 'credential-task-mismatch' };
