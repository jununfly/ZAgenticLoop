...
function requiredCredential(value, error) {
    if (!(typeof value === 'string' || Buffer.isBuffer(value)) || value.length === 0)
        throw new Error(error);
}
function pathSegment(value, error) {
...
function parseEndpoint(value) {
    ...
    try {
        ...
        throw new Error('transport-endpoint-invalid');
    }
    if (endpoint.protocol !== 'https:')
        throw new Error('transport-endpoint-https-required');
    return endpoint;
}
function bodyObject(value, error) {
...
function result(value) {
    ...
    if (body.side_effects_executed !== false || !['accepted', 'duplicate', 'blocked'].includes(String(body.status)))
        throw new Error('transport-response-invalid');
    ...
    if (!/^sha256:[0-9a-f]{64}$/.test(body.envelope_digest))
        throw new Error('transport-response-digest-invalid');
    const status = body.status;
    return { status, message_id: body.message_id, envelope_digest: body.envelope_digest, side_effects_executed: false };
}
export function createTlsTransportAdapter(input) {
            ...
            const req = request(options, (response) => {
                ...
                response.on('end', () => {
                    ...
                    try {
                        ...
                        reject(new Error('transport-response-json-invalid'));
                    }
                });
            });
    ...
    return {
        ...
        async send(session) {
            ...
            if (![200, 201, 202, 409].includes(response.statusCode))
                blocked(response);
