import { createRelaySession } from './relay-contract.js';
export const TRANSPORT_REPLAY_CONFORMANCE_SCHEMA = 'zj-loop.transport_replay_conformance.v1';
function requireText(value, error) {
    if (!value.trim())
        throw new Error(error);
}
export async function runTransportReplayConformance(input) {
    ...
    const delivery = (id) => ({ delivery_id: `${input.scenario_id}-${id}`, attempt_id: '', network_id: input.network_id, event_id: `${input.scenario_id}-${id}-event`, task_id: `${input.scenario_id}-${id}-task`, target_node_id: session.node_id, state: 'offered', retry_count: 0 });
    const first = delivery('first');
    const second = delivery('second');
    const record = (name, condition, reason) => assertions.push({ name, status: condition ? 'passed' : 'blocked', ...(condition || !reason ? {} : { reason }) });
    ...
    return { schema: TRANSPORT_REPLAY_CONFORMANCE_SCHEMA, scenario_id: input.scenario_id, status: complete ? 'passed' : 'blocked', assertions, final_session: finalSession ?? session, final_deliveries: [finalFirst, finalSecond].filter((value) => value !== null), side_effects_executed: false };
}
