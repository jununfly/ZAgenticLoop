...
import { getTriageRoleMapping, TRIAGE_CATEGORY_ROLES, TRIAGE_STATE_ROLES, validateTriageRoleMapping, } from './triage-role-mapping.js';
...
if (!role || labels.length === 0)
    throw new Error('--role and --labels are required for plan/apply');
const [group, roleName] = role.split('.');
if ((group !== 'category' && group !== 'state') || ![...TRIAGE_CATEGORY_ROLES, ...TRIAGE_STATE_ROLES].includes(roleName)) {
...
const result = {
    ...
    provider,
    role,
    before: current[group][roleName],
    after: labels,
    valid: proposedValidation.ok,
    errors: proposedValidation.errors,
    changed: JSON.stringify(current[group][roleName]) !== JSON.stringify(labels),
...
if (value('--confirm') !== APPLY_CONFIRMATION)
    throw new Error(`confirmation required: --confirm ${APPLY_CONFIRMATION}`);
...
function patchExistingMapping(text, providerName, groupName, roleKey, nextLabels) {
    const document = YAML.parseDocument(text);
    const node = document.getIn(['policy', 'triage_role_mapping', providerName, groupName, roleKey], true);
    if (!node?.range || node.range.length < 2)
        throw new Error('mapping target range unavailable; refusing non-local rewrite');
    return `${text.slice(0, node.range[0])}${JSON.stringify(nextLabels)}${text.slice(node.range[1])}`;
}
function insertDefaultMapping(text, providerName, mapping) {
    const marker = /\n(?=routes:\s*$)/m;
    const block = `\n  triage_role_mapping:\n    ${providerName}:\n${renderRoleGroup('category', mapping.category)}${renderRoleGroup('state', mapping.state)}\n`;
    if (!marker.test(text))
        throw new Error('Route Table routes section not found; refusing non-local rewrite');
    return text.replace(marker, `${block}\n`);
}
function renderRoleGroup(group, values) {
