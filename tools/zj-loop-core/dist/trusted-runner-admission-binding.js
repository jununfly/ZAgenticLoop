...
export function createAdmissionBoundLocalExecutionPreflight(input) {
    ...
    if (input.preflight.execution_id !== binding.provider_auth_ref.execution_id || input.preflight.attempt !== binding.provider_auth_ref.attempt || input.preflight.provider_id !== binding.provider_auth_ref.provider_id || binding.provider_auth_ref.network_id !== binding.network_id)
        throw new Error('trusted-runner-admission-binding-provider-auth-ref-mismatch');
...
export function createAdmissionBoundTrustedRunnerExecutionContext(input) {
    const binding = checkedBinding(input.binding);
    if (input.execution.execution_id && (input.execution.execution_id !== binding.provider_auth_ref.execution_id || input.execution.attempt !== binding.provider_auth_ref.attempt))
        throw new Error('trusted-runner-admission-binding-provider-auth-ref-mismatch');
