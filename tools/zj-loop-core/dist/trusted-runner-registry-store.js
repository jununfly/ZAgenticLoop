...
function canonicalDigest(value) {
    ...
    if (typeof json !== 'string')
        throw new Error('trusted-runner-registry-snapshot-canonicalization-invalid');
    return `sha256:${createHash('sha256').update(json, 'utf8').digest('hex')}`;
}
export function trustedRunnerRegistrySnapshotDigest(registry) { return canonicalDigest(registry); }
async function emptyRegistryRead(stateStore, network_id) {
...
export async function recordTrustedRunnerRegistryMutation(input) {
    if (input.mutation.network_id === '')
        throw new Error('trusted-runner-registry-network-required');
    let authority;
    try {
    ...
    if (previous && previous.canonical_payload_digest === input.mutation.canonical_payload_digest)
    ...
    if (current.snapshot.revision !== input.expected_revision)
        return { ...current, status: 'conflict', reason: 'revision-mismatch' };
    ...
    if (applied.status === 'blocked')
        return { ...current, status: 'blocked', reason: applied.reason };
    ...
    if (append.status === 'conflict')
        return { ...current, status: 'conflict', revision: append.current_revision, reason: append.reason };
    const snapshot = { network_id: input.mutation.network_id, revision: append.revision, digest: trustedRunnerRegistrySnapshotDigest(applied.registry), registry: applied.registry };
