...
function actionShape(value) {
    if (!['register', 'rotate', 'revoke'].includes(value.action))
        return false;
...
export function applyTrustedRunnerRegistryMutation(input) {
    ...
    if (input.mutation.action === 'register') {
        ...
        registry.push({ runner_id: input.mutation.runner_id, public_key_fingerprint: input.mutation.new_public_key_fingerprint, status: 'active' });
        return { status: 'recorded', registry };
    }
    if (!current || current.status !== 'active' || current.public_key_fingerprint !== input.mutation.old_public_key_fingerprint)
