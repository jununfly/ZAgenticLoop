...
export const TRUSTED_RUNNER_PROTOCOL_SCHEMA = 'zj-loop.trusted_runner_protocol.v1';
export const TRUSTED_RUNNER_PROOF_SCHEMA = 'zj-loop.trusted_runner_proof.v1';
export const TRUSTED_RUNNER_OBSERVATION_SCHEMA = 'zj-loop.trusted_runner_observation.v1';
const DIGEST = /^sha256:[0-9a-f]{64}$/;
function digest(value) { return typeof value === 'string' && DIGEST.test(value); }
function canonicalDigest(value, error) {
    const json = canonicalize(value);
    if (typeof json !== 'string')
        throw new Error(error);
    return `sha256:${createHash('sha256').update(json, 'utf8').digest('hex')}`;
}
function bytesDigest(value) { return `sha256:${createHash('sha256').update(value, 'utf8').digest('hex')}`; }
...
export function createFakeTrustedRunner(input) {
    ...
    return {
        async prepareExecution({ execution }) {
            if (execution.runner_id !== input.runner_id || !execution.execution_id || !Number.isInteger(execution.registry_revision) || execution.registry_revision < 1 || execution.attempt < 1 || !digest(execution.preflight_digest) || !digest(execution.registry_snapshot_digest) || !digest(execution.capabilities_digest) || !digest(execution.helper.executable_digest))
                throw new Error('trusted-runner-execution-context-invalid');
            const issuedAt = now();
            const expiresAt = new Date(Date.parse(issuedAt) + (input.expires_in_ms ?? 300_000)).toISOString();
            const unsigned = { schema: TRUSTED_RUNNER_PROOF_SCHEMA, status: 'signed', runner_id: execution.runner_id, runner_version: runnerVersion, registry_revision: execution.registry_revision, execution_id: execution.execution_id, attempt: execution.attempt, preflight_digest: execution.preflight_digest, registry_snapshot_digest: execution.registry_snapshot_digest, capabilities_digest: execution.capabilities_digest, helper_digest: execution.helper.executable_digest, issued_at: issuedAt, expires_at: expiresAt };
        ...
        async observe({ execution, proof, launch, output }) {
            if (proof.runner_id !== execution.runner_id || proof.registry_revision !== execution.registry_revision || proof.registry_snapshot_digest !== execution.registry_snapshot_digest || proof.capabilities_digest !== execution.capabilities_digest)
                throw new Error('trusted-runner-proof-binding-invalid');
            const unsigned = { schema: TRUSTED_RUNNER_OBSERVATION_SCHEMA, status: 'signed', runner_id: execution.runner_id, registry_revision: execution.registry_revision, execution_id: execution.execution_id, attempt: execution.attempt, preflight_digest: execution.preflight_digest, proof_digest: proof.proof_digest, registry_snapshot_digest: execution.registry_snapshot_digest, capabilities_digest: execution.capabilities_digest, stdout_digest: bytesDigest(output.stdout), stderr_digest: bytesDigest(output.stderr), stdout_bytes: Buffer.byteLength(output.stdout, 'utf8'), stderr_bytes: Buffer.byteLength(output.stderr, 'utf8'), output_truncated: false, process_boundary: { ...launch.process_boundary } };
        ...
        verifyBoundary(observation) {
            ...
            if (observation.status !== 'signed' || !validSignature(trustedRunnerObservationDigest(unsigned), signature))
                return { status: 'blocked', reason: 'trusted-runner-observation-invalid' };
            const value = observation.process_boundary;
            if (value.all_descendants_terminated && !value.orphan_processes_detected && !value.unknown_descendants_detected)
