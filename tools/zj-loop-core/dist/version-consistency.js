...
export function generatedFileHash(text) {
...
function extractTemplateHash(text) {
    return text.match(/^# zj-loop-template-hash: (?<hash>[a-f0-9]{16})$/m)?.groups?.hash ?? null;
}
function asLock(value) {
    if (!value || typeof value !== 'object')
        throw new Error('version lock must be an object');
    const lock = value;
    if (lock.schema !== 'zj-loop.version-lock.v1')
        throw new Error('unsupported version lock schema');
    if (!lock.core?.package || !lock.core.version || !lock.core.source)
        throw new Error('version lock core metadata is incomplete');
    if (!lock.generated_files || typeof lock.generated_files !== 'object')
        throw new Error('version lock generated_files is required');
    return lock;
}
async function readPackageVersion() {
    ...
    if (!packageJson.version)
        throw new Error('core package version is missing');
...
export async function checkVersionConsistency(input = {}) {
    ...
    if (lock && packageVersion === expectedVersion)
        checks.push({ name: 'core-package-version', status: 'passed', expected: expectedVersion, observed: packageVersion });
    else {
    ...
    if (lock) {
        for (const [relativePath, expectedFile] of Object.entries(lock.generated_files)) {
            ...
            try {
                ...
                if (!fileOk || !templateOk) {
                    ...
                    checks.push({ name: 'generated-file', status: 'passed', expected: expectedFile.sha256, observed: observedHash, path: relativePath });
            }
            catch {
        ...
        if (!sourceOk) {
            ...
            checks.push({ name: 'core-package-reference', status: 'passed', expected: lock.core.source, observed: references.join(', ') });
        if (lock.vendor) {
            const vendorPath = path.join(root, lock.vendor.path);
            try {
                const observed = await sha256(vendorPath);
                if (observed !== lock.vendor.sha256)
                    throw new Error(`vendor SHA256 mismatch: expected ${lock.vendor.sha256}, observed ${observed}`);
