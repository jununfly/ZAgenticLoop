...
process.exitCode = await runCli({
  ...
  async handler({ io, options }) {
    ...
    if (options.execute === true) {
      if (typeof options.root !== 'string' || options.root.trim() === '') throw new Error('--root is required with --execute');
    ...
    if (typeof options.root === 'string' && options.root.trim() !== '') {
      ...
      if (typeof options.ref === 'string' && options.ref.trim() !== '') {
        ...
        if (resolvedRef.toLowerCase() !== report.target.commit.toLowerCase()) throw new Error('target commit mismatch');
      }
      for (const [key, evidence] of Object.entries(report.evidence_refs)) {
        if (!evidence.hash) continue;
        ...
        if (actual !== evidence.hash.toLowerCase()) throw new Error(`evidence hash mismatch: ${key}`);
      }
      for (const [index, manifest] of report.verification_manifest.entries()) {
        if (!manifest.output_path || !manifest.output_sha256) continue;
        ...
        const data = typeof options.ref === 'string' && options.ref.trim() !== ''
          ...
          : await readFile(target);
        const actual = createHash('sha256').update(data).digest('hex');
        if (actual !== manifest.output_sha256.toLowerCase()) throw new Error(`manifest output hash mismatch: ${index}`);
      }
    }
...
async function readPinnedFile(root: string, ref: string, relativePath: string, label: string): Promise<Buffer> {
  if (path.isAbsolute(relativePath) || relativePath.split(/[\\/]/).includes('..')) throw new Error(`evidence path escapes root: ${label}`);
  try {
...
async function verifyPathWithinRoot(canonicalRoot: string, root: string, relativePath: string, label: string): Promise<string> {
  ...
  if (canonicalTarget !== canonicalRoot && !canonicalTarget.startsWith(`${canonicalRoot}${path.sep}`)) throw new Error(`evidence path escapes root: ${label}`);
  return canonicalTarget;
}

async function executeVerificationManifest(input: any, root: string): Promise<void> {
  const now = new Date().toISOString();
  for (const [index, manifest] of (input.verification_manifest ?? []).entries()) {
    if (typeof manifest.command !== 'string' || typeof manifest.output_path !== 'string') throw new Error(`manifest entry ${index} requires command and output_path`);
    ...
    const result = (input.verification_results ?? []).find((item: any) => item.command === manifest.command);
    if (result) {
...
async function runAllowlistedVerification(root: string, command: string): Promise<string> {
...
async function resolveNpmProjectRoot(root: string): Promise<string> {
  try {
    const packageJson = JSON.parse(await readFile(path.join(root, 'package.json'), 'utf8'));
    if (packageJson?.scripts?.['test:agent-local']) return root;
  } catch {
  ...
  try {
    ...
    if (packageJson?.scripts?.['test:agent-local']) return coreRoot;
  } catch {
...
async function prepareOutputPath(root: string, relativePath: string, label: string): Promise<string> {
  if (path.isAbsolute(relativePath) || relativePath.split(/[\\/]/).includes('..')) throw new Error(`manifest output path escapes root: ${label}`);
  const target = path.resolve(root, relativePath);
  if (target !== root && !target.startsWith(`${root}${path.sep}`)) throw new Error(`manifest output path escapes root: ${label}`);
