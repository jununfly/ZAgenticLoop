...
export const AGENT_CONTEXT_CAPABILITY_REVIEW_SCHEMA =
  'zj-loop.agent_context_capability_review.v1' as const;
...
export const agentContextCapabilityReviewSchema = z.object({
  ...
  for (const [index, item] of value.failure_or_blocked_cases.entries()) {
    for (const key of item.evidence_refs) {
      if (!evidenceKeys.has(key)) context.addIssue({ code: z.ZodIssueCode.custom, path: ['failure_or_blocked_cases', index, 'evidence_refs'], message: `unknown evidence ref: ${key}` });
    }
  }
  for (const [index, item] of value.verification_results.entries()) {
    for (const key of item.evidence_refs) {
      if (!evidenceKeys.has(key)) context.addIssue({ code: z.ZodIssueCode.custom, path: ['verification_results', index, 'evidence_refs'], message: `unknown evidence ref: ${key}` });
    }
  }
  ...
  for (const [index, result] of value.verification_results.entries()) {
    if (!manifestCommands.has(result.command)) context.addIssue({ code: z.ZodIssueCode.custom, path: ['verification_results', index, 'command'], message: 'verification result has no manifest entry' });
  }
  for (const [index, manifest] of value.verification_manifest.entries()) {
    ...
    if (manifest.status !== result.status) context.addIssue({ code: z.ZodIssueCode.custom, path: ['verification_manifest', index, 'status'], message: 'manifest status must match verification result' });
...
export type AgentContextCapabilityReview = z.infer<
  typeof agentContextCapabilityReviewSchema
>;

export type AgentContextCapabilityReviewInput = Omit<
  AgentContextCapabilityReview,
  'schema'
>;

export function buildAgentContextCapabilityReview(
  input: AgentContextCapabilityReviewInput,
): AgentContextCapabilityReview {
...
export function parseAgentContextCapabilityReview(
  value: unknown,
): AgentContextCapabilityReview {
  return agentContextCapabilityReviewSchema.parse(value);
}

export function validateAgentContextCapabilityReview(value: unknown) {
