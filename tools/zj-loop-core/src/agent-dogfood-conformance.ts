...
export type AgentDogfoodFixture = {
  ...
  environment: {
    ...
    network_policy: { mode: 'network-denied' | 'network-allowed'; policy_digest: string; status: 'proved' | 'blocked'; evidence_digest: string };
    ...
    trusted_runner: {
      ...
      network_policy: { mode: 'network-denied' | 'network-allowed'; policy_digest: string; evidence_digest: string };
...
export function evaluateAgentDogfoodConformance(input: AgentDogfoodFixture): AgentDogfoodConformanceReport {
  ...
  if (!['network-denied', 'network-allowed'].includes(input.environment.network_policy.mode) || input.environment.network_policy.status !== 'proved' || !digest(input.environment.network_policy.policy_digest) || !digest(input.environment.network_policy.evidence_digest)) reasons.push('network-policy-proof-missing');
  ...
  if (!observation.after_worktree_clean || !observation.after_network_policy_proved || !observation.after_credentials_clean || observation.side_effects_detected) reasons.push('post-run-safety-observation-invalid');
...
export function createAgentDogfoodFixture(mode: 'network-denied' | 'network-allowed' = 'network-denied'): AgentDogfoodFixture {
