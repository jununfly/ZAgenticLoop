...
export type AgentExecutionRequest = {
  ...
  request_id?: string;
  registration: { ref: string; path: string; sha256: string };
};
...
export function parseAgentExecutionRequest(note: string, marker: string): AgentExecutionRequest | null {
  if (!note.includes(marker)) return null;
  const match = note.match(/<!--\s*zj-loop\.agent_execution_request\.v1\s*([\s\S]*?)-->/);
  if (!match) return null;
  let value: unknown;
  try { value = JSON.parse(match[1].trim()); } catch { throw new Error("agent-execution-request-json-invalid"); }
  const item = value as Partial<AgentExecutionRequest> | null;
...
export function buildAgentLocalHandoff(input: {
  ...
  if (input.request.registration.ref !== input.registrationCommit) throw new Error("registration-commit-mismatch");
  const digest = createHash("sha256").update(input.registrationText).digest("hex");
  if (digest !== input.request.registration.sha256) throw new Error("registration-sha256-mismatch");
  const registration = yaml.parse(input.registrationText) as Registration;
  const route = registration?.routes?.find((candidate) => candidate.route_id === input.envelope.target_route);
  if (registration?.schema !== "zj-loop.project-registration.v1" || registration.project_path !== input.envelope.project_path || registration.default_branch !== "master" || !route || typeof route.marker !== "string" || route.marker !== `/zj-loop start ${input.envelope.target_route}`) throw new Error("registration-invalid");
  const executor = route.allowed_executors?.find((candidate) => candidate.kind === "agent-local" && candidate.profile === "human-codex-mac");
  if (!executor || !Array.isArray(executor.capabilities) || executor.capabilities.length === 0) throw new Error("agent-local-executor-not-allowlisted");
...
export async function persistAgentLocalHandoff(input: { client: StateBranchClient; handoff: AgentHandoff }): Promise<AgentLocalBridgeResult> {
