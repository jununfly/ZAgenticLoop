...
process.exitCode = await runCli(
  {
    ...
    async handler({ io, options }) {
      ...
      if (command === "list") {
        ...
        if (!value || typeof value !== "object") throw new Error("handoff-not-found");
        ...
        const handoff = (claim && typeof claim === "object" ? { ...value, status: "claimed", claim } : value) as AgentHandoff;
