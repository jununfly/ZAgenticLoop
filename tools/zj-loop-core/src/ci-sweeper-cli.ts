...
import {
  ...
  executeGitLabCiSweeperRepairMr,
  triggerGitLabCiSweeperConsumerPipeline,
} from './ci-sweeper-runner.js';
...
if (argv[0] === 'repair-plan') {
  ...
  process.exitCode = await runCli({
    ...
    async handler({ io, options }) {
      if (typeof options['route-decision'] !== 'string') throw new Error('--route-decision is required');
      if (typeof options.repo !== 'string') throw new Error('--repo is required');
  ...
  process.exitCode = await runCli({
    ...
    async handler({ io, options }) {
      if (typeof options['request-body'] !== 'string') throw new Error('--request-body is required');
      if (typeof options.project !== 'string') throw new Error('--project is required');
      if (typeof options.title !== 'string') throw new Error('--title is required');
...
async function findCiSweeperRoute(root: string) {
  const table = await loadRouteTable(root);
  const routes = [...(table.routes ?? []), ...(table.disabled_dispatch_routes ?? [])];
  const route = routes.find((candidate: any) => candidate.route_id === 'ci-sweeper' || candidate.consumer === 'ci-sweeper');
  if (!route) throw new Error('ci-sweeper-route-missing');
  return route;
}
