...
export async function createGitLabCiSweeperIssueFixRequest(input: {
  ...
  if (!input.projectPath.trim()) return blocked('project-path-required');
  if (!input.title.trim()) return blocked('issue-title-required');
  ...
  const findExisting = async () => {
    ...
    if (!response.ok) return { response, issue: null };
  ...
  if (!existing.response.ok) return blocked('issue-fix-request-dedupe-read-failed', { http_status: existing.response.status });
  if (existing.issue) return completedGitLabIssueFixRequest({ audit, outcome: 'duplicate', issue: existing.issue, input });
  ...
  try {
    ...
    if (!response.ok) return blocked('issue-fix-request-create-failed', { http_status: response.status });
    const issue = await response.json() as any;
    if (!Number.isInteger(Number(issue.iid))) return blocked('issue-fix-request-create-response-invalid');
...
export async function claimGitLabCiSweeperIssueFixRequest(input: {
  ...
  if (!input.token) return blocked('gitlab-token-required');
  if (!Number.isInteger(Number(input.issueIid)) || Number(input.issueIid) <= 0) return blocked('issue-iid-required');
  if (!input.requestId.trim() || !input.claimId.trim() || !input.sourcePipelineId.trim()) return blocked('claim-fields-required');
  if (!fetchImpl) return blocked('gitlab-fetch-unavailable');
  ...
  if (input.route) {
    ...
    const actionGate = buildCiSweeperRepairActionGate({
      request,
      route: input.route,
      changedFiles: (request.repair_actions ?? []).map((item: any) => item.file_path ?? item.path).filter(Boolean),
    });
  ...
  const readClaims = async () => {
    ...
    if (!response.ok) return { response, claims: [] };
    const notes = await response.json() as any[];
    return { response, claims: notes.map(parseCiSweeperClaim).filter((claim): claim is any => Boolean(claim && claim.request_id === request.request_id)) };
  };
  let before;
  try {
  ...
  if (!before.response.ok) return blocked('claim-read-failed', { request_id: request.request_id, http_status: before.response.status });
  if (before.claims.length > 0) return claimDuplicateResult(audit, before.claims[0]);
  ...
  try {
    ...
    if (!response.ok) return blocked('claim-write-failed', { request_id: request.request_id, http_status: response.status });
  } catch {
  ...
  if (!after.response.ok) return blocked('claim-reread-failed', { request_id: request.request_id, http_status: after.response.status });
...
export async function appendGitLabCiSweeperLifecycleEvidence(input: {
  ...
  if (!notesResponse.ok) return { schema: 'zj-loop.gitlab_ci_sweeper_lifecycle.v1', status: 'blocked', reason: 'claim-read-failed', audit, lifecycle: null };
...
export function buildCiSweeperVerifierPlan(input: { request?: any; route?: any }) {
  ...
  if (!Array.isArray(requested) || requested.length === 0) refusals.push('verification-gates-missing');
  if (!Array.isArray(allowlist) || allowlist.length === 0) refusals.push('verification-gate-allowlist-missing');
  const commands: any[] = [];
  if (Array.isArray(requested) && Array.isArray(allowlist)) {
    requested.forEach((command: any, index: number) => {
      if (!isStructuredCommand(command)) {
        refusals.push(`verification-command-must-be-structured:${index}`);
        return;
      }
      const allowed = allowlist.some((candidate: any) => isStructuredCommand(candidate) && sameCommand(candidate, command));
      if (!allowed) refusals.push(`verification-command-not-allowlisted:${command.id ?? index}`);
...
export function buildCiSweeperRepairActionGate(input: {
  ...
  const refusals: string[] = [];
  if (!Array.isArray(actions) || actions.length === 0) refusals.push('repair-actions-missing');
  if (!Array.isArray(allowedActions) || allowedActions.length === 0) refusals.push('repair-action-allowlist-missing');
  if (!Array.isArray(scopes) || scopes.length === 0) refusals.push('repair-scope-missing');
  ...
  if (Array.isArray(actions)) {
    for (const item of actions) {
      ...
      if (!file || file.startsWith('../') || file.startsWith('/')) refusals.push(`repair-action-path-escapes-repository:${String(actionPath ?? '')}`);
      if (Array.isArray(allowedActions) && !allowedActions.includes(item.action)) refusals.push(`repair-action-not-allowlisted:${item.action}`);
      if (Array.isArray(scopes) && !scopes.some((scope: unknown) => file.startsWith(normalizeScope(scope)))) {
        refusals.push(`repair-action-scope-mismatch:${String(actionPath ?? '')}`);
      }
      if (item.action === 'chmod' && !['100644', '100755'].includes(String(item.mode ?? ''))) refusals.push(`repair-action-mode-invalid:${String(item.path ?? '')}`);
    }
  }
  if (normalizedFiles.some((file) => !file || !scopes?.some((scope: unknown) => file.startsWith(normalizeScope(scope))))) {
...
export async function createGitLabCiSweeperRepairMr(input: {
  ...
  if (!input.token) return blocked('gitlab-token-required');
  if (!input.projectPath.trim() || !input.targetBranch.trim() || !input.branch.trim()) return blocked('repair-mr-fields-required');
  if (!/^automated\/ci-sweeper-gitlab-[a-zA-Z0-9._-]+$/.test(input.branch)) return blocked('repair-branch-name-invalid');
  ...
  if (dedupe.existing) return completedRepairMrResult({ ...audit, repair_dedupe_key: dedupe.dedupe.key, repair_content_digest: dedupe.dedupe.digest }, 'duplicate', dedupe.existing);
  ...
  if (!branchCreateResponse.ok && branchCreateResponse.status !== 400) return blocked('repair-branch-create-failed', { http_status: branchCreateResponse.status });
...
export async function triggerGitLabCiSweeperConsumerPipeline(input: {
  ...
  if (!response.ok) return blocked('consumer-trigger-failed', { http_status: response.status });
  const pipeline = await response.json() as any;
  if (!Number.isInteger(Number(pipeline.id))) return blocked('consumer-trigger-response-invalid');
  return {
    schema: 'zj-loop.gitlab_ci_sweeper_consumer_trigger.v1',
    status: 'completed',
    outcome: 'triggered',
...
export async function executeGitLabCiSweeperCloseout(input: {
  ...
  if (input.confirmationPhrase !== 'DELETE_MERGED_ROADMAP_BRANCH_AND_CLOSE_CARRIER') return blocked('confirmation-required');
  if (!input.token) return blocked('gitlab-token-required');
  if (!input.projectPath.trim() || !input.requestId.trim() || !input.targetBranch.trim()) return blocked('closeout-fields-required');
  if (!Number.isInteger(Number(input.mergeRequestIid)) || !Number.isInteger(Number(input.issueIid))) return blocked('closeout-issue-fields-invalid');
  if (!/^automated\/ci-sweeper-gitlab-[a-zA-Z0-9._-]+$/.test(input.branch)) return blocked('repair-branch-invalid');
  const fetchImpl = input.fetchImpl ?? globalThis.fetch;
  if (!fetchImpl) return blocked('gitlab-fetch-unavailable');
  const headers = { ...buildGitLabAuthHeaders({ token: input.token }), 'User-Agent': 'zj-loop-ci-sweeper' };
  ...
  if (!issueResponse.ok) return blocked('carrier-issue-read-failed', { http_status: issueResponse.status });
  const mr = await mrResponse.json() as any;
  const issue = await issueResponse.json() as any;
  if (!(mr.merged === true || mr.state === 'merged' || mr.merged_at)) return blocked('merge-request-not-merged');
  if (String(mr.source_branch ?? '') !== input.branch || String(mr.target_branch ?? '') !== input.targetBranch) return blocked('request-source-mismatch', { merge_request_source_branch: mr.source_branch ?? null, merge_request_target_branch: mr.target_branch ?? null });
  if (!String(mr.description ?? '').includes(input.requestId)) return blocked('request-source-mismatch', { reason_detail: 'request-id-missing-from-merge-request' });
  const parsed = parseIssueFixRequestComments([{ id: issue.iid ?? input.issueIid, body: String(issue.description ?? '') }])[0];
  const request = parsed?.validation.ok ? parsed.request : null;
  if (!request || request.request_id !== input.requestId || request.source_signal?.provider !== 'gitlab' || String(request.subject?.repo ?? '') !== input.projectPath || request.route_decision?.target_consumer !== 'ci-sweeper') return blocked('request-source-mismatch');
  let notesResponse;
  try {
  ...
  if (!notesResponse.ok) return blocked('claim-read-failed', { http_status: notesResponse.status });
  const claims = (await notesResponse.json() as any[]).map(parseCiSweeperClaim).filter((claim): claim is any => Boolean(claim && claim.request_id === input.requestId));
  if (claims.length !== 1 || claims[0].status !== 'claimed') return blocked('claim-required');
  let branchResponse;
  try {
    branchResponse = await fetchImpl(branchUrl, { headers });
  } catch {
  ...
  if (!branchResponse.ok && branchResponse.status !== 404) return blocked('repair-branch-read-failed', { http_status: branchResponse.status });
  const steps: any[] = [];
  if (branchResponse.status === 404) {
    ...
    if (!deleteResponse.ok && deleteResponse.status !== 404) return blocked('repair-branch-delete-failed', { http_status: deleteResponse.status, side_effects_executed: true, steps });
  ...
  if (!noteResponse.ok) return blocked('closeout-evidence-write-failed', { side_effects_executed: true, steps });
  ...
  if (!closeResponse.ok) return blocked('carrier-close-failed', { http_status: closeResponse.status, side_effects_executed: true, steps });
...
export async function executeGitLabCiSweeperRepairMr(input: {
  ...
  const blocked = (reason: string, extra: Record<string, unknown> = {}) => ({ schema: 'zj-loop.gitlab_ci_sweeper_execution.v1', status: 'blocked', reason, audit, claim: null, repair_mr: null, ...extra });
  if (!input.token) return blocked('gitlab-token-required');
  if (!input.route || input.route.maturity?.runner !== 'execution-ready') return blocked('runner-not-execution-ready');
  const fetchImpl = input.fetchImpl ?? globalThis.fetch;
  if (!fetchImpl) return blocked('gitlab-fetch-unavailable');
  const headers = { ...buildGitLabAuthHeaders({ token: input.token }), 'User-Agent': 'zj-loop-ci-sweeper' };
  const issueUrl = buildGitLabApiUrl({ apiBaseUrl: input.apiBaseUrl, projectPath: input.projectPath, path: ['issues', input.issueIid] });
  let issueResponse;
  try { issueResponse = await fetchImpl(issueUrl, { headers }); } catch { return blocked('issue-read-failed'); }
  if (!issueResponse.ok) return blocked('issue-read-failed', { http_status: issueResponse.status });
  const issue = await issueResponse.json() as any;
  const parsed = parseIssueFixRequestComments([{ id: issue.iid ?? input.issueIid, body: String(issue.description ?? '') }])[0];
  const request = parsed?.validation.ok ? parsed.request : null;
  ...
  const verifier = buildCiSweeperVerifierPlan({ request, route: input.route });
  const actionGate = buildCiSweeperRepairActionGate({ request, route: input.route, changedFiles: (request.repair_actions ?? []).map((item: any) => item.file_path ?? item.path).filter(Boolean) });
  if (verifier.status !== 'ready' || actionGate.status !== 'ready') return blocked('verifier-blocked', { verifier, action_gate: actionGate });
  const claim = await claimGitLabCiSweeperIssueFixRequest({ ...input, route: input.route, consumerId: 'ci-sweeper' });
  if (claim.status !== 'completed' || (claim as any).outcome !== 'claimed') return blocked('claim-not-won', { claim });
  ...
  if (running.status !== 'completed') return blocked('lifecycle-running-write-failed', { claim, lifecycle: running });
  ...
  return { schema: 'zj-loop.gitlab_ci_sweeper_execution.v1', status: repairMr.status === 'completed' && lifecycle.status === 'completed' ? 'completed' : 'blocked', outcome: (repairMr as any).outcome ?? null, audit, claim, repair_mr: repairMr, lifecycle };
}

function validateGitLabCommitActions(actions: any[]): boolean {
  return actions.every((item) => {
    if (!item || !['create', 'update', 'delete', 'move', 'chmod'].includes(item.action)) return false;
    if (!safeRepoPath(item.file_path) || (item.previous_path !== undefined && !safeRepoPath(item.previous_path))) return false;
    if (item.action === 'move' && !item.previous_path) return false;
    if (['create', 'update', 'move'].includes(item.action) && typeof item.content !== 'string') return false;
    if (item.encoding !== undefined && item.encoding !== 'base64') return false;
    if (item.action === 'chmod' && !['100644', '100755'].includes(String(item.execute_filemode ?? ''))) return false;
    return true;
  });
}

function safeRepoPath(value: unknown): boolean {
  const path = String(value ?? '').replaceAll('\\', '/');
  return Boolean(path && !path.startsWith('/') && !path.split('/').includes('..'));
}

async function postGitLabJson(fetchImpl: typeof fetch, url: string, headers: Record<string, string>, payload: any) {
  try {
...
async function readGitLabResponseError(response: { text?: () => Promise<string> }) {
...
function summarizeGitLabCommitRequest(payload: Record<string, any>) {
  return {
...
function completedRepairMrResult(audit: Record<string, unknown>, outcome: 'created' | 'duplicate', mergeRequest: any) {
  return {
    schema: 'zj-loop.gitlab_ci_sweeper_repair_mr.v1',
    status: 'completed',
    outcome,
...
function isStructuredCommand(value: any): boolean {
  return Boolean(value && typeof value.id === 'string' && value.id.trim() && typeof value.command === 'string' && value.command.trim() && Array.isArray(value.args) && value.args.every((arg: unknown) => typeof arg === 'string') && typeof value.cwd === 'string' && value.cwd.trim());
}

function sameCommand(left: any, right: any): boolean {
...
function completedGitLabIssueFixRequest(input: {
  ...
  issue: any;
  input: { apiBaseUrl?: string; projectPath: string };
}) {
...
function ciSweeperFilesOrAreasFor(provider: 'github' | 'gitlab') {
  if (provider === 'gitlab') {
...
function ciProviderMetadata(input: { provider: 'github' | 'gitlab'; runId: string; sourceUrl: string }) {
