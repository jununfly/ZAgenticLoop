import type { CompletionAlignmentCell, CompletionAlignmentLedger } from './completion-alignment.js';

export const COMPLETION_ALIGNMENT_TEXT_SCHEMA = 'zj-loop.completion-alignment-text.v1';

export function renderCompletionAlignmentText(ledger: CompletionAlignmentLedger): string {
...
function overallStatus(ledger: CompletionAlignmentLedger): string {
  if (ledger.summary.stale > 0) return 'stale';
  if (ledger.summary.blocked > 0) return 'blocked';
  if (ledger.summary.incomplete > 0 || ledger.summary.unsupported > 0) return 'incomplete';
  return 'complete';
}

function formatCells(cells: CompletionAlignmentCell[]): string[] {
...
function formatActions(actions: Array<{ cell: CompletionAlignmentCell; action: CompletionAlignmentCell['next_actions'][number] }>): string[] {
  if (actions.length === 0) return ['  - none'];
  return actions.map(({ cell, action }) =>
    `  - ${cell.route_id}:${cell.adapter_id} type=${action.type} target=${action.target} label=${action.label}`,
  );
}
