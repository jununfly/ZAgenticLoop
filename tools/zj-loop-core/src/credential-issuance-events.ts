...
function requireDigest(value: string): string {
  if (!DIGEST_PATTERN.test(value)) throw new Error('issuance-digest-invalid');
  return value;
}

function requireTimestamp(value: string, error: string): string {
...
function capabilities(values: string[]): string[] {
  ...
  if (result.some((value) => !value.trim())) throw new Error('credential-capability-invalid');
  return result;
}
...
export function createCredentialClaimEvent(input: { request_id: string; credential_id: string; claimed_at: string }): StateEventInput {
  requireText(input.request_id, 'request-id-required');
  requireText(input.credential_id, 'credential-id-required');
...
export function createCredentialRevokeEvent(input: { request_id: string; credential_id: string; revoked_at: string; reason: string }): StateEventInput {
  requireText(input.request_id, 'request-id-required');
  requireText(input.credential_id, 'credential-id-required');
...
export function createCredentialExpireEvent(input: { request_id: string; credential_id: string; expired_at: string }): StateEventInput {
  requireText(input.request_id, 'request-id-required');
  requireText(input.credential_id, 'credential-id-required');
...
export function createNodeRevokeEvent(input: { request_id: string; network_id: string; node_id: string; revoked_at: string; reason: string }): StateEventInput {
  requireText(input.request_id, 'request-id-required');
  requireText(input.network_id, 'network-id-required');
  requireText(input.node_id, 'node-id-required');
