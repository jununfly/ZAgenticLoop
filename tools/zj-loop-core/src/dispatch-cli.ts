...
import { dispatchSignal, readSignalEnvelope, resumeOrchestration } from './dispatch-runner.js';

const exitCode = await runCli({
  name: 'zj-loop-dispatch',
  description: 'Dispatch a structured ZAgenticLoop signal through the Route Table.',
  usage: 'zj-loop-dispatch (--signal <file> | --orchestration <id> --mode resume) [--root <dir>] [--mode auto|plan-only|execute|resume] [--format json]',
  options: [
    { name: 'signal', type: 'string', description: 'Path to a zj-loop.signal.v1 JSON file' },
    { name: 'orchestration', type: 'string', description: 'Persisted orchestration id; supported only with --mode resume' },
  ...
  async handler({ io, options }) {
    if (typeof options.orchestration === 'string' && options.orchestration.trim()) {
      if (options.mode !== 'resume') throw new Error('--orchestration requires --mode resume');
      if (typeof options.signal === 'string' && options.signal.trim()) throw new Error('Use either --signal or --orchestration, not both');
      const output = await resumeOrchestration({
