...
export type LoopDoctorReport = {
  ...
  summary: {
    ...
    route_health: Array<{ route_id: string; total: number; stopped: number; latest_status: string }>;
    provider_health: Array<{ provider: string; total: number; stopped: number; latest_status: string }>;
...
type OrchestrationLike = {
  ...
  consumer_adapter_result?: {
    ...
    review_artifacts?: Array<{ kind?: string; path?: string; status?: string }>;
  };
  stop_signal?: {
    reason?: string;
    next_steps?: string[];
...
type LoadedOrchestration = {
  path: string;
  artifactDir: string;
  value: OrchestrationLike;
};
...
async function readRunStates(root: string, filters?: LoopDoctorFilters): Promise<LoadedRun[]> {
  const runsDir = path.resolve(root, 'zj-loop', 'runs');
  const states: LoadedRun[] = [];
  for (const entry of await safeReaddir(runsDir)) {
    if (!entry.endsWith('.json')) continue;
    const relativePath = `zj-loop/runs/${entry}`;
    const value = JSON.parse(await readFile(path.join(runsDir, entry), 'utf8')) as RunStateLike;
    if (filters?.runId && value.run_id !== filters.runId) continue;
    if (filters?.orchestrationId || filters?.provider || filters?.subject) continue;
    states.push({ path: relativePath, value });
  }
  return states.sort((left, right) => left.path.localeCompare(right.path));
}

async function readOrchestrations(root: string, filters?: LoopDoctorFilters): Promise<LoadedOrchestration[]> {
  const orchestrationDir = path.resolve(root, 'zj-loop', 'orchestrations');
  const states: LoadedOrchestration[] = [];
  for (const entry of await safeReaddir(orchestrationDir)) {
    if (!entry.endsWith('.json')) continue;
    const relativePath = `zj-loop/orchestrations/${entry}`;
    const value = JSON.parse(await readFile(path.join(orchestrationDir, entry), 'utf8')) as OrchestrationLike;
    if (filters?.orchestrationId && value.orchestration_id !== filters.orchestrationId) continue;
    if (filters?.provider && value.signal?.provider !== filters.provider) continue;
    if (filters?.subject && subjectKey(value) !== filters.subject) continue;
    states.push({
      path: relativePath,
...
function runSummary(run: LoadedRun): LoopRunSummary {
  return {
...
function orchestrationSummary(orchestration: LoadedOrchestration): LoopOrchestrationSummary {
...
function runArtifacts(run: LoadedRun): LoopEvidenceArtifact[] {
  return [
...
function orchestrationArtifacts(orchestration: LoadedOrchestration): LoopEvidenceArtifact[] {
...
async function readOrchestrationChildArtifacts(root: string, orchestrations: LoadedOrchestration[]): Promise<LoopEvidenceArtifact[]> {
  const artifacts: LoopEvidenceArtifact[] = [];
...
function classifyRunStopSignals(run: LoadedRun): ClassifiedStopSignal[] {
...
function classifyOrchestrationStopSignals(orchestration: LoadedOrchestration): ClassifiedStopSignal[] {
  const value = orchestration.value;
...
function stopSignalPreset(stopCode: string): {
  category: string;
  responsible_layer: string;
  severity: ClassifiedStopSignal['severity'];
  recoverability: ClassifiedStopSignal['recoverability'];
...
function summarizeFindings(input: {
  runs: LoadedRun[];
  classifiedStopSignals: ClassifiedStopSignal[];
}): LoopDoctorFinding[] {
...
function buildSummary(input: {
  runSummaries: LoopRunSummary[];
  orchestrationSummaries: LoopOrchestrationSummary[];
  classifiedStopSignals: ClassifiedStopSignal[];
}): LoopDoctorReport['summary'] {
  const allStatuses = [
...
function healthByRoute(input: {
  runSummaries: LoopRunSummary[];
  orchestrationSummaries: LoopOrchestrationSummary[];
}): Array<{ route_id: string; total: number; stopped: number; latest_status: string }> {
  const rows = new Map<string, { route_id: string; total: number; stopped: number; latest_status: string; latest?: string }>();
  for (const item of [...input.runSummaries, ...input.orchestrationSummaries]) {
    ...
    row.total++;
    if (item.status === 'stopped' || item.status === 'hard_stopped' || item.stop_reason) row.stopped++;
...
function healthByProvider(orchestrations: LoopOrchestrationSummary[]): Array<{ provider: string; total: number; stopped: number; latest_status: string }> {
  const rows = new Map<string, { provider: string; total: number; stopped: number; latest_status: string; latest?: string }>();
  for (const item of orchestrations) {
    ...
    row.total++;
    if (item.status === 'hard_stopped' || item.stop_reason) row.stopped++;
    if (!row.latest || stringOr(item.updated_at, '').localeCompare(row.latest) > 0) {
      row.latest = item.updated_at;
      row.latest_status = item.status;
    }
...
function runStopReason(run: RunStateLike): string | undefined {
  return run.stop_signal?.reason ?? run.machine_envelope?.stop_signal?.reason;
}

function orchestrationStopReason(orchestration: OrchestrationLike): string | undefined {
...
function subjectKey(orchestration: OrchestrationLike): string | undefined {
  const kind = orchestration.signal?.subject?.kind;
  const id = orchestration.signal?.subject?.id;
  return kind && id ? `${kind}:${id}` : undefined;
}

function normalizeStopCode(value: string): string {
  const trimmed = value.trim();
  if (!trimmed) return 'unknown-stop';
  if (/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(trimmed)) return trimmed;
  if (/missing required credential/i.test(trimmed)) return 'credential-missing';
  if (/ambiguous-route/i.test(trimmed)) return 'ambiguous-route';
  if (/protocol/i.test(trimmed) && /repair/i.test(trimmed)) return 'needs-protocol-repair';
  if (/dirty workspace/i.test(trimmed)) return 'dirty-workspace-conflict';
