...
export async function closeGitLabChangelogDraft(input: {
  projectPath: string; mergeRequestIid: string | number; issueIid: string | number; requestId: string; claimId: string; branch: string; targetBranch: string; confirmationPhrase: string; token?: string; apiBaseUrl?: string; fetchImpl?: typeof fetch;
}) {
  ...
  const blocked = (reason: string, extra: any = {}) => ({ schema: 'zj-loop.gitlab_changelog_draft_closeout.v1', status: 'blocked', reason, audit, side_effects_executed: false, steps: [], ...extra });
  if (input.confirmationPhrase !== CHANGELOG_DRAFTER_CLOSEOUT_CONFIRMATION) return blocked('confirmation-required', { required_phrase: CHANGELOG_DRAFTER_CLOSEOUT_CONFIRMATION });
  if (!input.token) return blocked('gitlab-token-required');
  if (!/^automated\/changelog-drafter-gitlab-[a-z0-9-]+$/.test(input.branch) || !input.projectPath.trim() || !input.requestId.trim() || !input.claimId.trim()) return blocked('closeout-fields-or-branch-invalid');
  ...
  if (!request || request.request_id !== input.requestId || request.release_window?.repo !== input.projectPath || request.release_window?.base_branch !== input.targetBranch) return blocked('request-source-mismatch');
  const notesUrl = `${issueUrl}/notes`;
  const notesResponse = await fetchImpl(`${notesUrl}?per_page=100`, { headers });
  if (!notesResponse.ok) return blocked('notes-read-failed', { http_status: notesResponse.status });
  ...
  if (!claim) return blocked('claim-not-found');
  ...
  if (mr.state !== 'merged' || !mr.merged_at || !mr.merge_commit_sha) return blocked('merge-not-confirmed', { provider_state: mr.state ?? null, merged_at: mr.merged_at ?? null, merge_commit_sha: mr.merge_commit_sha ?? null });
  if (String(mr.source_branch ?? '') !== input.branch || String(mr.target_branch ?? '') !== input.targetBranch) return blocked('merge-request-binding-mismatch');
  ...
  if (!existing) {
    ...
    if (!noteWrite.ok) return blocked('closeout-evidence-write-failed', { http_status: noteWrite.status, side_effects_executed: true, steps });
    steps.push({ name: 'append-closeout-evidence', status: 'written' });
  } else steps.push({ name: 'append-closeout-evidence', status: 'existing' });
  const reread = await fetchImpl(`${notesUrl}?per_page=100`, { headers });
  if (!reread.ok) return blocked('closeout-evidence-reread-failed', { http_status: reread.status, side_effects_executed: true, steps });
  const rereadNotes = await reread.json() as any[];
  ...
  if (closeoutMatches.length !== 1) return blocked('closeout-evidence-reread-ambiguous', { side_effects_executed: true, steps });
  ...
  if (!deleteResponse.ok && deleteResponse.status !== 404) return blocked('branch-delete-failed', { http_status: deleteResponse.status, side_effects_executed: true, steps });
