...
export async function executeGitLabDependencySweeperCloseout(input: {
  projectPath: string; mergeRequestIid: string | number; issueIid: string | number; requestId: string; claimId: string;
  branch: string; targetBranch: string; token?: string; confirmationPhrase?: string; apiBaseUrl?: string; fetchImpl?: typeof fetch;
}) {
  ...
  const blocked = (reason: string, extra: Record<string, unknown> = {}) => ({ schema: 'zj-loop.gitlab_dependency_sweeper_closeout.v1', status: 'blocked', reason, audit, side_effects_executed: false, ...extra });
  if (input.confirmationPhrase !== DEPENDENCY_SWEEPER_CLOSEOUT_CONFIRMATION_PHRASE) return blocked('confirmation-required', { confirmation: { location: 'GitLab manual job or Codex conversation', required_phrase: DEPENDENCY_SWEEPER_CLOSEOUT_CONFIRMATION_PHRASE } });
  if (!input.token) return blocked('gitlab-token-required');
  if (!input.projectPath.trim() || !input.requestId.trim() || !input.claimId.trim() || !input.branch.trim() || !input.targetBranch.trim()) return blocked('closeout-fields-required');
  if (!/^automated\/dependency-sweeper-gitlab-[a-z0-9-]+$/.test(input.branch)) return blocked('repair-branch-invalid');
  ...
  try { mrResponse = await fetchImpl(mrUrl, { headers }); issueResponse = await fetchImpl(issueUrl, { headers }); } catch { return blocked('closeout-preflight-read-failed'); }
  ...
  if (String(mr.source_branch ?? '') !== input.branch || String(mr.target_branch ?? '') !== input.targetBranch) return blocked('request-source-mismatch');
  ...
  const actualFiles = (changes.changes ?? []).map((change: any) => String(change.new_path ?? change.old_path ?? '')).sort();
  if (JSON.stringify(actualFiles) !== JSON.stringify(expectedFiles)) return blocked('repair-scope-mismatch', { expected_files: expectedFiles, actual_files: actualFiles });
  ...
  if (claims.length !== 1 || claims[0].claim_id !== input.claimId) return blocked('claim-required');
  ...
  const steps: any[] = [];
  if (branchResponse.status === 404) steps.push({ name: 'delete-repair-branch', status: 'skipped', reason: 'branch-absent' });
  else {
