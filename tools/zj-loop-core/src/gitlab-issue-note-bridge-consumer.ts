...
export type GitLabIssueNoteBridgeConsumerResult = {
  ...
  pipeline: { source: string | null; ref: string | null };
  binding: {
...
type ProjectRegistration = {
  ...
  routes?: Array<{
    route_id?: unknown;
    marker?: unknown;
    default_executor?: { kind?: unknown; profile?: unknown };
    allowed_executors?: Array<{ kind?: unknown; profile?: unknown; capabilities?: unknown }>;
  }>;
  initiators?: { pipeline_request?: { allowed_gitlab_user_ids?: unknown[] } };
};

type FetchLike = (input: string, init?: { headers?: Record<string, string> }) => Promise<{ status: number; json(): Promise<unknown> }>;
...
export async function runGitLabIssueNoteBridgeConsumer(input: {
  ...
  for (const key of REQUIRED_BRIDGE_VARIABLES) {
    if (!values[key]) return blocked(result, `${key.toLowerCase()}-required`);
  }
  ...
  try {
    ...
    registration = yaml.parse(registrationText) as ProjectRegistration;
  } catch {
  ...
  if (!registrationCheck.ok) return blocked(result, registrationCheck.reason, { registrationPath });

  const providerCheck = await rereadSourceBinding({ env, values, marker: registrationCheck.marker, fetchImpl: input.fetchImpl ?? fetch });
  if (!providerCheck.ok) return blocked(result, providerCheck.reason, { registrationPath });
  ...
  try {
    ...
    if (!decision.allowed) return blocked(result, 'route-not-enabled', { registrationPath });
  } catch {
...
function baseResult(input: { values: Record<string, string | null>; projectPath: string | null; targetRoute: string | null }): GitLabIssueNoteBridgeConsumerResult {
  return {
...
function blocked(
  result: GitLabIssueNoteBridgeConsumerResult,
  reason: string,
  extra: { registrationPath?: string } = {},
): GitLabIssueNoteBridgeConsumerResult {
...
function validateRegistration(registration: ProjectRegistration, projectPath: string, routeId: string): { ok: true; executor: { kind: string; profile: string }; marker: string } | { ok: false; reason: string } {
  if (!registration || registration.schema !== GITLAB_PROJECT_REGISTRATION_SCHEMA) return { ok: false, reason: 'project-registration-schema-invalid' };
  if (registration.project_path !== projectPath) return { ok: false, reason: 'registration-project-mismatch' };
  if (registration.default_branch !== 'master') return { ok: false, reason: 'registration-default-branch-mismatch' };
  const route = registration.routes?.find((candidate) => candidate.route_id === routeId);
  if (!route) return { ok: false, reason: 'registration-route-missing' };
  if (typeof route.marker !== 'string' || !route.marker.trim()) return { ok: false, reason: 'registration-marker-required' };
  const executor = route.default_executor;
  if (executor?.kind !== 'gitlab-pipeline' || executor.profile !== 'ai-studio-master-pipeline') return { ok: false, reason: 'registration-default-executor-mismatch' };
  const allowed = route.allowed_executors?.some((candidate) => candidate.kind === executor.kind && candidate.profile === executor.profile);
  if (!allowed) return { ok: false, reason: 'registration-executor-not-allowlisted' };
  const allowedInitiators = registration.initiators?.pipeline_request?.allowed_gitlab_user_ids;
  if (!Array.isArray(allowedInitiators) || !allowedInitiators.includes(81)) return { ok: false, reason: 'registration-initiator-allowlist-missing' };
...
async function rereadSourceBinding(input: {
  ...
  if (!jobToken && !fallbackToken) return { ok: false, reason: 'gitlab-read-token-required' };
  ...
  try {
    for (const candidate of auth) {
      ...
      if (noteResponse.status !== 200) continue;
      ...
      if (typeof note.body !== 'string' || !note.body.includes(input.marker) || Number(note.author?.id) !== 81) return { ok: false, reason: 'gitlab-note-request-mismatch' };
      return { ok: true };
    }
