...
export async function persistGitLabIssueNoteBridgeReceipt(input: {
  ...
  if (existingDedupe && existingDedupe.event_id !== input.envelope.event_id) {
    ...
    const existingReceipt = await readJsonIfPresent<GitLabIssueNoteBridgeReceipt>(existingReceiptPath);
    if (existingReceipt) {
      return {
        status: 'duplicate',
  ...
  if (existingReceipt) {
    ...
    const receiptDedupe = await readJsonIfPresent<GitLabIssueNoteBridgeDedupeRecord>(paths.dedupe);
    if (receiptDedupe) {
      return {
        status: 'duplicate',
        receipt: existingReceipt,
