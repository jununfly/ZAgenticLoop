...
export function buildGitLabPrStewardIssueFixRequest(report: any) {
...
export async function createGitLabPrStewardIssueFixRequest(input: {
  projectPath: string; report: any; token?: string; confirmationPhrase?: string; apiBaseUrl?: string; fetchImpl?: typeof fetch;
}) {
  const source = input.report?.report?.source_review;
  const observations = input.report?.report?.observations;
  const audit: Record<string, unknown> = { project_path: input.projectPath, merge_request_iid: Number(source?.mr_iid), request_id: null, auth_source: input.token ? 'GITLAB_TOKEN' : null };
  const blocked = (reason: string, extra: Record<string, unknown> = {}) => ({ schema: 'zj-loop.gitlab_pr_steward_issue_fix_request.v1', status: 'blocked', reason, audit, issue: null, side_effects_executed: false, ...extra });
  if (input.confirmationPhrase !== CONFIRM_PR_STEWARD_ISSUE_FIX_REQUEST) return blocked('confirmation-required', { confirmation: { location: 'GitLab manual job or Codex conversation', required_phrase: CONFIRM_PR_STEWARD_ISSUE_FIX_REQUEST } });
  if (!input.token) return blocked('gitlab-token-required');
  if (input.report?.schema !== 'zj-loop.gitlab_pr_steward_report.v1' || input.report?.status !== 'completed' || input.report?.report?.status !== 'candidate-fix-request') return blocked('report-not-eligible');
  if (!input.projectPath.trim() || source?.provider !== 'gitlab' || source?.repo !== input.projectPath || !source?.mr_iid || !source?.head_sha || observations?.checks !== 'failure' || !Array.isArray(observations.failed_jobs) || observations.failed_jobs.length === 0) return blocked('request-source-mismatch');
  ...
  if (!existingResponse.ok) return blocked('issue-fix-request-dedupe-read-failed', { http_status: existingResponse.status });
  const existing = (await existingResponse.json() as any[]).find((issue) => parseIssueFixRequestComments([{ id: issue.iid, body: String(issue.description ?? '') }]).some((item) => item.validation.ok && item.request.request_id === request.request_id));
  if (existing) return { schema: 'zj-loop.gitlab_pr_steward_issue_fix_request.v1', status: 'completed', outcome: 'duplicate', audit, issue: { iid: Number(existing.iid), url: String(existing.web_url ?? '') }, side_effects_executed: false };
  ...
  if (!response.ok) return blocked('issue-fix-request-create-failed', { http_status: response.status, side_effects_executed: true });
