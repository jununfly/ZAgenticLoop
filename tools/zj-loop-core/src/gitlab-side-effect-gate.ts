...
export function buildGitLabCarrierSideEffectGate(input: GitLabSideEffectGateInput) {
  const audit = {
  ...
  if (!input.routeFamily.trim()) return blocked('route-family-required');
  ...
  if (!isEnabled(input.carrierEnabled)) return blocked('carrier-disabled');
