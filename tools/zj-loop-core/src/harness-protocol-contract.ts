...
export function recordLoopRunMetrics(input: {
  ...
  for (const output of input.outputs) {
    ...
    if (output.status === 'needs_confirmation') humanHandoffCount++;
    if (output.stop_signal !== undefined) structuredStopSignals++;
...
function renderNextActions(nextActions: LoopHarnessNextAction[]): string[] {
...
function validateStatusSpecificFields(value: Partial<LoopHarnessProtocolOutput>, errors: string[]) {
...
function validateConfirmationEnvelope(confirmation: unknown, errors: string[]) {
...
function validateNextActions(nextActions: unknown, errors: string[]) {
  ...
  for (const [index, action] of nextActions.entries()) {
    ...
    const value = action as Partial<LoopHarnessNextAction>;
