...
process.exitCode = await runCli({
  ...
  async handler({ io, options }) {
    ...
    if (!networkId || !pairingEndpoint || !humanId || !keyTag || !helperPath) throw new Error('network-id-pairing-endpoint-human-id-key-tag-helper-path-required');
    ...
    const upstream = createPairingHttpUpstream({ endpoint: pairingEndpoint, authorization: typeof options['owner-authorization'] === 'string' ? options['owner-authorization'] : undefined, ca: typeof options.ca === 'string' ? await readFile(options.ca, 'utf8') : undefined, cert: typeof options['client-cert'] === 'string' ? await readFile(options['client-cert'], 'utf8') : undefined, key: typeof options['client-key'] === 'string' ? await readFile(options['client-key'], 'utf8') : undefined });
    const bootstrapToken = randomBytes(32).toString('base64url');
    const server = createHumanApprovalUiServer({ signer, network_id: networkId, upstream, bootstrap_token: bootstrapToken });
