...
import type { RealAgentDogfoodApprovalRequest, RealAgentDogfoodApprovalUiUpstream } from './real-agent-dogfood-approval-ui-upstream.js';
...
function publicIdentity(identity: HumanSignerIdentity): Record<string, string> {
...
function pairingUpstreamReason(error: unknown, fallback = 'pairing-upstream-unavailable'): string {
...
async function readBody(request: IncomingMessage): Promise<Record<string, unknown>> {
  ...
  for await (const chunk of request) {
    ...
    size += value.length;
    if (size > 64 * 1024) throw new Error('ui-payload-too-large');
    chunks.push(value);
  }
  let parsed: unknown;
  try { parsed = JSON.parse(Buffer.concat(chunks).toString('utf8')); } catch { throw new Error('ui-json-invalid'); }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) throw new Error('ui-json-invalid');
  return parsed as Record<string, unknown>;
}

export function createHumanApprovalUiServer(input: HumanApprovalUiServerInput): Server {
  ...
  return createServer(async (request, response) => {
    ...
    if (dogfoodApprovalMatch) {
      ...
      const requestDigest = typeof body.request_digest === 'string' ? body.request_digest : '';
      if (!requestDigest) { blocked(response, 400, 'dogfood-approval-input-invalid'); return; }
      let current: RealAgentDogfoodApprovalRequest | undefined;
      try { current = (await input.dogfoodApprovals.list()).requests.find((item) => item.dogfood_id === dogfoodId); } catch { blocked(response, 503, 'dogfood-approval-read-model-unavailable'); return; }
      if (!current || current.network_id !== input.network_id || current.status !== 'pending' || current.summary_digest !== requestDigest) { blocked(response, 409, 'dogfood-approval-state-conflict'); return; }
      const expiresAt = new Date(Date.parse(now()) + 50 * 60 * 1000).toISOString();
      let context: HumanApprovalContext;
      try { context = await signApprovalContext({ signer: input.signer, network_id: input.network_id, human_device: input.human_device, action: 'real-agent-dogfood.approve', request_id: current.dogfood_id, request_digest: current.summary_digest, approved_capabilities: [], issued_at: now(), expires_at: expiresAt }); } catch { blocked(response, 400, 'dogfood-approval-signing-failed'); return; }
      try {
        const signerIdentity = await Promise.resolve(input.signer.getPublicIdentity());
        const identity = { ...signerIdentity, schema: context.schema } as const;
      ...
      } catch (error) { blocked(response, 503, error instanceof Error ? error.message : 'dogfood-approval-recording-failed'); }
      return;
    }
    ...
    if (evidenceMatch) {
      ...
      } catch { blocked(response, 503, 'evidence-upstream-unavailable'); }
      return;
    }
    ...
    if (approveMatch) {
      ...
      const requestDigest = typeof body.request_digest === 'string' ? body.request_digest : '';
      const capabilities = Array.isArray(body.approved_capabilities) && body.approved_capabilities.every((value) => typeof value === 'string') ? [...new Set(body.approved_capabilities as string[])].sort() : null;
      if (!requestDigest || !capabilities) { blocked(response, 400, 'ui-approval-input-invalid'); return; }
      let current: PairingRequestProjection | undefined;
      try { current = (await input.upstream.list({ network_id: input.network_id })).requests.find((item) => item.request_id === requestId); } catch { blocked(response, 503, 'pairing-upstream-unavailable'); return; }
      if (!current) { blocked(response, 404, 'pairing-request-not-found'); return; }
      ...
      let result: Record<string, unknown>;
      try { result = await input.upstream.approve({ network_id: input.network_id, request_id: requestId, request_digest: requestDigest, approved_capabilities: capabilities, context }); } catch (error) { blocked(response, 503, pairingUpstreamReason(error)); return; }
    ...
    if (rejectMatch) {
      ...
      if (!requestDigest || !reason || !allowedReasons.has(reason)) { blocked(response, 400, 'ui-rejection-input-invalid'); return; }
      let current: PairingRequestProjection | undefined;
      try { current = (await input.upstream.list({ network_id: input.network_id })).requests.find((item) => item.request_id === requestId); } catch { blocked(response, 503, 'pairing-upstream-unavailable'); return; }
      if (!current) { blocked(response, 404, 'pairing-request-not-found'); return; }
      if (current.status !== 'pending' || current.request_digest !== requestDigest) { blocked(response, 409, 'pairing-state-conflict'); return; }
      ...
      let result: Record<string, unknown>;
      try { result = await input.upstream.reject({ network_id: input.network_id, request_id: requestId, request_digest: requestDigest, reason, context }); } catch (error) { blocked(response, 503, pairingUpstreamReason(error)); return; }
      json(response, 201, { schema: HUMAN_APPROVAL_UI_SCHEMA, status: 'recorded', request_id: requestId, result, side_effects_executed: true });
      return;
    }
...
async function requestPairingApi(input: PairingHttpUpstreamInput, pathname: string, method: 'GET' | 'POST', body?: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  return await new Promise((resolve, reject) => {
    const request = httpsRequest(options, (response) => {
      ...
      response.on('end', () => {
        ...
        try { parsed = JSON.parse(text) as Record<string, unknown>; } catch { reject(new Error('pairing-upstream-response-invalid')); return; }
        if ((response.statusCode ?? 500) >= 400) { reject(new Error(typeof parsed.reason === 'string' ? parsed.reason : 'pairing-upstream-failed')); return; }
        resolve(parsed);
      });
    });
...
export function createPairingHttpUpstream(input: PairingHttpUpstreamInput): HumanApprovalUiUpstream {
