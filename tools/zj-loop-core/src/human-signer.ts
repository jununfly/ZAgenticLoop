...
export function createInMemoryHumanSigner(input: { human_id: string }): HumanSigner {
  ...
  const identity: HumanSignerIdentity = { schema: HUMAN_SIGNER_SCHEMA, human_id: humanId, algorithm: 'Ed25519', public_key_pem: publicKeyPem, public_key_fingerprint: publicKeyFingerprint };
  return {
    ...
    async sign(input) {
      ...
      return { schema: HUMAN_SIGNATURE_SCHEMA, algorithm: 'Ed25519', public_key_fingerprint: publicKeyFingerprint, signature_base64: sign(null, Buffer.from(input.payload), keys.privateKey).toString('base64') };
    },
  };
}
...
export function verifyHumanSignature(input: { identity: HumanSignerIdentity; payload: Uint8Array; signature: HumanSignature }): boolean {
  if (input.identity.schema !== HUMAN_SIGNER_SCHEMA || input.identity.algorithm !== 'Ed25519' || input.signature.schema !== HUMAN_SIGNATURE_SCHEMA || input.signature.algorithm !== 'Ed25519') return false;
  ...
  try {
    ...
    if (publicKey.asymmetricKeyType !== 'ed25519' || fingerprint(publicKey) !== input.identity.public_key_fingerprint) return false;
    return verify(null, Buffer.from(input.payload), publicKey, Buffer.from(input.signature.signature_base64, 'base64'));
  } catch {
    return false;
  }
}
