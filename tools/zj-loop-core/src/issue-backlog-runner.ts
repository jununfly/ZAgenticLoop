...
export async function scanGitLabIssueBacklog(input: GitLabBacklogScanInput) {
  const fetchImpl = input.fetchImpl ?? globalThis.fetch;
  const headers = buildGitLabAuthHeaders({ token: input.token, jobToken: input.jobToken });
  if (!fetchImpl) throw new Error('GitLab backlog scan requires fetch support');
  if (Object.keys(headers).length === 0) throw new Error('GitLab backlog scan requires GITLAB_TOKEN or CI_JOB_TOKEN');
  ...
  if (!response.ok) throw new Error(`GitLab backlog scan failed: ${response.status}`);
  ...
  const recommendations = issues.map((issue) => ({
    ...
    assignees: Array.isArray(issue.assignees) ? issue.assignees.map((assignee: any) => String(assignee.username ?? '')) : [],
...
function classifyGitLabIssue(issue: any) {
  const labels = (Array.isArray(issue.labels) ? issue.labels : []).map((label: unknown) => String(label).toLowerCase());
  ...
  if (issue.assignees?.length) return { recommendation: 'human-owned', reason: 'issue already has an assignee' };
  if (/need confirmation|需要.*确认/i.test(description)) return { recommendation: 'needs-human-clarification', reason: 'issue asks for confirmation' };
