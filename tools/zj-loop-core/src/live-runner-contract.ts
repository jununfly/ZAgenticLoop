...
export function buildLiveRunnerEvidence(input: Omit<LiveRunnerEvidence, 'schema'>): LiveRunnerEvidence {
  return {
    schema: LIVE_RUNNER_EVIDENCE_SCHEMA,
    ...input,
  } as LiveRunnerEvidence;
}

export function validateLiveRunnerEvidence(evidence: unknown): { ok: boolean; errors: string[] } {
  const errors: string[] = [];
  if (!evidence || typeof evidence !== 'object') {
    return { ok: false, errors: ['evidence must be an object'] };
  }

  const value = evidence as Partial<LiveRunnerEvidence>;
  ...
  if (!RUNNER_EVIDENCE_STATUSES.includes(value.status as LiveRunnerEvidenceStatus)) {
...
export function buildLiveRunnerEvidenceComment(evidence: LiveRunnerEvidence): string {
...
export function parseLiveRunnerEvidenceComments(
  comments: LiveRunnerEvidenceComment[],
): LiveRunnerEvidenceParseResult[] {
  const parsed: LiveRunnerEvidenceParseResult[] = [];
  for (const comment of Array.isArray(comments) ? comments : []) {
    const body = String(comment.body ?? '');
    for (const match of body.matchAll(STRUCTURED_COMMENT_PATTERN)) {
      try {
        const evidence = JSON.parse(match.groups?.json?.trim() ?? '') as LiveRunnerEvidence;
...
function completionFormsFor(consumerKind: string): string[] {
  return [...(COMPLETION_FORMS_BY_KIND[consumerKind as LiveRunnerConsumerKind] ?? [])];
}

function validateSideEffects(sideEffects: unknown, errors: string[]) {
  if (!sideEffects || typeof sideEffects !== 'object') {
    errors.push('side_effects must be an object');
    return;
  }
  const value = sideEffects as LiveRunnerEvidence['side_effects'];
  if (typeof value.executed !== 'boolean') {
    errors.push('side_effects.executed must be boolean');
  }
  if (!LIVE_RUNNER_SIDE_EFFECT_LEVELS.includes(value.level as typeof LIVE_RUNNER_SIDE_EFFECT_LEVELS[number])) {
...
function validateCompletionStatus(evidence: Partial<LiveRunnerEvidence>, errors: string[]) {
