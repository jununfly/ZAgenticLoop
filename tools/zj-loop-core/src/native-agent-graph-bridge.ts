import { createNativeOpnTracerExecution, type NativeOpnTracerExecution } from './native-opn-tracer-execution.js';
...
export function buildNativeOpnTracerExecutionFromNativeAgent(input: {
  ...
  if (!status) throw new Error('native-agent-graph-execution-not-terminal');
  if (status === 'succeeded' && !input.output_evidence_digest) throw new Error('native-agent-graph-output-required');
