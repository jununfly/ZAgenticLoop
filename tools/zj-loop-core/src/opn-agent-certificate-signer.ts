...
export const OPN_AGENT_CERTIFICATE_SIGNER_SCHEMA = 'zj-loop.opn_agent_certificate_signer.v1' as const;
...
export async function signOpnAgentCertificate(input: {
  ...
  if (await exists(certificatePath)) throw new Error('certificate-signer-output-exists');
  if (!(await exists(csrPath)) || !(await exists(caKeyPath)) || !(await exists(caCertPath))) throw new Error('certificate-signer-input-not-found');
  const days = input.days ?? 30;
  if (!Number.isInteger(days) || days < 1 || days > 825) throw new Error('certificate-signer-days-invalid');
