...
import { claimOpnAgentCredential, createOpnAgentJoinRequest, fetchOpnAgentJoinStatus, type OpnAgentJoinSession, submitOpnAgentJoinRequest } from './opn-agent-join.js';
...
const spec: CliSpec = {
  ...
  async handler({ options, io }) {
    ...
    if (command === 'status' || command === 'claim') {
      const session = JSON.parse(await read('session_file')) as OpnAgentJoinSession;
      ...
      const body = response.body as { status?: string; reason?: string; token?: string; [key: string]: unknown };
      if (response.statusCode < 200 || response.statusCode >= 300) throw new Error(`opn-agent-${command}-rejected:${body.reason ?? body.status ?? response.statusCode}`);
      ...
      } else io.stdout(JSON.stringify({ schema: 'zj-loop.opn_agent_join_status.v1', ...body, side_effects_executed: false }));
      return;
    }
