...
export function submitOpnAgentJoinRequest(input: {
  ...
  const options: RequestOptions = {
    ...
    // Node refuses CN-only matching in checkServerIdentity, so we replicate the
    // intended CN equality check here. Server identity is still bound by the
    // pinned Dev CA via `ca` + `rejectUnauthorized`.
