...
import { acquireOpnEndpointLock, createOpnEndpointBinding, classifyOpnEndpointStatus, isProcessAlive, opnEndpointRuntimePaths, probeOpnEndpointHealthz, readOpnEndpointBinding, spawnOpnEndpointServe, writeOpnEndpointBinding, type OpnEndpointRuntimeConfig } from './opn-endpoint-supervisor.js';
...
const spec: CliSpec = {
  ...
  options: [
    { name: 'command', type: 'positional' as const, description: 'Lifecycle command (default: serve)' },
    ...
    { name: 'session_ttl_minutes', flag: 'session-ttl-minutes', type: 'string' as const, description: 'Maximum pairing and transport session lifetime in minutes (default: 50)' },
    { name: 'runtime_dir', flag: 'runtime-dir', type: 'string' as const, description: 'Supervisor runtime directory (default: <state-store>.runtime)', valueName: 'PATH' },
    { name: 'identity_dir', flag: 'identity-dir', type: 'string' as const, description: 'Identity directory containing opn-endpoint.json (default: OPN_IDENTITY_DIR)', valueName: 'PATH' },
  ],
  async handler({ options, io }) {
    ...
    let fileConfig: OpnEndpointRuntimeConfig | undefined;
    if (identity_dir) {
      try { fileConfig = await loadOpnEndpointConfig(identity_dir); } catch (error) {
        if (command !== 'serve' || options.identity_dir !== undefined || process.env.OPN_IDENTITY_DIR) throw error;
      }
    }
    const resolved = (name: keyof OpnEndpointRuntimeConfig, configName: keyof OpnEndpointRuntimeConfig = name): string => options[name] !== undefined ? String(options[name]) : String(fileConfig?.[configName] ?? '');
    ...
    if (command !== 'serve') {
      if (!Number.isInteger(port) || port <= 0) throw new Error('opn-endpoint-supervisor-port-required');
      const binding = await readOpnEndpointBinding(paths.binding);
      if (command === 'start' || command === 'restart') {
        if (command === 'start' && binding && isProcessAlive(binding.pid)) throw new Error('opn-endpoint-already-running');
      ...
      if (command === 'stop') {
        if (binding && isProcessAlive(binding.pid)) process.kill(binding.pid, 'SIGTERM');
...
function endpointArgs(config: OpnEndpointRuntimeConfig): string[] {
  const names: Array<[keyof OpnEndpointRuntimeConfig, string]> = [['bind', '--bind'], ['port', '--port'], ['network_id', '--network-id'], ['state_store', '--state-store'], ['server_key', '--server-key'], ['server_cert', '--server-cert'], ['client_ca', '--client-ca']];
  const args = names.flatMap(([name, flag]) => [flag, String(config[name] ?? '')]);
  const optional: Array<[keyof OpnEndpointRuntimeConfig, string]> = [['artifact_store', '--artifact-store'], ['owner_human_id', '--owner-human-id'], ['owner_public_key', '--owner-public-key'], ['owner_token', '--owner-token'], ['session_ttl_minutes', '--session-ttl-minutes']];
  for (const [name, flag] of optional) if (config[name] !== undefined) args.push(flag, String(config[name]));
  return args;
}
