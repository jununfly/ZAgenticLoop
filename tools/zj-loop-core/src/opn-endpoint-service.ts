...
function windowsWrapperCommand(spec: OpnEndpointServiceSpec): string {
  return [spec.executable, spec.script, ...spec.args].map((item) => `"${item.replaceAll('"', '\\"')}"`).join(' ');
}

export function createWindowsWrapper(spec: OpnEndpointServiceSpec): string {
...
export function opnWebUiServiceLabel(network_id: string): string {
  const network = network_id.trim();
  if (!network) throw new Error('opn-web-ui-service-network-required');
...
export async function installOpnEndpointService(spec: OpnEndpointServiceSpec, platform: NodeJS.Platform = process.platform): Promise<{ platform: 'darwin' | 'win32'; path?: string; command?: string[] }> {
  ...
  if (platform === 'darwin') {
    ...
    if (previous.status !== 0 && !/Could not find service|No such process/i.test(previous.stderr || '')) throw new Error(`opn-endpoint-launchd-replace-failed:${(previous.stderr || '').trim()}`);
  ...
  if (platform === 'win32') {
    ...
    if (started.status !== 0) throw new Error(`opn-endpoint-task-start-failed:${(started.stderr || '').trim()}`);
