  ...
  const server = createPairingHttpServer({
    ...
    ownerOutboundTaskApproval: {
      ...
      async decide({ network_id, approval, decision, human_id, human_note }) {
        ...
        if (decision === 'rejected' || result.status === 'duplicate') return { status: result.status, approval: result.approval };
        if (result.approval.status !== 'approved') return { status: result.status, approval: result.approval };
        if (result.approval.task_artifact_id) {
          const taskArtifact = await input.artifact_store?.read(result.approval.task_artifact_id);
          if (!taskArtifact) throw new Error('outbound-task-artifact-store-unavailable');
