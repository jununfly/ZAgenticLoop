...
const spec: CliSpec = {
  ...
  async handler({ options, io }) {
    if (String(options.command ?? '') !== 'run') throw new Error('opn-graph-verifier-command-invalid');
    ...
    const providerKind = value('provider', 'opn-graph-verifier-provider-required') as 'codex' | 'workbuddy-code';
    if (providerKind !== 'codex' && providerKind !== 'workbuddy-code') throw new Error('opn-graph-verifier-provider-invalid');
    ...
    if (discovery.status !== 'found' || !discovery.executable) throw new Error(`opn-graph-verifier-provider-unavailable:${JSON.stringify(discovery)}`);
    ...
    try {
      ...
      if (!envelope) { io.stdout(JSON.stringify({ schema: 'zj-loop.opn_graph_verifier.v1', status: 'empty', side_effects_executed: false })); return; }
...
if (process.argv[1]?.endsWith('opn-graph-verifier-cli.js')) process.exitCode = await runCli(spec);
