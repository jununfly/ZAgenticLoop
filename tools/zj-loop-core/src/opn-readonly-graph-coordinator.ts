...
export async function receiveOpnReadOnlyGraphVerificationResult(input: {
  ...
  expected: { graph_id: string; network_id: string; plan_id: string; plan_revision: number; task_id: string; plan_digest: string; source_evidence_ref: string; verifier_node_id: string };
  state_store: SqliteStateStore;
  artifact_store: OpnArtifactStore;
  downloadArtifact(artifact_id: string): Promise<Buffer>;
}): Promise<{ status: 'empty' | 'recorded' | 'duplicate' | 'blocked'; message_id?: string; result?: OpnReadOnlyGraphVerificationResult; reason?: string; side_effects_executed: false }> {
  ...
  const blocked = (reason: string) => ({ status: 'blocked' as const, message_id: envelope.message_id, reason, side_effects_executed: false as const });
  if (envelope.target_node_id !== input.coordinator_id || envelope.notification_kind !== 'graph.verification.result' || envelope.artifact_refs.length < 1) return blocked('opn-read-only-graph-verification-result-envelope-invalid');
  const resultRef = envelope.artifact_refs[0].artifact_id;
  if (!DIGEST.test(resultRef) || envelope.artifact_refs[0].content_sha256 !== resultRef) return blocked('opn-read-only-graph-verification-result-artifact-ref-invalid');
  ...
  if (append.status === 'conflict') return blocked('opn-read-only-graph-verification-result-state-conflict');
