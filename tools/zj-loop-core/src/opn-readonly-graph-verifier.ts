...
import { createOpnReadOnlyGraphVerificationResult, type OpnReadOnlyGraphVerificationResult } from './opn-readonly-graph-verification.js';
...
type ProviderResult = { status: 'completed' | 'failed' | 'cancelled' | 'timed-out'; success: boolean; stdout: string; stderr: string };
type ReadOnlyProvider = { run(input: { cwd: string; prompt: string; mode: 'read-only'; env_allowlist: string[]; env: Record<string, string>; timeout_ms: number; termination_grace_ms: number; max_stdout_bytes: number; max_stderr_bytes: number }): Promise<ProviderResult> };

function canonical(value: unknown): string {
  const result = canonicalize(value);
  if (typeof result !== 'string') throw new Error('opn-read-only-graph-verifier-canonicalization-invalid');
  return result;
}

function digest(value: unknown): Digest { return `sha256:${createHash('sha256').update(canonical(value), 'utf8').digest('hex')}`; }
function bytesDigest(value: Uint8Array): Digest { return `sha256:${createHash('sha256').update(value).digest('hex')}`; }
function text(value: unknown): value is string { return typeof value === 'string' && value.trim() !== '' && !value.includes('\0'); }

function sourceEvidence(value: unknown, artifact_id: string): { graph_id: string; plan_digest: Digest; task_id: string; node_id: string; snapshot_digest: Digest; findings: string } {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('opn-read-only-graph-source-evidence-invalid');
  const item = value as Record<string, unknown>;
  if (item.schema !== SOURCE_SCHEMA || !text(item.graph_id) || !text(item.plan_digest) || !DIGEST.test(item.plan_digest) || !text(item.task_id) || !text(item.node_id) || !text(item.snapshot_digest) || !DIGEST.test(item.snapshot_digest) || !text(item.findings)) throw new Error('opn-read-only-graph-source-evidence-invalid');
  if (!DIGEST.test(artifact_id)) throw new Error('opn-read-only-graph-source-evidence-binding-invalid');
  return { graph_id: item.graph_id, plan_digest: item.plan_digest as Digest, task_id: item.task_id, node_id: item.node_id, snapshot_digest: item.snapshot_digest as Digest, findings: item.findings };
}

function verificationEvidence(input: { envelope: TransportEnvelope; source: ReturnType<typeof sourceEvidence>; verifier_node_id: string; provider: ProviderResult; source_evidence_ref: Digest }): Record<string, unknown> {
  return {
...
export async function processOpnReadOnlyGraphVerificationRequest(input: {
  ...
  publishArtifact?: (value: { bytes: Buffer; metadata: Awaited<ReturnType<OpnArtifactStore['put']>>['metadata']; transfer_id: string; target_node_id: string }) => Promise<void>;
  provider: ReadOnlyProvider;
  transport: Pick<TransportAdapter, 'send' | 'acknowledge'>;
  now?: () => string;
}): Promise<{ status: 'processed' | 'blocked'; verification_result?: OpnReadOnlyGraphVerificationResult; result_artifact_id?: Digest; message_id: string; reason?: string; side_effects_executed: false }> {
  const now = input.now ?? (() => new Date().toISOString());
  const envelope = input.envelope;
  if (envelope.target_node_id !== input.verifier_node_id || envelope.notification_kind !== 'graph.verification.request' || envelope.state !== 'available' || envelope.artifact_refs.length !== 1) return { status: 'blocked', message_id: envelope.message_id, reason: 'opn-read-only-graph-verification-request-invalid', side_effects_executed: false };
  const sourceRef = envelope.artifact_refs[0].artifact_id;
  if (!DIGEST.test(sourceRef) || envelope.artifact_refs[0].content_sha256 !== sourceRef) return { status: 'blocked', message_id: envelope.message_id, reason: 'opn-read-only-graph-verification-source-ref-invalid', side_effects_executed: false };

  let source: ReturnType<typeof sourceEvidence>;
  let sourceBytes: Buffer;
  try {
    sourceBytes = await input.downloadArtifact(sourceRef);
    if (bytesDigest(sourceBytes) !== sourceRef) throw new Error('opn-read-only-graph-source-artifact-integrity-invalid');
    source = sourceEvidence(JSON.parse(sourceBytes.toString('utf8')) as unknown, sourceRef);
    if (source.task_id !== envelope.task_id) throw new Error('opn-read-only-graph-source-task-binding-invalid');
  } catch (error) {
  ...
  const evidencePayload = verificationEvidence({ envelope, source, verifier_node_id: input.verifier_node_id, provider, source_evidence_ref: sourceRef as Digest });
  ...
  if (input.publishArtifact) await input.publishArtifact({ bytes: evidenceBytes, metadata: evidence.metadata, transfer_id: `verification-evidence:${envelope.message_id}`, target_node_id: envelope.from_node_id });
  const result = createOpnReadOnlyGraphVerificationResult({ graph_id: source.graph_id, network_id: envelope.network_id, plan_id: envelope.plan_id, plan_revision: envelope.plan_revision, task_id: envelope.task_id, plan_digest: source.plan_digest, source_evidence_ref: sourceRef as Digest, verification_evidence_ref: evidence.metadata.artifact_id as Digest, verifier_node_id: input.verifier_node_id, status: provider.status === 'completed' && provider.success ? 'passed' : 'outcome-uncertain' });
  ...
  if (input.publishArtifact) await input.publishArtifact({ bytes: resultBytes, metadata: resultArtifact.metadata, transfer_id: `verification-result:${envelope.message_id}`, target_node_id: envelope.from_node_id });
  ...
  return { status: 'processed', message_id: envelope.message_id, verification_result: result, result_artifact_id: resultArtifact.metadata.artifact_id as Digest, side_effects_executed: false };
}
