...
export const OPN_TRANSPORT_HTTP_SCHEMA = 'zj-loop.opn_transport_http.v1' as const;
...
async function body(request: IncomingMessage): Promise<Record<string, unknown>> {
  ...
  if (declared > 64 * 1024) throw new Error('transport-request-too-large');
  const chunks: Buffer[] = [];
  let size = 0;
  for await (const chunk of request) {
    const value = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk);
    size += value.length;
    if (size > 64 * 1024) throw new Error('transport-request-too-large');
    chunks.push(value);
  }
  if (size === 0) throw new Error('json-body-required');
  let parsed: unknown;
  try { parsed = JSON.parse(Buffer.concat(chunks).toString('utf8')); } catch { throw new Error('json-invalid'); }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) throw new Error('json-object-required');
  return parsed as Record<string, unknown>;
}

function token(request: IncomingMessage): string | null {
...
function offeredEventId(envelope: TransportEnvelope): string {
  return `${OPN_TRANSPORT_OFFERED_EVENT}:${envelope.message_id}:${envelope.envelope_digest}`;
}

function acknowledgedEventId(envelope: TransportEnvelope): string {
...
export function createOpnTransportHttpService(input: { network_id: string; stateStore: SqliteStateStore; credentialVerifier: CredentialVerifier; now?: () => string; session_ttl_ms?: number }): OpnTransportHttpService {
  if (!input.network_id.trim()) throw new Error('opn-transport-network-id-required');
  if (!input.stateStore) throw new Error('opn-transport-state-store-required');
  if (!input.credentialVerifier) throw new Error('opn-transport-credential-verifier-required');
  const now = input.now ?? (() => new Date().toISOString());
  const sessionTtl = input.session_ttl_ms ?? 50 * 60 * 1000;
  if (!Number.isInteger(sessionTtl) || sessionTtl <= 0) throw new Error('opn-transport-session-ttl-invalid');
  const sessions = new Map<string, Session>();

  async function authorize(request: IncomingMessage, session: Session, node_id: string): Promise<{ status: 'allowed' } | { status: 'blocked'; reason: string }> {
    const bearer = token(request);
    if (!bearer) return { status: 'blocked', reason: 'transport-credential-required' };
    if (session.node_id !== node_id) return { status: 'blocked', reason: 'transport-session-node-mismatch' };
    if (Date.parse(now()) >= Date.parse(session.expires_at)) return { status: 'blocked', reason: 'transport-session-expired' };
    ...
    if (result.status !== 'allowed' || result.credential_id !== session.credential_id) return { status: 'blocked', reason: result.reason ?? 'transport-credential-invalid' };
  ...
  return {
    async handle({ request, response, node_id }) {
      ...
      if (!url.pathname.startsWith('/v1/transport/')) return false;
      ...
      if (request.method === 'POST' && url.pathname === '/v1/transport/sessions') {
        ...
        if (Object.keys(value).sort().join(',') !== 'network_id,node_id,protocol_version' || value.network_id !== input.network_id || value.node_id !== node_id || typeof value.protocol_version !== 'string' || !value.protocol_version.trim()) { blocked(response, 400, 'transport-session-request-invalid'); return true; }
        const bearer = token(request);
        if (!bearer) { blocked(response, 401, 'transport-credential-required'); return true; }
        ...
        if (verification.status !== 'allowed' || !verification.credential_id || !verification.expires_at) { blocked(response, 403, verification.reason ?? 'transport-credential-invalid'); return true; }
        const session: Session = { session_id: `ots_${randomUUID().replaceAll('-', '')}`, network_id: input.network_id, node_id, credential_id: verification.credential_id, expires_at: new Date(Math.min(Date.parse(verification.expires_at), Date.parse(now()) + sessionTtl)).toISOString() };
        sessions.set(session.session_id, session);
        json(response, 201, { schema: OPN_TRANSPORT_HTTP_SCHEMA, status: 'created', session, side_effects_executed: false });
        return true;
      }
      if (!sessionId || !sessionMatch || (action !== null && suffix === null && action !== 'envelopes' && action !== 'ack') || (action === 'envelopes' && suffix !== null)) { blocked(response, 404, 'route-not-found'); return true; }
      const session = sessions.get(sessionId);
      if (!session) { blocked(response, 404, 'transport-session-not-found'); return true; }
      const authorization = await authorize(request, session, node_id);
      if (authorization.status !== 'allowed') { blocked(response, authorization.reason === 'transport-session-expired' ? 410 : 403, authorization.reason); return true; }
      if (request.method === 'DELETE' && action === null) { sessions.delete(sessionId); response.statusCode = 204; response.end(); return true; }
      if (request.method === 'POST' && action === 'envelopes') {
        let value: unknown;
        try { value = await body(request); } catch (error) { blocked(response, 400, error instanceof Error ? error.message : 'json-invalid'); return true; }
        const validation = validateTransportEnvelope(value);
        if (validation.status !== 'valid') { blocked(response, 400, validation.reason); return true; }
        const envelope = value as TransportEnvelope;
        if (envelope.network_id !== input.network_id || envelope.from_node_id !== node_id || envelope.from_node_id === envelope.target_node_id) { blocked(response, 403, envelope.network_id !== input.network_id ? 'transport-network-mismatch' : envelope.from_node_id !== node_id ? 'transport-sender-identity-mismatch' : 'transport-self-target-forbidden'); return true; }
        const result = await appendEnvelope(envelope);
        if (result === 'conflict') { blocked(response, 409, 'transport-message-conflict'); return true; }
      ...
      if (request.method === 'POST' && action === 'ack') {
        let value: Record<string, unknown>;
        try { value = await body(request); } catch (error) { blocked(response, 400, error instanceof Error ? error.message : 'json-invalid'); return true; }
        if (Object.keys(value).sort().join(',') !== 'envelope_digest,message_id' || typeof value.message_id !== 'string' || typeof value.envelope_digest !== 'string') { blocked(response, 400, 'transport-ack-request-invalid'); return true; }
        const message = (await messages()).get(value.message_id);
        if (!message || message.envelope.target_node_id !== node_id || message.envelope.envelope_digest !== value.envelope_digest) { blocked(response, 409, 'transport-ack-message-mismatch'); return true; }
        const result = await appendAck(message.envelope);
        if (result === 'conflict') { blocked(response, 409, 'transport-ack-conflict'); return true; }
