...
export function createPairingOwnerAuthenticator(input: {
  ...
  return {
    authenticate(request) {
      ...
      if (request.action === 'pairing.list' || request.action === 'pairing.inbox' || request.action === 'human.action.list' || request.action === 'human.action.decide' || request.action === 'message.send') return { status: 'allowed', human_id: input.identity.human_id };
