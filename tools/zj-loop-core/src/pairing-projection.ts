...
export type PairingLifecycleRecord =
  | { type: 'pairing-requested'; event_id: string; occurred_at: string; request: PairingRequest; request_digest: string }
  | { type: 'human-approved'; event_id: string; occurred_at: string; request_id: string; request_digest: string; human_id: string; approved_capabilities: string[] }
  | { type: 'pairing-rejected'; event_id: string; occurred_at: string; request_id: string; request_digest: string; reason: string }
  | { type: 'pairing-expired'; event_id: string; occurred_at: string; request_id: string; request_digest: string };
...
export function projectPairingRequests(input: { network_id: string; records: PairingLifecycleRecord[]; now?: string }): PairingRequestProjection[] {
  ...
  for (const record of input.records) {
    ...
    if (record.type === 'pairing-requested') {
      if (record.request.network_id !== input.network_id || projections.has(record.request.request_id)) conflict();
    ...
    if (!projection || projection.network_id !== input.network_id || projection.request_digest !== record.request_digest) continue;
