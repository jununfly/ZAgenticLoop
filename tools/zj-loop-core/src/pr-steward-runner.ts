...
export async function defaultPrStewardRunner(command: string, args: string[] = []) {
  try {
...
export async function readPrStewardIssueFixRequest(path: string) {
  return JSON.parse(await readFile(path, 'utf8'));
}

export function validatePrStewardLiveRequest(input: { request?: any; currentPrHeadSha?: string } = {}) {
  const errors: string[] = [];
  ...
  if (request?.subject?.type !== expectedSubjectType) errors.push(`subject.type must be ${expectedSubjectType}`);
  if (request?.subject?.base_branch !== 'main') {
    errors.push(`subject.base_branch must be main, got ${request?.subject?.base_branch ?? 'missing'}`);
  }
  if (provider === 'gitlab') {
    if (!request?.subject?.mr_iid) errors.push('subject.mr_iid is required');
  ...
  if (!input.currentPrHeadSha) errors.push('current_pr_head_sha is required before live execution');
  ...
  if (request?.source_signal?.source !== expectedSource) errors.push(`source_signal.source must be ${expectedSource}`);
...
export async function executePrStewardLiveRunner(
  plan: any,
  { runner = defaultPrStewardRunner }: { runner?: (command: string, args: string[]) => Promise<any> } = {},
) {
...
function buildActions(input: {
  planMode: string;
  branch: string;
  repairCommands: Array<{ command: string; args: string[] }>;
  verificationCommands: Array<{ command: string; args: string[] }>;
  repairFiles: string[];
  request: any;
}) {
...
function buildRepairPrBody(input: { request: any; repairFiles: string[] }) {
  ...
  return [
    ...
    '## Verification',
    '',
    ...(input.request?.verification_gate?.commands ?? []).map((command: string) => `- ${command}`),
    '',
    '## Safety',
    '',
...
function normalizeRepairFiles(files: string[]) {
