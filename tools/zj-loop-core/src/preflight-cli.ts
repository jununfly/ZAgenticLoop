...
import { evaluateRuntimePreflight, PreflightExecutionLayer } from './preflight.js';
import { findRoute, loadRouteTable } from './route.js';

const EXECUTION_LAYERS = ['report-only', 'review-artifact', 'live-side-effect'] as const;

export async function runPreflightCli(argv: readonly string[] = process.argv.slice(2)): Promise<number> {
  return runCli({
    ...
    async handler({ io, options }) {
      ...
      const result = evaluateRuntimePreflight({
        route,
        executionLayer: String(options.executionLayer) as PreflightExecutionLayer,
        signal,
        runtime: {
      ...
      if (options.json === true) {
        ...
        if (result.stop_signal) io.stdout(`stop: ${result.stop_signal.stop_code} ${result.stop_signal.reason}`);
      }
...
function parseWorkUnits(value: unknown): number {
