...
import { createProviderAuthAuthorityChildProcessLauncher, type ProviderAuthAuthorityExternalProcessLauncher } from './provider-auth-authority-external-process-launcher.js';
...
export type ProviderAuthAuthorityExternalStartController = {
  ...
  stop(): Promise<{ status: 'stopped' } | { status: 'outcome-uncertain'; reason: 'provider-auth-authority-external-close-failed' | 'provider-auth-authority-binding-residue' }>;
};

function matches(binding: ProviderAuthAuthorityBinding, config: ProviderAuthAuthorityStartConfig): boolean {
...
async function waitForBinding(config: ProviderAuthAuthorityStartConfig, timeoutMs: number, pollMs: number): Promise<ProviderAuthAuthorityBinding> {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() <= deadline) {
    if (await exists(config.binding_path)) {
      const binding = await readProviderAuthAuthorityBinding(config.binding_path);
      if (!matches(binding, config)) throw new Error('provider-auth-authority-binding-mismatch');
      return binding;
    }
...
export async function createProviderAuthAuthorityExternalStartController(input: {
  config_path: string;
  create_launcher?: (input: { config_path: string; config: ProviderAuthAuthorityStartConfig }) => Promise<ProviderAuthAuthorityExternalProcessLauncher> | ProviderAuthAuthorityExternalProcessLauncher;
  ...
  if (!Number.isInteger(startupTimeoutMs) || startupTimeoutMs < 1 || !Number.isInteger(pollIntervalMs) || pollIntervalMs < 1) throw new Error('provider-auth-authority-external-start-timeout-invalid');
  let started = false;
  return {
    async start() {
      if (started) throw new Error('provider-auth-authority-external-already-started');
      if (await exists(config.binding_path)) throw new Error('provider-auth-authority-binding-already-exists');
      try {
    ...
    async stop() {
      ...
      started = false;
      try { await launcher.close(); } catch { return { status: 'outcome-uncertain', reason: 'provider-auth-authority-external-close-failed' }; }
