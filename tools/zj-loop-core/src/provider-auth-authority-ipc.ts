...
import { connectUnixFramedJson, createUnixFramedJsonServer, type FramedJsonConnection } from './framed-json-unix.js';
import { createProviderAuthAuthorityIpcFrame, createProviderAuthAuthorityRevokeRequest, createProviderAuthAuthorityRevokeResponse, type ProviderAuthAuthorityIpcFrame, type ProviderAuthAuthorityRevokeRequest, type ProviderAuthAuthorityRevokeResponse, validateProviderAuthAuthorityIpcFrame } from './provider-auth-authority-ipc-protocol.js';
...
export type ProviderAuthAuthorityRevokeClientResult = ProviderAuthAuthorityRevokeResponse | { status: 'blocked' | 'outcome-uncertain'; reason: string };

export async function revokeProviderAuthRefOverIpc(input: ProviderAuthAuthorityRevokeClientInput): Promise<ProviderAuthAuthorityRevokeClientResult> {
...
export function createProviderAuthAuthorityIpcServer(input: { socket_path: string; correlation_id: string; expected_authority_contract_digest: string; verify_peer: (socket: import('node:net').Socket) => Promise<boolean> | boolean; handle_revoke: (request: ProviderAuthAuthorityRevokeRequest) => Promise<ProviderAuthAuthorityRevokeResponse> }): ReturnType<typeof createUnixFramedJsonServer> {
  const states = new WeakMap<FramedJsonConnection, { challenged: boolean; nonce: string; responded: boolean }>();
  return createUnixFramedJsonServer({ socket_path: input.socket_path, correlation_id: input.correlation_id, verify_peer: input.verify_peer, validate: validateProviderAuthAuthorityIpcFrame, on_frames: async (frames, connection) => {
    ...
    for (const frame of frames as ProviderAuthAuthorityIpcFrame[]) {
      if (state.responded) { connection.close(); return; }
      if (frame.kind === 'challenge') {
        if (state.challenged || !frame.nonce || typeof frame.payload?.schema !== 'string' || frame.payload.schema !== CHALLENGE_SCHEMA || frame.payload.nonce !== frame.nonce) { connection.close(); return; }
        state.challenged = true; state.nonce = frame.nonce; continue;
      }
