...
export function createProviderRuntimeIpcCleanupCoordinator(input: ProviderRuntimeIpcCleanupInput): () => Promise<{ status: 'cleaned'; proof_digest: string } | { status: 'uncertain'; reason: string }> {
  return async () => {
    ...
    try {
      ...
      if (frame.network_id !== input.network_id || frame.node_id !== input.node_id || frame.provider_id !== input.provider_id || frame.execution_id !== input.execution_id || frame.attempt !== input.attempt || frame.launch_handle_digest !== handle.handle.handle_digest) return { status: 'uncertain', reason: 'provider-runtime-cleanup-response-binding-mismatch' };
      ...
      if (payload.schema !== CLEANUP_RESPONSE_SCHEMA) return { status: 'uncertain', reason: 'provider-runtime-cleanup-response-invalid' };
...
export function createProviderRuntimeCleanupRequest(input: { correlation_id: string; sequence?: number; handle: ProviderLaunchHandle; network_id: string; node_id: string; provider_id: string; execution_id: string; attempt: number; cleaned_at: string }): ProviderAuthIpcFrame {
