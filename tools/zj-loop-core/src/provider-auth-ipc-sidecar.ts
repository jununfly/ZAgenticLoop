...
export function createProviderAuthRuntimeIpcSidecar(input: {
  ...
  const server = createUnixProviderAuthIpcServer({ socket_path: input.socket_path, correlation_id: input.correlation_id, verify_peer: async (socket) => {
    ...
    if (frame.kind === 'challenge') {
      ...
      if (Object.keys(value).some((key) => !['schema', 'auth_ref_digest', 'contract_digest', 'adapter_contract_digest', 'task'].includes(key)) || value.schema !== LAUNCH_REQUEST_SCHEMA || value.auth_ref_digest !== input.auth_ref.ref_digest || value.contract_digest !== input.contract_digest || value.adapter_contract_digest !== input.adapter_contract_digest || !value.task || typeof value.task !== 'object' || Array.isArray(value.task)) { await error(connection, frame, 'provider-auth-ipc-launch-request-invalid', state.sequence++); return; }
      let launch: Awaited<ReturnType<ProviderAuthRuntime['launch']>>;
      try { launch = await input.runtime.launch({ ref: input.auth_ref, network_id: frame.network_id, node_id: frame.node_id, provider_id: frame.provider_id, execution_id: frame.execution_id, attempt: frame.attempt, contract_digest: input.contract_digest, adapter_contract_digest: input.adapter_contract_digest, issued_at: now(), expires_at: input.auth_ref.expires_at }); } catch { await error(connection, frame, 'provider-auth-ipc-launch-failed', state.sequence++); return; }
