...
export function createProviderAuthRuntimeIpcLauncher(input: {
  ...
  if (!Number.isInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 15 * 60 * 1000) throw new Error('provider-runtime-ipc-invocation-timeout-invalid');
  ...
  const sidecar = createProviderAuthRuntimeIpcSidecar({
    ...
    invoke: async ({ task }) => {
      ...
      if (!cwd.startsWith('/')) throw new Error('provider-runtime-ipc-task-cwd-invalid');
      ...
      const result = await provider.run({ cwd, prompt, mode: modeValue as CodexExecutionMode, env_allowlist: Object.keys(boundedEnv), env: boundedEnv, timeout_ms: timeoutMs, termination_grace_ms: terminationGraceMs, max_stdout_bytes: 10 * 1024 * 1024, max_stderr_bytes: 10 * 1024 * 1024 });
