...
import { createProviderRuntimeArtifactManifest, validateProviderRuntimeArtifactManifest, type ProviderRuntimeArtifactManifest } from './provider-runtime-artifact-manifest.js';
...
export const PROVIDER_RUNTIME_ARTIFACT_APPROVAL_CHALLENGE_SCHEMA = 'zj-loop.provider_runtime_artifact_approval_challenge.v1' as const;
...
type SignatureFacts = { identifier: string; team_id: string | null; code_directory_hash: string; kind: 'ad-hoc' | 'developer-id'; notarized: boolean };

function canonical(value: unknown): string { const result = canonicalize(value); if (typeof result !== 'string') throw new Error('provider-runtime-artifact-bootstrap-canonicalization-invalid'); return result; }
function digest(value: unknown): string { return `sha256:${createHash('sha256').update(canonical(value), 'utf8').digest('hex')}`; }
function bytesDigest(value: Uint8Array): string { return `sha256:${createHash('sha256').update(value).digest('hex')}`; }
...
function platform(value: NodeJS.Platform | undefined): ProviderRuntimeArtifactManifest['platform'] { const current = value ?? process.platform; if (current === 'darwin' || current === 'win32' || current === 'linux') return current; throw new Error('provider-runtime-artifact-bootstrap-platform-unsupported'); }
function challengeDigest(value: Omit<ProviderRuntimeArtifactApprovalChallenge, 'challenge_digest'>): string { return digest(value); }

async function fileDigest(filePath: string): Promise<string> { return bytesDigest(await readFile(filePath)); }
async function adHocSign(filePath: string, identifier: string): Promise<void> { await execFile('/usr/bin/codesign', ['--force', '--sign', '-', '--identifier', identifier, filePath]); }
function syntheticSignature(fileDigestValue: string): SignatureFacts { return { identifier: 'development-local', team_id: null, code_directory_hash: fileDigestValue.slice(7, 47), kind: 'ad-hoc', notarized: false }; }

async function writeJson(filePath: string, value: unknown, mode: number): Promise<void> { await writeFile(filePath, `${JSON.stringify(value, null, 2)}\n`, { mode }); await chmod(filePath, mode); }
...
export async function bootstrapProviderRuntimeArtifact(input: {
  ...
  if (!absolute(input.source_runtime_path) || !absolute(input.source_helper_path) || !absolute(input.artifact_root)) throw new Error('provider-runtime-artifact-bootstrap-path-invalid');
  const checkedConfig = validateProviderRuntimeStartConfig(input.base_config);
  if (checkedConfig.status === 'blocked') throw new Error(`provider-runtime-artifact-bootstrap-config-${checkedConfig.reason}`);
  ...
  if (targetPlatform === 'darwin') { await sign(runtimeArtifactPath, 'com.zagenticloop.development.runtime'); await sign(helperArtifactPath, 'com.zagenticloop.development.runtime'); }
  const runtimeDigest = await fileDigest(runtimeArtifactPath);
  const helperDigest = await fileDigest(helperArtifactPath);
  const inspect = input.inspect_signature ?? (targetPlatform === 'darwin' ? inspectMacOSSignature : async (filePath: string) => syntheticSignature(await fileDigest(filePath)));
  const runtimeSignature = await inspect(runtimeArtifactPath);
  const helperSignature = await inspect(helperArtifactPath);
  if (runtimeSignature.identifier !== helperSignature.identifier || runtimeSignature.team_id !== helperSignature.team_id || runtimeSignature.kind !== helperSignature.kind || runtimeSignature.notarized !== helperSignature.notarized) throw new Error('provider-runtime-artifact-bootstrap-signature-pair-mismatch');
  ...
  if (startCheck.status === 'blocked') throw new Error(`provider-runtime-artifact-bootstrap-start-config-${startCheck.reason}`);
