...
import { readProviderRuntimeArtifactManifest, validateProviderRuntimeArtifactManifest, type ProviderRuntimeArtifactManifest, type ProviderRuntimeArtifactTrustProfile } from './provider-runtime-artifact-manifest.js';

const execFile = promisify(execFileCallback);

export type ProviderRuntimeArtifactVerification = { status: 'verified'; manifest: ProviderRuntimeArtifactManifest } | { status: 'blocked'; reason: string };
...
async function inspectMacOSSignature(filePath: string): Promise<{ identifier: string; team_id: string | null; code_directory_hash: string; kind: 'ad-hoc' | 'developer-id'; notarized: boolean }> {
  ...
  if (!identifier || !cdhash) throw new Error('provider-runtime-macos-signature-metadata-invalid');
...
export function createProviderRuntimeArtifactVerifier(input: {
  ...
  inspect_signature?: (filePath: string) => Promise<{ identifier: string; team_id: string | null; code_directory_hash: string; kind: 'ad-hoc' | 'developer-id'; notarized: boolean }>;
}): { verify(): Promise<ProviderRuntimeArtifactVerification> } {
  if (!absolute(input.manifest_path) || !absolute(input.runtime_artifact_path) || !absolute(input.helper_artifact_path)) throw new Error('provider-runtime-artifact-verifier-path-invalid');
  const platform = input.platform ?? process.platform;
  const inspect = input.inspect_signature ?? inspectMacOSSignature;
  return {
    async verify() {
      let manifest: unknown;
      try { manifest = await readProviderRuntimeArtifactManifest(input.manifest_path); } catch { return { status: 'blocked', reason: 'provider-runtime-artifact-manifest-read-failed' }; }
      ...
      if (value.platform !== platform) return { status: 'blocked', reason: 'provider-runtime-artifact-platform-mismatch' };
      let runtimeDigest: string;
      let helperDigest: string;
      try { runtimeDigest = await fileDigest(input.runtime_artifact_path); helperDigest = await fileDigest(input.helper_artifact_path); } catch { return { status: 'blocked', reason: 'provider-runtime-artifact-read-failed' }; }
      if (runtimeDigest !== value.runtime_artifact_digest || helperDigest !== value.helper_artifact_digest) return { status: 'blocked', reason: 'provider-runtime-artifact-digest-mismatch' };
      if (platform !== 'darwin') return { status: 'verified', manifest: value };
      let runtimeSignature: Awaited<ReturnType<typeof inspect>>;
      let helperSignature: Awaited<ReturnType<typeof inspect>>;
      try { runtimeSignature = await inspect(input.runtime_artifact_path); helperSignature = await inspect(input.helper_artifact_path); } catch { return { status: 'blocked', reason: 'provider-runtime-artifact-signature-unavailable' }; }
      const expected = value.signing;
      if (runtimeSignature.identifier !== expected.identifier || helperSignature.identifier !== expected.identifier || runtimeSignature.team_id !== expected.team_id || helperSignature.team_id !== expected.team_id || runtimeSignature.kind !== expected.kind || helperSignature.kind !== expected.kind || runtimeSignature.code_directory_hash !== value.runtime_code_directory_hash || helperSignature.code_directory_hash !== value.helper_code_directory_hash || runtimeSignature.notarized !== expected.notarized || helperSignature.notarized !== expected.notarized) return { status: 'blocked', reason: 'provider-runtime-artifact-signature-mismatch' };
