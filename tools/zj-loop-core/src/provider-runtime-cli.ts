...
import { bootstrapProviderRuntimeArtifact, type ProviderRuntimeArtifactBootstrapResult } from './provider-runtime-artifact-bootstrap.js';
import { readProviderRuntimeStartConfig } from './provider-runtime-start-config-store.js';
import { readProviderRuntimeArtifactManifest, type ProviderRuntimeArtifactManifest } from './provider-runtime-artifact-manifest.js';
...
import { createProviderRuntimeStartAssembly, type ProviderRuntimeStartAssembly } from './provider-runtime-start-assembly.js';
...
export function runProviderRuntimeCli(argv: readonly string[] = process.argv.slice(2), io: CliIo = defaultCliIo, deps?: {
  ...
  create_start_assembly?: (input: { config: Awaited<ReturnType<typeof readProviderRuntimeStartConfig>>; process_identity_digest: string }) => ProviderRuntimeStartAssembly;
}) : Promise<number> {
  return runCli({
    ...
    async handler({ options }) {
      ...
      if (command === 'bootstrap') {
        const required = (name: string): string => { const value = options[name]; if (typeof value !== 'string' || value.trim() === '') throw new Error(`provider-runtime-bootstrap-${name.replaceAll('_', '-')}-required`); return value; };
        ...
        const result: ProviderRuntimeArtifactBootstrapResult = await bootstrap({ source_runtime_path: required('runtime_source'), source_helper_path: required('helper_source'), artifact_root: required('artifact_root'), base_config: await readStartConfig(required('config')), platform: typeof options.platform === 'string' ? options.platform as NodeJS.Platform : undefined });
        io.stdout(JSON.stringify({ schema: SCHEMA, status: result.status, side_effects_executed: result.side_effects_executed, artifact_path: result.artifact_path, manifest_path: result.manifest_path, manifest_digest: result.manifest.manifest_digest, challenge_path: result.challenge_path, challenge_digest: result.challenge.challenge_digest, start_config_path: result.start_config_path, next_action: 'human-approve' as const }));
        return 0;
      }
      if (command === 'approve') {
        const required = (name: string): string => { const value = options[name]; if (typeof value !== 'string' || value.trim() === '') throw new Error(`provider-runtime-approval-${name.replaceAll('_', '-')}-required`); return value; };
        const challenge = JSON.parse(await readFile(required('challenge'), 'utf8')) as { schema: string; status: string; challenge_id: string; network_id: string; node_id: string; device_id: string; artifact_id: string; manifest_digest: string; artifact_profile: ProviderRuntimeArtifactManifest['profile']; platform: ProviderRuntimeArtifactManifest['platform']; created_at: string; challenge_digest: string };
        if (challenge.schema !== 'zj-loop.provider_runtime_artifact_approval_challenge.v1' || challenge.status !== 'pending') throw new Error('provider-runtime-approval-challenge-invalid');
        const { challenge_digest, ...unsignedChallenge } = challenge;
        const canonicalChallenge = canonicalize(unsignedChallenge);
        if (typeof canonicalChallenge !== 'string' || `sha256:${createHash('sha256').update(canonicalChallenge, 'utf8').digest('hex')}` !== challenge_digest) throw new Error('provider-runtime-approval-challenge-digest-invalid');
        const manifest = await readProviderRuntimeArtifactManifest(required('manifest'));
        if (manifest.manifest_digest !== challenge.manifest_digest || manifest.artifact_id !== challenge.artifact_id || manifest.profile !== challenge.artifact_profile || manifest.platform !== challenge.platform) throw new Error('provider-runtime-approval-challenge-manifest-mismatch');
      ...
      if (command === 'start') {
        ...
        if (!/^sha256:[0-9a-f]{64}$/.test(processIdentityDigest)) throw new Error('provider-runtime-start-process-identity-digest-required');
        const createAssembly = deps?.create_start_assembly ?? ((input: { config: Awaited<ReturnType<typeof readProviderRuntimeStartConfig>>; process_identity_digest: string }) => createProviderRuntimeStartAssembly({ config: input.config, process_identity_digest: input.process_identity_digest, revoke_ref: async () => ({ status: 'blocked' as const, reason: 'provider-runtime-revoke-authority-not-configured' }), create_launcher: ({ runtime, resolver, config, state_store }) => {
          if (!config.macos_helper_path || !config.macos_helper_digest) throw new Error('provider-runtime-start-macos-helper-required');
      ...
      if (!bindingPath) throw new Error('provider-runtime-binding-required');
...
if (process.argv[1] && path.resolve(process.argv[1]) === path.resolve(fileURLToPath(import.meta.url))) process.exitCode = await runProviderRuntimeCli();
