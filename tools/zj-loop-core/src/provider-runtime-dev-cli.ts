...
export function runProviderRuntimeDevCli(argv: readonly string[] = process.argv.slice(2), io: CliIo = defaultCliIo): Promise<number> {
  return runCli({
    ...
    async handler({ options }) {
      ...
      if (command !== 'start') throw new Error('provider-runtime-dev-command-unsupported');
      ...
      if (!runtimeDir.startsWith('/')) throw new Error('provider-runtime-dev-runtime-dir-invalid');
...
if (process.argv[1] && process.argv[1].endsWith('provider-runtime-dev-cli.js')) process.exitCode = await runProviderRuntimeDevCli();
