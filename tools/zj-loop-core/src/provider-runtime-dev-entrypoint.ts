...
import { createInMemoryProviderAuthRuntime, validateProviderAuthRef, type ProviderAuthRef, type ProviderRuntimeIdentityBinding } from './provider-auth-runtime.js';
...
import { createProviderRuntimeServiceBinding, persistProviderRuntimeServiceBinding, validateProviderRuntimeServiceBinding, type ProviderRuntimeServiceBinding } from './provider-runtime-service-binding.js';
import { createInMemoryTrustedRunnerPeerIdentityVerifier, type TrustedRunnerPeerIdentity } from './trusted-runner-peer-identity.js';

export const PROVIDER_RUNTIME_DEV_BINDING_SCHEMA = 'zj-loop.provider_runtime_dev_binding.v1' as const;
...
export type DevelopmentProviderRuntime = {
  start(): Promise<{ status: 'started'; binding: ProviderRuntimeServiceBinding; dev_binding: ProviderRuntimeDevBinding }>;
  close(): Promise<void>;
};

function canonical(value: unknown): string {
  const result = canonicalize(value);
  if (typeof result !== 'string') throw new Error('provider-runtime-dev-canonicalization-invalid');
  return result;
}
...
function absolute(value: string): boolean { return value.startsWith('/'); }

export function validateProviderRuntimeDevBinding(value: unknown): { status: 'valid'; binding: ProviderRuntimeDevBinding } | { status: 'blocked'; reason: string } {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return { status: 'blocked', reason: 'provider-runtime-dev-binding-invalid' };
  const item = value as Record<string, unknown>;
  if (item.schema !== PROVIDER_RUNTIME_DEV_BINDING_SCHEMA || item.profile !== 'development-local' || item.warning !== 'development-only-in-memory-auth-authority' || typeof item.auth_ref_path !== 'string' || !absolute(item.auth_ref_path) || typeof item.correlation_id !== 'string' || item.correlation_id.trim() === '' || typeof item.dev_binding_path !== 'string' || !absolute(item.dev_binding_path) || !item.binding || typeof item.binding !== 'object') return { status: 'blocked', reason: 'provider-runtime-dev-binding-invalid' };
  if (validateProviderRuntimeServiceBinding(item.binding).status === 'blocked') return { status: 'blocked', reason: 'provider-runtime-dev-service-binding-invalid' };
  if (validateProviderAuthRef(item.auth_ref).status === 'blocked' || !DIGEST.test(String((item.auth_ref as Record<string, unknown>).ref_digest))) return { status: 'blocked', reason: 'provider-runtime-dev-auth-ref-invalid' };
  return { status: 'valid', binding: item as ProviderRuntimeDevBinding };
}

function validateConfig(input: DevelopmentProviderRuntimeConfig): void {
  if (input.profile !== 'development-local') throw new Error('provider-runtime-dev-profile-required');
  if (![input.network_id, input.runtime_id, input.provider_id, input.node_id, input.execution_id, input.provider_secret].every((value) => typeof value === 'string' && value.trim() !== '' && !value.includes('\0'))) throw new Error('provider-runtime-dev-config-invalid');
  if (!Number.isInteger(input.attempt) || input.attempt < 1) throw new Error('provider-runtime-dev-attempt-invalid');
  if (![input.socket_path, input.binding_path, input.auth_ref_path, input.provider_executable, input.working_directory].every(absolute)) throw new Error('provider-runtime-dev-path-invalid');
  if (input.auth_ref_ttl_ms !== undefined && (!Number.isInteger(input.auth_ref_ttl_ms) || input.auth_ref_ttl_ms < 1 || input.auth_ref_ttl_ms > 24 * 60 * 60 * 1000)) throw new Error('provider-runtime-dev-auth-ref-ttl-invalid');
  if (input.provider_auth_env !== undefined && !/^[A-Za-z_][A-Za-z0-9_]*$/.test(input.provider_auth_env)) throw new Error('provider-runtime-dev-provider-auth-env-invalid');
}

export function createDevelopmentProviderRuntime(input: DevelopmentProviderRuntimeConfig, options: { peer_identity_digest?: string; peer_process_id?: number | null; now?: () => string } = {}): DevelopmentProviderRuntime {
  validateConfig(input);
  const now = options.now ?? (() => new Date().toISOString());
  const peerDigest = options.peer_identity_digest ?? 'a'.repeat(64);
  if (!/^[0-9a-f]{64}$/.test(peerDigest)) throw new Error('provider-runtime-dev-peer-identity-digest-invalid');
  const peerIdentity: TrustedRunnerPeerIdentity = { schema: 'zj-loop.trusted_runner_peer_identity.v1', platform: process.platform === 'darwin' ? 'darwin' : process.platform === 'win32' ? 'win32' : 'linux', kind: process.platform === 'darwin' ? 'process-audit' : process.platform === 'win32' ? 'named-pipe-token' : 'peer-credentials', identity_digest: peerDigest, process_id: options.peer_process_id ?? null };
  const runtimeBinding: ProviderRuntimeIdentityBinding = {
  ...
  let launcher: ReturnType<typeof createProviderAuthRuntimeIpcLauncher> | undefined;
  let started = false;
  let binding: ProviderRuntimeServiceBinding | undefined;
  let devBinding: ProviderRuntimeDevBinding | undefined;
  ...
  return {
    async start() {
      if (started) throw new Error('provider-runtime-dev-already-started');
      ...
      if (issued.status === 'blocked') throw new Error(issued.reason);
      ...
      if (readiness.status === 'blocked') { await launcher.close(); launcher = undefined; throw new Error(readiness.reason); }
    ...
    async close() {
      ...
      launcher = undefined; binding = undefined; devBinding = undefined; started = false;
    },
  };
}
