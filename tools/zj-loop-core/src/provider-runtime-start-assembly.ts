...
export function createProviderRuntimeStartAssembly(input: {
  ...
  create_launcher: (input: { runtime: ProviderAuthRuntime; resolver: ProviderAuthRefResolver; config: ProviderRuntimeStartConfig; state_store: SqliteStateStore }) => ProviderAuthRuntimeIpcLauncher;
  process_id?: number;
  now?: () => string;
