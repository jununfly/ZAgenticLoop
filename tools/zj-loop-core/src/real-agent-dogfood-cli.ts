...
async function bindAdmission(options: Record<string, string | boolean | undefined>) {
  ...
  try {
    ...
    if (!admissionBindingsEqual(replayed.binding, admission.binding)) throw new Error('trusted-runner-admission-binding-provenance-mismatch');
...
async function resume(options: Record<string, string | boolean | undefined>) {
  ...
  try {
    ...
    let envelope: { schema?: unknown; dogfood_id?: unknown; execution_id?: unknown; attempt?: unknown; lifecycle_revision?: unknown; policy_digest?: unknown; approval_summary_digest?: unknown; admission_digest?: unknown; approval?: HumanApprovalContext; identity?: HumanPublicIdentity };
    try {
    ...
    const summaryPath = path.join(evidenceStore, `${dogfoodId}.approval-summary.json`);
    const summary = JSON.parse(await readFile(summaryPath, 'utf8')) as { summary_digest?: unknown; admission_digest?: unknown; policy_digest?: unknown; lifecycle_revision?: unknown; goal?: unknown; executable?: unknown; worktree_path?: unknown };
