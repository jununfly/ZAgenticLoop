...
type ProviderRuntimeIpcBinding = { socket_path: string; correlation_id?: string; timeout_ms?: number; contract_digest: string; adapter_contract_digest: string; runtime_binding: { runtime_identity_fingerprint: string; runtime_manifest_digest: string; provider_capabilities_digest: string } };
...
async function start(options: Record<string, string | boolean | undefined>) {
  ...
  if (configuredNetworkId && !/^[A-Za-z0-9._:-]{1,128}$/.test(configuredNetworkId)) throw new Error('network-id-invalid');
...
async function bindAdmission(options: Record<string, string | boolean | undefined>) {
  ...
  try {
    ...
    let runtimeIpc: ProviderRuntimeIpcBinding | undefined;
    const runtimeIpcPath = typeof options['provider-runtime-ipc'] === 'string' ? options['provider-runtime-ipc'] : '';
    if (runtimeIpcPath) {
      try { runtimeIpc = JSON.parse(await readFile(runtimeIpcPath, 'utf8')) as ProviderRuntimeIpcBinding; } catch { throw new Error('provider-runtime-ipc-invalid'); }
      if (!runtimeIpc || typeof runtimeIpc.socket_path !== 'string' || !runtimeIpc.socket_path.trim() || !DIGEST.test(runtimeIpc.contract_digest) || !DIGEST.test(runtimeIpc.adapter_contract_digest) || JSON.stringify(runtimeIpc.runtime_binding) !== JSON.stringify(admission.binding.runtime_binding)) throw new Error('provider-runtime-ipc-binding-invalid');
    }
...
async function resume(options: Record<string, string | boolean | undefined>) {
  ...
  try {
    ...
    const attemptApprovalPath = (attempt: number): string => path.join(evidenceStore, attempt > 1 ? `${approvalId}.attempt-${attempt}.json` : `${approvalId}.json`);
    try {
      envelope = JSON.parse(await readFile(attemptApprovalPath(lifecycle.attempt), 'utf8')) as typeof envelope;
    } catch {
...
async function retry(options: Record<string, string | boolean | undefined>) {
  const dogfoodId = required(options, 'dogfood-id');
  const networkId = required(options, 'network-id');
  const defaults = defaultRealAgentDogfoodRuntimePaths();
  const evidenceStore = await canonicalPath(typeof options['evidence-store'] === 'string' ? options['evidence-store'] : defaults.evidence_store);
  ...
  try {
    const snapshot = await stateStore.readEvents({ network_id: networkId, aggregate_type: 'real-agent-dogfood', aggregate_id: dogfoodId });
    const lifecycle = projectRealAgentDogfoodLifecycle(snapshot.events as unknown as RealAgentDogfoodEvent[]);
    if (lifecycle.dogfood_id !== dogfoodId || lifecycle.network_id !== networkId) throw new Error('retry-lifecycle-binding-invalid');
    if (!['blocked', 'outcome-uncertain', 'request-revision'].includes(lifecycle.status)) throw new Error('retry-lifecycle-status-invalid');
    const summaryPath = path.join(evidenceStore, `${dogfoodId}.approval-summary.json`);
    const previousSummaryText = await readFile(summaryPath, 'utf8');
    const previousSummary = JSON.parse(previousSummaryText) as Record<string, unknown>;
    if (previousSummary.dogfood_id !== dogfoodId || previousSummary.network_id !== networkId || previousSummary.execution_id !== lifecycle.execution_id || previousSummary.attempt !== lifecycle.attempt || typeof previousSummary.summary_digest !== 'string') throw new Error('retry-summary-binding-invalid');
    ...
    if (!/^[A-Za-z0-9._:-]{1,256}$/.test(nextExecutionId) || nextExecutionId === lifecycle.execution_id) throw new Error('retry-execution-id-invalid');
    ...
    if (retryReason.length > 2048) throw new Error('retry-reason-invalid');

    let nextGraphPlan: RealAgentDogfoodGraphPlan | undefined;
    if (previousSummary.graph_plan && typeof previousSummary.graph_plan === 'object') {
      nextGraphPlan = createRealAgentDogfoodGraphPlan({ ...(previousSummary.graph_plan as RealAgentDogfoodGraphPlan), attempt: nextAttempt, execution_id: nextExecutionId });
      const graphPlanPath = typeof options['graph-plan'] === 'string' ? options['graph-plan'].trim() : '';
      if (!graphPlanPath) throw new Error('retry-graph-plan-path-required');
    ...
    try { await readFile(archivedSummaryPath, 'utf8'); } catch { await writeFile(archivedSummaryPath, previousSummaryText, { mode: 0o600, flag: 'wx' }); }
    const now = new Date().toISOString();
    ...
    if (result.status === 'conflict' || result.revision === undefined) throw new Error('retry-draft-record-conflict');
    ...
    if (result.status === 'conflict' || result.revision === undefined) throw new Error('retry-preflight-record-conflict');
    revision = result.revision;
    ...
    if (result.status === 'conflict' || result.revision === undefined) throw new Error('retry-approval-record-conflict');
    revision = result.revision;
...
async function preflight(options: Record<string, string | boolean | undefined>) {
  const planPath = required(options, 'graph-plan');
  const plan = createRealAgentDogfoodGraphPlan(JSON.parse(await readFile(planPath, 'utf8')) as RealAgentDogfoodGraphPlan);
  ...
  let providerRuntimeIpc: { socket_path: string; contract_digest: string };
  try { providerRuntimeIpc = JSON.parse(runtimeValue) as { socket_path: string; contract_digest: string }; } catch { throw new Error('provider-runtime-ipc-json-invalid'); }
  const evidenceStore = await createContentAddressedEvidenceStore({ root: evidencePath, initialize: false });
...
async function conformance(options: Record<string, string | boolean | undefined>) {
  const plan = createRealAgentDogfoodGraphPlan(JSON.parse(await readFile(required(options, 'graph-plan'), 'utf8')) as RealAgentDogfoodGraphPlan);
