...
export const REAL_AGENT_DOGFOOD_DIGEST_PROFILE = 'zj-loop.real-agent-dogfood-digest.v1' as const;
const DIGEST = /^sha256:[0-9a-f]{64}$/;

function digest(value: unknown): string {
  const encoded = canonicalize(value);
  if (typeof encoded !== 'string') throw new Error('real-agent-dogfood-digest-canonicalization-invalid');
  return `sha256:${createHash('sha256').update(encoded, 'utf8').digest('hex')}`;
}

function requireDigest(value: string, name: string): void {
  if (!DIGEST.test(value)) throw new Error(`${name}-invalid`);
}
...
export function realAgentDogfoodExecutionBindingDigest(input: {
  ...
  if (!input.execution_id.trim() || !Number.isInteger(input.attempt) || input.attempt < 1 || !input.provider_id.trim() || !Array.isArray(input.resource_scope) || input.resource_scope.some((value) => !value.trim()) || !input.network_policy.trim() || !Number.isInteger(input.timeout_ms) || input.timeout_ms <= 0) throw new Error('execution-binding-input-invalid');
  return digest({
...
export function realAgentDogfoodCoordinatorLeaseDigest(input: {
  ...
  if (!input.execution_id.trim() || !input.session_id.trim() || !input.lease_id.trim() || !input.human_id.trim() || !input.coordinator_id.trim() || !Number.isFinite(Date.parse(input.expires_at))) throw new Error('coordinator-lease-digest-input-invalid');
  return digest({
