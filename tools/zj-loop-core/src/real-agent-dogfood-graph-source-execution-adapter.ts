import { createRealAgentDogfoodExecutionBinding, createRealAgentDogfoodExecutionBindingDigest, type RealAgentDogfoodExecutionBinding } from './real-agent-dogfood-binding.js';
...
export function createRealAgentDogfoodGraphSourceExecutionAdapter(input: {
  ...
  return async () => {
    if (input.lifecycle.status !== 'running') return { status: 'outcome-uncertain', reason: 'source-execution-lifecycle-not-running' };
    if (input.lifecycle.execution_id !== input.plan.execution_id || input.lifecycle.attempt !== input.plan.attempt) return { status: 'outcome-uncertain', reason: 'source-execution-lifecycle-binding-invalid' };
    if (input.admission_bound_execution.preflight.cwd !== input.plan.source_worktree || input.admission_bound_execution.execution.execution_id !== input.plan.execution_id || input.admission_bound_execution.execution.attempt !== input.plan.attempt) return { status: 'outcome-uncertain', reason: 'source-execution-admission-binding-invalid' };
    ...
    } catch { return { status: 'outcome-uncertain', reason: 'source-execution-binding-unavailable' }; }
    if (expectedBindingDigest !== input.execution_binding_digest) return { status: 'outcome-uncertain', reason: 'source-execution-binding-digest-mismatch' };
    ...
    if (lease.status !== 'acquired' && lease.status !== 'reused' && lease.status !== 'renewed') return { status: 'outcome-uncertain', reason: lease.status === 'blocked' ? `source-execution-${lease.reason}` : 'source-execution-worker-lease-unavailable' };
    let binding: RealAgentDogfoodExecutionBinding;
    try { binding = await createRealAgentDogfoodExecutionBinding({ executable: input.executable, args: [...input.args], cwd: input.plan.source_worktree, worktree_path: input.plan.source_worktree, lease_id: lease.lease_id }); }
    catch { return { status: 'outcome-uncertain', reason: 'source-execution-binding-unavailable' }; }
    if (binding.execution_binding_digest !== input.execution_binding_digest) return { status: 'outcome-uncertain', reason: 'source-execution-binding-digest-mismatch' };

    let result: RealAgentDogfoodWorkerResult;
    try {
      result = await (input.worker_runner ?? executeRealAgentDogfoodWorker)({ stateStore: input.state_store, evidenceStore: input.evidence_store, lifecycle: input.lifecycle, worker_id: input.worker_id, lease_id: lease.lease_id, binding, admission_bound_execution: input.admission_bound_execution, worktree_path: input.plan.source_worktree, executable: input.executable, goal: input.goal, execution_mode: input.plan.execution_mode as CodexExecutionMode, git_scope: { repo_root: input.plan.source_worktree, baseline_commit: input.plan.baseline_commit, allowed_files: [...input.plan.allowed_files] }, provider: input.provider, provider_cleanup: input.provider_cleanup, post_run_proof_factory: input.post_run_proof_factory, expected_revision: lease.revision, now: input.now });
    } catch { return { status: 'outcome-uncertain', reason: 'source-execution-worker-outcome-uncertain' }; }
    ...
    return { status, ...(reason ? { reason } : {}), ...(evidence ? { evidence_digest: evidence } : {}), ...(record ? { record } : {}) };
  };
}
