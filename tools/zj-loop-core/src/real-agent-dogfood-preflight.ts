...
export type RealAgentDogfoodPreflightCheck = { id: string; status: 'passed' | 'blocked'; reason?: string };
...
function canonical(value: unknown): string {
  ...
  if (typeof json !== 'string') throw new Error('real-agent-dogfood-preflight-canonicalization-invalid');
  return json;
}

function digest(value: unknown): string { return `sha256:${createHash('sha256').update(canonical(value), 'utf8').digest('hex')}`; }
function check(id: string, status: 'passed' | 'blocked', reason?: string): RealAgentDogfoodPreflightCheck { return { id, status, ...(reason ? { reason } : {}) }; }
...
async function readableFile(filePath: string, executable = false): Promise<boolean> {
  try { await access(filePath, executable ? constants.X_OK : constants.R_OK); return true; } catch { return false; }
}
...
export async function preflightRealAgentDogfood(input: {
  ...
  provider_runtime_ipc: { socket_path: string; contract_digest: string };
  human: { human_id: string; key_tag: string; helper_path: string };
  platform?: NodeJS.Platform;
}): Promise<RealAgentDogfoodPreflightResult> {
  const checks: RealAgentDogfoodPreflightCheck[] = [];
  const platform = input.platform ?? process.platform;
  const plan = createRealAgentDogfoodGraphPlan(input.plan);
  let head: string | undefined;
  try { head = await gitHead(input.repo_root); checks.push(check('baseline-commit', head === plan.baseline_commit ? 'passed' : 'blocked', head === plan.baseline_commit ? undefined : 'baseline-commit-drift')); }
  catch { checks.push(check('baseline-commit', 'blocked', 'git-repository-unavailable')); }
  ...
  if (DIGEST.test(input.conformance_evidence_digest)) {
    try {
      const value = JSON.parse((await input.evidence_store.readOnly({ digest: input.conformance_evidence_digest })).toString('utf8')) as { schema?: unknown; status?: unknown; plan_digest?: unknown; core_commit?: unknown; test_command?: unknown; failure_matrix_digest?: unknown; digest_profile?: unknown; exit_code?: unknown };
  ...
  const unsigned = { schema: 'zj-loop.real_agent_dogfood_preflight.v1' as const, status: checks.every((item) => item.status === 'passed') ? 'execution-ready' as const : 'blocked' as const, side_effects_executed: false as const, plan_digest: plan.plan_digest, plan_definition_digest: plan.plan_definition_digest, checks };
