...
export async function replayRealAgentDogfoodGraphReadModel(input: {
  ...
  const status: RealAgentDogfoodGraphReplayReadModel['status'] = failures.length > 0 ? 'outcome-uncertain' : phase?.status === 'blocked' ? 'blocked' : phase?.status === 'outcome-uncertain' ? 'outcome-uncertain' : lifecycle.status === 'accepted' && phase?.completed_phases.at(-1) === 'cleanup' ? 'passed' : 'in-progress';
