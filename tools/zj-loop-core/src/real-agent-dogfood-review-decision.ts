...
import { getRealAgentDogfoodReviewWarningIds, validateRealAgentDogfoodReviewPackage, type RealAgentDogfoodReviewPackage } from './real-agent-dogfood-review-package.js';
...
export type RealAgentDogfoodReviewRevisionRequirement = { requirement_id: string; description: string; evidence_refs: string[] };
export type RealAgentDogfoodReviewDecision = { schema: typeof REAL_AGENT_DOGFOOD_REVIEW_DECISION_SCHEMA; package_digest: string; lifecycle_revision: number; human_id: string; signer_fingerprint: string; decision: 'accept' | 'reject' | 'request-revision'; comment: string; acknowledged_warning_ids: string[]; revision_requirements: RealAgentDogfoodReviewRevisionRequirement[]; decided_at: string; canonical_payload_digest: string; signature: HumanSignature; side_effects_executed: false };
...
function normalizeWarningIds(value: string[] | undefined): string[] {
  const ids = value ?? [];
  if (!Array.isArray(ids) || ids.some((id) => !text(id)) || new Set(ids).size !== ids.length) throw new Error('warning-acknowledgement-invalid');
  return [...ids].sort();
}

function normalizeRevisionRequirements(value: RealAgentDogfoodReviewRevisionRequirement[] | undefined): RealAgentDogfoodReviewRevisionRequirement[] {
  const requirements = value ?? [];
  if (!Array.isArray(requirements) || requirements.length > MAX_REQUIREMENTS || requirements.some((requirement) => !requirement || !text(requirement.requirement_id) || !text(requirement.description) || !Array.isArray(requirement.evidence_refs) || !requirement.evidence_refs.every((ref) => DIGEST.test(ref))) || new Set(requirements.map((requirement) => requirement.requirement_id)).size !== requirements.length) throw new Error('revision-requirements-invalid');
...
export async function createRealAgentDogfoodReviewDecision(input: { signer: HumanSigner; review_package: RealAgentDogfoodReviewPackage; decision: RealAgentDogfoodReviewDecision['decision']; comment: string; acknowledged_warning_ids?: string[]; revision_requirements?: RealAgentDogfoodReviewRevisionRequirement[]; decided_at: string }): Promise<RealAgentDogfoodReviewDecision> {
  ...
  if (input.decision === 'accept' && input.review_package.decisionability !== 'ready') throw new Error('real-agent-dogfood-review-package-not-decisionable');
  ...
  if (input.decision === 'accept' && JSON.stringify(acknowledged_warning_ids) !== JSON.stringify(warningIds)) throw new Error('warning-acknowledgement-required');
  if (input.decision !== 'request-revision' && revision_requirements.length > 0) throw new Error('revision-requirements-unexpected');
  if (input.decision === 'request-revision' && revision_requirements.length === 0) throw new Error('revision-requirements-required');
...
export function validateRealAgentDogfoodReviewDecision(input: { decision: RealAgentDogfoodReviewDecision; identity: HumanSignerIdentity; review_package: RealAgentDogfoodReviewPackage }): { status: 'valid' | 'blocked'; errors: string[] } {
  ...
  if (value.decision === 'accept' && JSON.stringify(value.acknowledged_warning_ids) !== JSON.stringify(getRealAgentDogfoodReviewWarningIds(input.review_package))) errors.push('warning-acknowledgement-required');
  if (value.decision !== 'request-revision' && value.revision_requirements?.length > 0) errors.push('revision-requirements-unexpected');
  if (value.decision === 'request-revision' && value.revision_requirements?.length === 0) errors.push('revision-requirements-required');
