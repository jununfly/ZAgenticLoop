...
export async function executeRealAgentDogfoodWorker(input: { stateStore: SqliteStateStore; evidenceStore: ContentAddressedEvidenceStore; lifecycle: RealAgentDogfoodLifecycle; worker_id: string; lease_id: string; binding: RealAgentDogfoodExecutionBinding; worktree_path: string; executable: string; goal: string; provider: Provider; post_run_proof?: RealAgentDogfoodPostRunProof; expected_revision: number; now?: string }): Promise<RealAgentDogfoodWorkerResult> {
  ...
  else if (!input.post_run_proof) { to = 'outcome-uncertain'; reasonCode = 'post-run-proof-missing-or-invalid'; nextAction = 'human-reconcile-execution'; }
  else {
