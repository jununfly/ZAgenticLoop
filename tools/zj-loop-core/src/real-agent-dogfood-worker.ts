...
// Keep the worker lease above the Provider Registry's 15-minute execution window,
// leaving room for cleanup, proof collection, and slow local IPC.
export const REAL_AGENT_DOGFOOD_WORKER_DEFAULT_LEASE_TTL_MS = 20 * 60 * 1000;
...
export type RealAgentDogfoodWorkerLeaseResult =
  ...
  | { status: 'released'; lease_id: string; worker_id: string; revision: number }
  | { status: 'abandoned'; lease_id: string; worker_id: string; revision: number }
  | { status: 'blocked'; reason: 'worker-lease-expired' | 'worker-lease-mismatch' | 'worker-lease-released' | 'worker-lease-abandoned' };
...
function latest(events: StateEvent[]): { fact: LeaseFact; revision: number } | null {
  ...
  if (fact.schema !== REAL_AGENT_DOGFOOD_WORKER_LEASE_SCHEMA || fact.network_id !== event.network_id || fact.execution_id !== event.aggregate_id || !['acquired', 'renewed', 'released', 'abandoned'].includes(fact.operation) || !/^sha256:[0-9a-f]{64}$/.test(fact.execution_binding_digest) || !/^sha256:[0-9a-f]{64}$/.test(fact.worker_lease_digest)) throw new Error('worker-lease-fact-invalid');
...
export async function acquireRealAgentDogfoodWorkerLease(input: { stateStore: SqliteStateStore; network_id: string; execution_id: string; worker_id: string; execution_binding_digest: string; now?: string; ttl_ms?: number }): Promise<RealAgentDogfoodWorkerLeaseResult> {
  ...
  if (current) {
    ...
    if (current.fact.execution_binding_digest !== input.execution_binding_digest) return { status: 'blocked', reason: 'worker-lease-mismatch' };
...
export async function renewRealAgentDogfoodWorkerLease(input: { stateStore: SqliteStateStore; network_id: string; execution_id: string; lease_id: string; worker_id: string; execution_binding_digest: string; expected_revision: number; now?: string; ttl_ms?: number }): Promise<RealAgentDogfoodWorkerLeaseResult> {
  ...
  if (current.fact.operation === 'abandoned' || Date.parse(now) >= Date.parse(current.fact.expires_at)) return { status: 'blocked', reason: current.fact.operation === 'abandoned' ? 'worker-lease-abandoned' : 'worker-lease-expired' };
...
export async function releaseRealAgentDogfoodWorkerLease(input: { stateStore: SqliteStateStore; network_id: string; execution_id: string; lease_id: string; worker_id: string; execution_binding_digest: string; expected_revision: number; now?: string }): Promise<RealAgentDogfoodWorkerLeaseResult> {
  const now = input.now ?? new Date().toISOString();
  const current = await readLease(input);
  if (!current || current.fact.lease_id !== input.lease_id || current.fact.worker_id !== input.worker_id || current.fact.execution_binding_digest !== input.execution_binding_digest) return { status: 'blocked', reason: 'worker-lease-mismatch' };
  if (current.fact.operation === 'released') return { status: 'blocked', reason: 'worker-lease-released' };
  ...
  if (result.status !== 'recorded') return { status: 'blocked', reason: 'worker-lease-mismatch' };
  return { status: 'released', lease_id: input.lease_id, worker_id: input.worker_id, revision: result.revision as number };
}

export async function abandonRealAgentDogfoodWorkerLease(input: { stateStore: SqliteStateStore; network_id: string; execution_id: string; lease_id: string; worker_id: string; execution_binding_digest: string; expected_revision: number; now?: string }): Promise<RealAgentDogfoodWorkerLeaseResult> {
  const now = input.now ?? new Date().toISOString();
  const current = await readLease(input);
  if (!current || current.fact.lease_id !== input.lease_id || current.fact.worker_id !== input.worker_id || current.fact.execution_binding_digest !== input.execution_binding_digest) return { status: 'blocked', reason: 'worker-lease-mismatch' };
  if (current.fact.operation === 'released') return { status: 'blocked', reason: 'worker-lease-released' };
  if (current.fact.operation === 'abandoned') return { status: 'blocked', reason: 'worker-lease-abandoned' };
  if (Date.parse(now) < Date.parse(current.fact.expires_at)) return { status: 'blocked', reason: 'worker-lease-expired' };
  const result = await input.stateStore.appendEvent({
    network_id: input.network_id,
    expected_revision: input.expected_revision,
  ...
  if (result.status !== 'recorded') return { status: 'blocked', reason: 'worker-lease-mismatch' };
  return { status: 'abandoned', lease_id: input.lease_id, worker_id: input.worker_id, revision: result.revision as number };
}
