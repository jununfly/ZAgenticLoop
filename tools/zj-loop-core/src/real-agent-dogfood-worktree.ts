...
export type RealAgentDogfoodWorktreeResult =
  | { status: 'prepared' | 'reused'; execution_id: string; branch: string; worktree_path: string; base_commit: string }
...
export async function prepareRealAgentDogfoodWorktree(input: { repo_root: string; worktree_root: string; execution_id: string }): Promise<RealAgentDogfoodWorktreeResult> {
  if (!/^[A-Za-z0-9_-]{1,256}$/.test(input.execution_id)) return { status: 'blocked', reason: 'execution-id-invalid' };
  if (!path.isAbsolute(input.repo_root) || !path.isAbsolute(input.worktree_root)) return { status: 'blocked', reason: 'absolute-paths-required' };
  let repo: string;
  ...
  } catch { return { status: 'blocked', reason: 'repo-unreadable' }; }
  if (inside(worktreeRoot, repo)) return { status: 'blocked', reason: 'worktree-root-inside-repo' };
  ...
  try {
    ...
    if (dirty) return { status: 'blocked', reason: 'repo-not-clean' };
    const baseCommit = await git(['-C', repo, 'rev-parse', 'HEAD'], repo);
    if (!/^[0-9a-f]{40}$/i.test(baseCommit)) return { status: 'blocked', reason: 'base-commit-invalid' };
    try {
      ...
      if (currentCommit !== baseCommit || currentBranch !== branch) return { status: 'blocked', reason: 'existing-worktree-binding-mismatch' };
