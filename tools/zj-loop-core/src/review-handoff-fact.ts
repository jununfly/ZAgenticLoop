...
export const REVIEW_HANDOFF_RECORDED_SCHEMA = 'zj-loop.review_handoff_recorded.v1' as const;
...
function scopeId(handoff: ReviewHandoffRecord): string {
...
function eventId(handoff: ReviewHandoffRecord): string {
...
export async function recordReviewHandoff(input: {
  ...
  const result = await input.stateStore.runAtomic((transaction) => {
    const rows = transaction.database.prepare(
      "SELECT event_id, payload_json FROM state_events WHERE network_id = ? AND aggregate_type = 'review-handoff' AND aggregate_id = ? AND event_type IN ('review-handoff.accepted', 'review-handoff.blocked') ORDER BY revision LIMIT 1",
    ).all(handoff.network_id, aggregate_id) as Array<{ event_id: string; payload_json: string }>;
    if (rows.length > 0) {
      const payload = JSON.parse(rows[0].payload_json) as { handoff?: { handoff_digest?: string } };
      return payload.handoff?.handoff_digest === handoff.handoff_digest && rows[0].event_id === event_id
        ...
        : { status: 'conflict' as const, event_id, current_revision: input.expected_revision, reason: 'review-handoff-conflict' };
    }
