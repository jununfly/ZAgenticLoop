...
import { CliOptionSpec, runCli } from './cli.js';
...
const SHARED_OPTIONS: CliOptionSpec[] = [
...
export async function runRouteConsumerCli(
  config: RouteConsumerCliConfig,
  ...
  return runCli({
    ...
    async handler({ io, options }) {
      ...
      if (options.json === true) {
        ...
        for (const step of plan.next_steps) io.stdout(`next step: ${step}`);
      }
