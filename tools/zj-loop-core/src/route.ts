...
async function collectFixRunnerPromotionEvidence(input: {
  ...
  if (
    input.route.request_kind === 'issue-fix-request' &&
  ...
  if (
    (input.route.execution_mode === 'live' && input.route.recent_success_evidence.length > 0) ||
  ...
  if (
    lowerState.includes('live runner') &&
  ...
  if (
    (lowerState.includes('verification gate') || lowerState.includes('verifier') || input.route.capability_verifiers.length > 0) &&
    (lowerState.includes('repair-pr') || lowerState.includes('escalation-issue'))
  ) {
  ...
  if (
    input.route.guards?.auto_merge === false &&
    (
...
async function collectDraftConsumerPromotionEvidence(input: {
  ...
  if (
    lowerState.includes('draft request') ||
  ...
  if (
    lowerState.includes('draft lifecycle') ||
    lowerState.includes('draft-request-candidate ->') ||
    lowerState.includes('draft request candidate')
  ) {
  ...
  if (
    lowerState.includes('reviewable draft outcome') &&
  ...
  if (
    input.route.guards?.tag_allowed === false &&
