...
export function evaluateScheduleHealth(input: any) {
  ...
  if (now.getTime() < activeWindow.start.getTime() + graceMs) return { ...base, status: 'not_due', expected_window: { start: activeWindow.start.toISOString(), grace_minutes: 10 }, next_steps: [] };
  if (!pipeline || pipeline.source !== 'schedule' || new Date(pipeline.created_at).getTime() <= new Date(schedule.updated_at).getTime() || (!firstRunWindow && new Date(pipeline.created_at).getTime() < expectedWindow.start.getTime())) return result(base, 'execution_missing', 'scheduled-pipeline-missing');
...
function getFirstScheduledEvidenceWindow(schedule: any, pipeline: any, expectedWindow: { start: Date }): Date | null {
  if (!pipeline || pipeline.source !== 'schedule' || typeof schedule?.updated_at !== 'string' || typeof pipeline.created_at !== 'string') return null;
  const updatedAt = new Date(schedule.updated_at);
  const createdAt = new Date(pipeline.created_at);
  if (Number.isNaN(updatedAt.getTime()) || Number.isNaN(createdAt.getTime()) || createdAt.getTime() <= updatedAt.getTime()) return null;
  ...
  if (createdAt.getTime() >= firstWindowStart.getTime()) return null;
  return createdAt;
}
...
function validateChangelogArtifacts(target: any, pipeline: any): string[] {
  const evidence = pipeline.artifact_payload;
  const routeDecision = pipeline.supporting_artifact_payload;
  const consumerPlan = pipeline.supporting_artifact_2_payload;
  const errors: string[] = [];
  if (!target.supporting_artifact || !target.supporting_artifact_schema || !target.supporting_artifact_2 || !target.supporting_artifact_2_schema) errors.push('supporting-artifact-target');
  if (pipeline.artifact !== target.artifact) errors.push('artifact');
  if (pipeline.artifact_schema !== target.artifact_schema) errors.push('artifact_schema');
  if (pipeline.supporting_artifact !== target.supporting_artifact) errors.push('supporting_artifact');
  if (pipeline.supporting_artifact_schema !== target.supporting_artifact_schema) errors.push('supporting_artifact_schema');
  if (pipeline.supporting_artifact_2 !== target.supporting_artifact_2) errors.push('supporting_artifact_2');
  if (pipeline.supporting_artifact_2_schema !== target.supporting_artifact_2_schema) errors.push('supporting_artifact_2_schema');
  if (!evidence || typeof evidence !== 'object') errors.push('artifact_payload');
  if (!routeDecision || typeof routeDecision !== 'object') errors.push('supporting_artifact_payload');
  if (!consumerPlan || typeof consumerPlan !== 'object') errors.push('supporting_artifact_2_payload');
  if (!evidence || evidence.schema !== 'zj-loop.gitlab_changelog_draft_evidence.v1') errors.push('artifact.schema');
  if (!routeDecision || routeDecision.schema !== 'zj-loop.route_decision.v1') errors.push('supporting_artifact.schema');
  if (!consumerPlan || consumerPlan.schema !== 'zj-loop.consumer_run_plan.v1') errors.push('supporting_artifact_2.schema');
  if (evidence?.status !== 'completed') errors.push('artifact.status');
  if (evidence?.outcome !== 'draft-evidence') errors.push('artifact.outcome');
  if (evidence?.audit?.project_path !== target.project) errors.push('artifact.audit.project_path');
  if (evidence?.audit?.draft_mode !== 'evidence') errors.push('artifact.audit.draft_mode');
  if (evidence?.audit?.side_effects_executed !== false) errors.push('artifact.audit.side_effects_executed');
  const window = evidence?.audit?.release_window;
  if (window?.repo !== target.project) errors.push('artifact.audit.release_window.repo');
  if (window?.base_branch !== 'master') errors.push('artifact.audit.release_window.base_branch');
  if (!window?.since_ref) errors.push('artifact.audit.release_window.since_ref');
  if (!window?.until_ref) errors.push('artifact.audit.release_window.until_ref');
  if (routeDecision?.route !== target.route_id) errors.push('supporting_artifact.route');
  if (routeDecision?.request_kind !== 'report-only') errors.push('supporting_artifact.request_kind');
  if (routeDecision?.target_consumer !== 'changelog-drafter') errors.push('supporting_artifact.target_consumer');
  if (routeDecision?.source !== 'gitlab-release-window') errors.push('supporting_artifact.source');
  if (routeDecision?.allowed !== true) errors.push('supporting_artifact.allowed');
  if (consumerPlan?.route_id !== target.route_id) errors.push('supporting_artifact_2.route_id');
  if (consumerPlan?.consumer !== 'changelog-drafter') errors.push('supporting_artifact_2.consumer');
  if (consumerPlan?.execution_mode !== 'report-only') errors.push('supporting_artifact_2.execution_mode');
  if (consumerPlan?.request_kind !== 'report-only') errors.push('supporting_artifact_2.request_kind');
  if (consumerPlan?.status !== 'report-only') errors.push('supporting_artifact_2.status');
  if (consumerPlan?.execution_allowed !== false) errors.push('supporting_artifact_2.execution_allowed');
  if (consumerPlan?.validation?.valid !== true) errors.push('supporting_artifact_2.validation.valid');
  if (!consumerPlan?.route_decision || JSON.stringify(consumerPlan.route_decision) !== JSON.stringify(routeDecision)) errors.push('supporting_artifact_2.route_decision');
  return errors;
}
...
function buildScheduleHealthReplayCommand(target: any, apiUrl?: string, expectedWithinMinutes?: unknown): string[] {
  ...
  if (target.supporting_artifact_2) command.push('--supporting-artifact-2', String(target.supporting_artifact_2));
  if (target.supporting_artifact_2_schema) command.push('--supporting-artifact-2-schema', String(target.supporting_artifact_2_schema));
