...
export async function runScheduleProbeCli(input: ScheduleProbeCliInput = {}): Promise<number> {
  ...
  let activeProbeId: string | undefined;
  let signal: 'SIGINT' | 'SIGTERM' | undefined;
  let cleanupPromise: Promise<any> | undefined;
  let common: any;

  const beginCleanup = () => {
    if (!signal || !activeProbeId || cleanupPromise) return;
    cleanupPromise = runner.restoreGitLabScheduleProbe({ ...common, probeId: activeProbeId });
  };
  const handleSignal = (received: 'SIGINT' | 'SIGTERM') => {
    signal ??= received;
    controller.abort();
    beginCleanup();
  };
  ...
  try {
    return await runCli({
      ...
      async handler({ io, options }) {
        ...
        if (!token) throw new Error('GitLab token environment variable is required');
        ...
        if (action === 'resume') activeProbeId = String(options.probeId ?? '');
        const onArmed = (state: any) => { activeProbeId = state.probe_id; beginCleanup(); };
