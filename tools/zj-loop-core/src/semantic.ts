...
type RecommendationMatchField = {
  code: string;
  sourceField: string;
  points: number;
  values: (pattern: RegistryPattern) => string[];
  message: (matchedValue: string, word: string) => string;
};

type RecommendationBoostRule = {
  useCaseIncludes: string;
  patternIdIncludes: string;
  code: string;
  message: string;
  points: number;
};
...
type RecommendationPolicy = {
  ...
  confidenceBands: Array<{ minScore: number; confidence: 'low' | 'medium' | 'high' }>;
};

const RECOMMENDATION_POLICY: RecommendationPolicy = {
  minWordLength: 3,
  matchFields: [
    {
      code: 'match.id',
...
function applyWordMatches(pattern: RegistryPattern, word: string, reasons: SemanticReason[]): number {
  let score = 0;

  for (const field of RECOMMENDATION_POLICY.matchFields) {
    for (const value of field.values(pattern)) {
      if (!includesNeedle(value, word)) continue;
...
function applyBoosts(pattern: RegistryPattern, lowerUseCase: string, reasons: SemanticReason[]): number {
  let score = 0;

  for (const boost of RECOMMENDATION_POLICY.boostRules) {
    if (!lowerUseCase.includes(boost.useCaseIncludes) || !pattern.id.includes(boost.patternIdIncludes)) continue;
