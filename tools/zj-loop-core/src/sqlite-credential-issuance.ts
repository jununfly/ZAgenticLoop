...
function legacyPairingCredentialIssuanceDigest(input: PairingCredentialIssuanceRequest): string {
    ...
    async issuePairingIntent(request) {
      ...
      return atomic((database, appendEvent) => {
        ...
        if (existing) {
          ...
          if (existing.issuance_digest !== issuanceDigest.slice('sha256:'.length) && existing.issuance_digest !== legacyDigest.slice('sha256:'.length)) throw new Error('request-id-conflict');
          if (existing.issuance_digest === legacyDigest.slice('sha256:'.length)) database.prepare('UPDATE credential_issue_intents SET issuance_digest = ? WHERE request_id = ?').run(issuanceDigest.slice('sha256:'.length), request.request_id);
