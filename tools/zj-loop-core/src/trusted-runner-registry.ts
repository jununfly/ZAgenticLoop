...
function normalizeCapabilities(capabilities: string[] = []): string[] { return [...capabilities].sort(); }
export function validateTrustedRunnerCapabilities(capabilities: string[] = []): { status: 'valid' | 'blocked'; errors: string[] } {
  const errors: string[] = [];
  if (!Array.isArray(capabilities) || new Set(capabilities).size !== capabilities.length || !capabilities.every((capability) => typeof capability === 'string' && (TRUSTED_RUNNER_CAPABILITIES as readonly string[]).includes(capability))) errors.push('registry-capability-unknown');
  return { status: errors.length === 0 ? 'valid' : 'blocked', errors };
}
export function trustedRunnerCapabilitiesDigest(capabilities: string[] = []): string { return `sha256:${createHash('sha256').update(canonicalize(normalizeCapabilities(capabilities)) as string, 'utf8').digest('hex')}`; }
...
export function applyTrustedRunnerRegistryMutation(input: {
  ...
  else current.capabilities = normalizeCapabilities(input.mutation.capabilities ?? []);
  return { status: 'recorded', registry };
}
