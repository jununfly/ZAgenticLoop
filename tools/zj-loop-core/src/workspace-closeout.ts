...
async function writeCloseoutRecord(
  root: string,
  relativePath: string,
  record: WorkspaceCloseoutResult & { schema_version: number; updated_at: string },
): Promise<WorkspaceCloseoutResult> {
...
function resumeCommand(orchestrationId: string): string[] {
  return [
