...
async function readGitWorkspace(root: string): Promise<
  | { ok: true; branch: string; headSha: string; trackedFiles: string[]; untrackedFiles: string[]; trackedPatch: string }
  | { ok: false }
> {
  try {
...
async function gitDiffNoIndex(root: string, file: string): Promise<string> {
  try {
