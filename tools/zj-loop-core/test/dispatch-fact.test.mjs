...
test('task.dispatched fact is CAS-backed and idempotent without Provider side effects', async () => {
  ...
  try {
    ...
    const gate = evaluateDispatchGate({ intent, claim: { status: 'claimed', plan_digest: intent.plan_digest, plan_revision: 1, task_id: 'task-1', node_id: 'codex-node' }, revalidation: { status: 'passed', plan_digest: intent.plan_digest, plan_revision: 1 } });
