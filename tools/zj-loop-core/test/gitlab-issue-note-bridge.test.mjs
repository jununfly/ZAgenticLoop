...
const base = {
  ...
  payload: {
    object_kind: 'issue',
    project: { path_with_namespace: 'example-group/product-project', web_url: 'https://git.example/example-group/product-project' },
    issue: { iid: 42, web_url: 'https://git.example/example-group/product-project/-/issues/42' },
    object_attributes: {
      id: 99,
      ...
      noteable_iid: 42,
      action: 'create',
      url: 'https://git.example/example-group/product-project/-/issues/42#note_99',
    },
  },
};
...
test('accepts a fixed Issue Note marker and emits only a lightweight envelope', () => {
  ...
  assert.deepEqual(result.envelope, {
    ...
    note_id: 99,
    mr_iid: null,
    source_url: 'https://git.example/example-group/product-project/-/issues/42#note_99',
    target_route: 'roadmap-sliced-development',
    target_ref: 'master',
    received_at: '2026-07-17T00:00:00.000Z',
    dedupe_key: 'gln_2acc90912acd0381',
...
test('accepts real GitLab Note Hook payloads without an action field', () => {
  const { action: _action, ...realNoteAttributes } = base.payload.object_attributes;
  const result = buildGitLabIssueNoteBridgeEnvelope({
    ...base,
    headers: { ...base.headers, event: 'Note Hook', eventId: 'real-note-event-1' },
    payload: { ...base.payload, object_kind: 'note', object_attributes: realNoteAttributes },
  });
  assert.equal(result.status, 'accepted');
  assert.equal(result.envelope?.event_type, 'Note Hook');
  assert.equal(result.side_effects_executed, false);
