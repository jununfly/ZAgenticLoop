...
const environment = { cwd: process.cwd(), sandbox_policy: '(version 1) (deny network*) (allow process*) (allow file-read*)', env_allowlist: ['PATH', 'LANG'], env: { PATH: '/usr/bin', LANG: 'C' } };
...
test('MacOSTrustedRunner adapter accepts a signed observation from an active registry runner', { skip: !isMacOS }, async () => {
  ...
  try {
    ...
    const first = request(helper.binary, {
      ...
      ...environment, ...macosEnvironmentPolicyDigests(environment),
    });
    ...
    assert.equal(result.observation?.environment_proof.network_denied.status, 'proved');
...
test('MacOSTrustedRunner adapter blocks network-allowing or credential-bearing policy before helper spawn', { skip: !isMacOS }, async () => {
  ...
  const result = await adapter.run({
    key_tag: 'unused',
    execution: { runner_id: 'runner-1', execution_id: 'execution-unsafe-environment', attempt: 1, preflight_digest: digest('1'), proof_digest: digest('2'), registry_snapshot_digest: macosTrustedRunnerRegistryDigest([]) },
    argv: ['/bin/echo', 'unused'],
    environment: { cwd: process.cwd(), sandbox_policy: '(version 1) (allow default)', env_allowlist: ['GITLAB_TOKEN'], env: { GITLAB_TOKEN: 'secret' } },
    timeout_ms: 100,
    termination_grace_ms: 50,
  });

  assert.deepEqual(result, { status: 'blocked', reasons: ['credential-env-key-forbidden', 'network-deny-policy-missing'] });
});
