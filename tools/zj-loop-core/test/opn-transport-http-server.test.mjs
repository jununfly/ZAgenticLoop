...
test('TransportAdapter long-polls and returns an envelope offered during the wait', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'zj-opn-transport-long-poll-'));
  const stateStore = createSqliteStateStore({ filename: path.join(root, 'state.db') });
  await stateStore.createNetwork({ network_id: 'network-1', owner_id: 'human-1', now: '2026-08-07T12:00:00.000Z' });
  const serverMaterial = await certificate(root, 'server', 'DNS:localhost,IP:127.0.0.1');
  const senderMaterial = await certificate(root, 'sender');
  ...
  const receiverNodeId = createHash('sha256').update(new X509Certificate(receiverMaterial.cert).raw).digest('hex');
  const transport = createOpnTransportHttpService({
    network_id: 'network-1', stateStore,
    credentialVerifier: { verify: async () => ({ status: 'allowed', credential_id: 'credential-1', expires_at: '2026-08-07T13:00:00.000Z' }) },
    now: () => '2026-08-07T12:01:00.000Z',
  });
  const server = createPairingHttpServer({ tls: { key: serverMaterial.key, cert: serverMaterial.cert, ca: `${senderMaterial.cert}${receiverMaterial.cert}` }, recordStore: { append: async () => ({ status: 'recorded', record: {} }), list: async () => [], appendIfPending: async () => ({ status: 'recorded', record: {} }) }, transport });
  ...
  try {
    ...
    const senderSession = await sender.openSession({ network_id: 'network-1', node_id: senderNodeId });
    const receiverSession = await receiver.openSession({ network_id: 'network-1', node_id: receiverNodeId });
    const envelope = createTransportEnvelope({ message_id: 'long-poll-message', network_id: 'network-1', event_id: 'long-poll-event', plan_id: 'plan-1', plan_revision: 1, task_id: 'task-1', from_node_id: senderNodeId, target_node_id: receiverNodeId, notification_kind: 'evidence-available', state: 'available', artifact_refs: [{ artifact_id: digest('c'), content_sha256: digest('d'), kind: 'evidence' }], created_at: '2026-08-07T12:01:00.000Z', expires_at: '2026-08-07T12:30:00.000Z' });
    const receiving = receiver.receive({ session_id: receiverSession.session_id, wait_ms: 2_000 });
    await new Promise((resolve) => setTimeout(resolve, 100));
    await sender.send({ session_id: senderSession.session_id, envelope });
    assert.deepEqual(await receiving, envelope);
  } finally {
    await new Promise((resolve) => server.close(resolve));
    await stateStore.close();
    await rm(root, { recursive: true, force: true });
...
test('TransportAdapter long-poll returns null after its bounded wait', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'zj-opn-transport-long-poll-empty-'));
  const stateStore = createSqliteStateStore({ filename: path.join(root, 'state.db') });
  await stateStore.createNetwork({ network_id: 'network-1', owner_id: 'human-1', now: '2026-08-07T12:00:00.000Z' });
  const serverMaterial = await certificate(root, 'server', 'DNS:localhost,IP:127.0.0.1');
  const receiverMaterial = await certificate(root, 'receiver');
  const receiverNodeId = createHash('sha256').update(new X509Certificate(receiverMaterial.cert).raw).digest('hex');
  const transport = createOpnTransportHttpService({ network_id: 'network-1', stateStore, credentialVerifier: { verify: async () => ({ status: 'allowed', credential_id: 'credential-1', expires_at: '2026-08-07T13:00:00.000Z' }) }, now: () => '2026-08-07T12:01:00.000Z' });
  const server = createPairingHttpServer({ tls: { key: serverMaterial.key, cert: serverMaterial.cert, ca: receiverMaterial.cert }, recordStore: { append: async () => ({ status: 'recorded', record: {} }), list: async () => [], appendIfPending: async () => ({ status: 'recorded', record: {} }) }, transport });
  await new Promise((resolve) => server.listen(0, '127.0.0.1', resolve));
  try {
    const receiver = createTlsTransportAdapter({ endpoint: `https://localhost:${server.address().port}`, ca: serverMaterial.cert, cert: receiverMaterial.cert, key: receiverMaterial.key, bearer_token: 'credential-1' });
    const session = await receiver.openSession({ network_id: 'network-1', node_id: receiverNodeId });
    const started = Date.now();
    assert.equal(await receiver.receive({ session_id: session.session_id, wait_ms: 300 }), null);
