...
test('registry requires an explicit write-enabled mode and forwards it to Codex', async () => {
  const calls = [];
  const provider = createRealAgentDogfoodProvider({
    provider_id: 'codex',
    executable: '/opt/codex/bin/codex',
    process_adapter: {
      async launch(spec) {
        ...
        return {
          ...
          stdin: { end() {} },
          cancel() {},
          async wait() { return { schema: 'zj-loop.local_process_adapter.v1', status: 'completed', success: true, pid: 8, exit_code: 0, signal: null, stdout: 'done', stderr: '' }; },
        };
      },
    },
  });
