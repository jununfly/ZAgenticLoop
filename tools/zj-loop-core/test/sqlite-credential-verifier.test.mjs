...
test('SQLite credential provider stores only an opaque token hash and verifies its scope', async () => {
  ...
  try {
    ...
    assert.deepEqual(await verifier.verify({ token: issued.token, node_id: 'node-1', network_id: 'network-1', operation: 'POST /v1/networks/network-1/events', event_id: 'event-1', task_id: 'task-1', required_capabilities: ['event.append'] }), { status: 'allowed' });
