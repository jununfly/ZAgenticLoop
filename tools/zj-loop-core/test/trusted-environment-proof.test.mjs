...
function fixture() {
  ...
  const environment = createFakeTrustedEnvironmentProof({
    ...
    network_evidence_digest: digest('7'),
...
test('accepts a signed pre-launch proof with network deny and credential-clean evidence', () => {
  const value = fixture();
...
test('blocks when any environment fact is missing or policy digest drifts', () => {
  const value = fixture();
  value.proof.network_denied.status = 'blocked';
  assert.deepEqual(verifyTrustedEnvironmentProof({ ...value, now: '2026-08-01T12:01:00.000Z' }), { status: 'blocked', reasons: ['network-denied-proof-missing', 'trusted-environment-proof-digest-invalid'] });
...
test('builds and validates a deterministic macOS Seatbelt and credential-clean environment policy', () => {
  ...
  const digests = macosEnvironmentPolicyDigests({ sandbox_policy: policy, ...environment });
  assert.equal(validateMacOSTrustedEnvironmentPolicy({ sandbox_policy: policy, ...environment }).status, 'accepted');
  ...
  assert.deepEqual(unsafe, { status: 'blocked', reasons: ['credential-env-key-forbidden', 'network-deny-policy-missing'] });
});
