...
import { parsePatternRegistry, runCli, type CliHandlerContext, type CliSpec, type PatternRegistry } from '@jununfly/zj-loop-core';
import {
  assertValidLevel,
  estimateCost,
  formatEstimateHuman,
...
function registryCandidates(options: Record<string, unknown>): RegistrySource[] {
  const explicitRegistry = typeof options.registry === 'string' ? path.resolve(String(options.registry)) : undefined;
  if (explicitRegistry) return [{ label: 'explicit', path: explicitRegistry }];
...
async function loadRegistryWithSource(candidates: RegistrySource[]): Promise<LoadedRegistry> {
...
async function handleCostCommand({ io, options }: CliHandlerContext) {
  ...
  if (json) io.stdout(JSON.stringify({ registry: loadedRegistrySource, estimate: result }, null, 2));
  else io.stdout(formatEstimateHuman(result, loadedRegistrySource));
}
