...
const DEFAULT_GITLAB_CORE_PACKAGE = '@jununfly/zj-loop-core@0.1.30';
...
async function handleAddArtifacts(artifacts, targetDir, templatesRoot, patterns, dryRun, force, io, gitlabStage, gitlabRunnerTags, gitlabImage, gitlabCorePackage) {
    ...
    io.stdout(`\n=== Next steps ===
  npx --yes --package @jununfly/zj-loop-core@0.1.30 zj-loop-first-run plan --root ${targetDir}
  npx --yes --package @jununfly/zj-loop-core@0.1.30 zj-loop-first-run plan --root ${targetDir} --json
    ...
    return [
        {
            ...
            command: 'npx --yes --package @jununfly/zj-loop-core@0.1.30 zj-loop-route status',
        },
        {
            label: 'Enable roadmap activation only when appropriate',
            command: 'npx --yes --package @jununfly/zj-loop-core@0.1.30 zj-loop-route enable roadmap-sliced-development --confirm "enable roadmap-sliced-development side effects"',
        },
        {
            label: 'Disable a route without a confirmation phrase',
            command: 'npx --yes --package @jununfly/zj-loop-core@0.1.30 zj-loop-route disable roadmap-sliced-development',
        },
    ];
}
...
`;
    ...
    commandIo.stdout(`\n=== Next steps ===
  npx --yes --package @jununfly/zj-loop-core@0.1.30 zj-loop-first-run plan --root ${target === '.' ? '.' : target}
  npx --yes --package @jununfly/zj-loop-core@0.1.30 zj-loop-first-run plan --root ${target === '.' ? '.' : target} --json
...
Options:
  ...
  --gitlab-core-package
                  Package source rendered into generated GitLab npx --package calls (default: @jununfly/zj-loop-core@0.1.30)
