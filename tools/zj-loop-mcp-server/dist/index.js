...
function patternSummariesForLegacyJson(registry) {
...
function formatRecommendationsMarkdown(registry, useCase) {
...
function formatCostEstimateMarkdown(registry, patternId, level, cadence) {
    ...
    if ('code' in result) {
        ...
        if (result.code === 'invalid_cadence')
            return `Invalid cadence: ${result.cadence}`;
...
// ── Tools ──────────────────────────────────────────────────────────
server.tool('loop_list_patterns', 'List all available agentic loop working patterns with their goals, cadences, and risk levels', {}, async () => {
    const root = await resolveProjectRoot();
    const registry = await loadRegistry(root);
    if (!registry) {
...
server.tool('loop_recommend_pattern', 'Recommend the best loop pattern for a given use case', {
    ...
    return { content: [{ type: 'text', text: formatRecommendationsMarkdown(registry, useCase) }] };
});
server.tool('loop_estimate_cost', 'Estimate daily token cost for a pattern at a given readiness level', {
    ...
    return { content: [{ type: 'text', text: formatCostEstimateMarkdown(registry, patternId, level, cadence) }] };
});
server.tool('opn_inbox_read', 'Read one pending OPN message through the local OPN Gateway without acknowledging it', {}, async () => {
...
server.tool('opn_gateway_status', 'Check local OPN Gateway configuration and transport connectivity', {}, async () => {
    const result = await opnGatewayStatus();
    return { content: [{ type: 'text', text: JSON.stringify(result.status === 'ok' ? result.value : { schema: 'zj-loop.opn_mcp_error.v1', status: result.status, reason: result.reason }, null, 2) }] };
});
server.tool('opn_message_send', 'Publish a structured message artifact and send its OPN envelope through the local Gateway', {
