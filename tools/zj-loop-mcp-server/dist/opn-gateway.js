...
export async function opnGatewayStatus() {
    try {
        const value = await config();
        const transport = createTlsTransportAdapter({ endpoint: value.endpoint, ca: value.ca, cert: value.cert, key: value.key, bearer_token: value.credential_token });
        const session = await transport.openSession({ network_id: value.network_id, node_id: value.node_id });
...
export async function opnAgentTaskSend(input) {
    try {
        if (!input.target_node_id.trim() || !input.task_json.trim())
            throw new Error('opn-task-target-and-content-required');
        let task;
        try {
            task = JSON.parse(input.task_json);
        }
        catch {
            throw new Error('opn-task-json-invalid');
        }
