...
async function ownerRequest(input: { value: GatewayConfig; method: 'GET' | 'POST'; pathname: string; body?: Record<string, unknown> }): Promise<Record<string, unknown>> {
  if (!input.value.owner_token) throw new Error('opn-owner-token-required');
  ...
  return await new Promise((resolve, reject) => {
    const req = httpsRequest(options, (response) => { const chunks: Buffer[] = []; response.on('data', (chunk) => chunks.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk))); response.on('end', () => { let body: Record<string, unknown>; try { body = JSON.parse(Buffer.concat(chunks).toString('utf8')) as Record<string, unknown>; } catch { reject(new Error(`opn-owner-http-${response.statusCode}`)); return; } if ((response.statusCode ?? 500) >= 400) { reject(new Error(String(body.reason ?? `opn-owner-http-${response.statusCode}`))); return; } resolve(body); }); });
...
export async function opnTaskDraftList(): Promise<GatewayResult> {
  try {
    const value = await config();
