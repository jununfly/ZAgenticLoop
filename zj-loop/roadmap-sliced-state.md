...
|------|--------|----------|-------------|
| 2-5-route-specific-execution-apis | in-progress | CI Sweeper packaged repair-plan API and `zj-loop-ci-sweeper repair-plan` command added; Post-Merge Closeout packaged closeout-plan API and `zj-loop-post-merge-closeout closeout-plan` command added; Issue Fix Request contract helpers moved into core; Dependency Sweeper packaged repair-plan API and `zj-loop-dependency-sweeper repair-plan` command added; PR Steward packaged fix-plan API and `zj-loop-pr-steward fix-plan` command added with source PR head guard, independent repair PR/escalation actions, and no source-PR side-effect evidence; `cd tools/zj-loop-core && npm test`; next slice: continue route-specific execution API migration for remaining draft/action consumers | pending |
