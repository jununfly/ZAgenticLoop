...
|------|--------|----------|-------------|
| 2-5-route-specific-execution-apis | in-progress | CI Sweeper packaged repair-plan API and `zj-loop-ci-sweeper repair-plan` command added; deterministic command plan is package-list driven and covered by `cd tools/zj-loop-core && npm test`; `node tools/zj-loop-audit/dist/cli.js .`; `git diff --check`; next slice: continue route-specific execution API migration for remaining repair/draft/cleanup consumers | pending |
