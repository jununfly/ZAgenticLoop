...
|------|--------|----------|-------------|
| 2-5-route-specific-execution-apis | in-progress | CI Sweeper packaged repair-plan API and `zj-loop-ci-sweeper repair-plan` command added; Post-Merge Closeout packaged closeout-plan API and `zj-loop-post-merge-closeout closeout-plan` command added for contract parsing, GitHub input collection, dry-run guard evidence, and structured dry-run comments; `cd tools/zj-loop-core && npm test`; next slice: continue route-specific execution API migration for remaining repair/draft/cleanup consumers | pending |
