...
|------|--------|----------|-------------|
| 2-5-route-specific-execution-apis | in-progress | CI Sweeper packaged repair-plan API and `zj-loop-ci-sweeper repair-plan` command added; Post-Merge Closeout packaged closeout-plan API and `zj-loop-post-merge-closeout closeout-plan` command added; Issue Fix Request contract helpers moved into core; Dependency Sweeper packaged repair-plan API and `zj-loop-dependency-sweeper repair-plan` command added; PR Steward packaged fix-plan API and `zj-loop-pr-steward fix-plan` command added; Changelog Drafter packaged draft-plan API and `zj-loop-changelog-drafter draft-plan` command added; Issue Triage Action packaged action-plan API and `zj-loop-issue-triage-action action-plan` command added with allowlist/human-guard/live-rejection evidence; `cd tools/zj-loop-core && npm test`; next slice: migrate Roadmap Activation execution API | pending |
